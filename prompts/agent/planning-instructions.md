...
**Operator management.** In the desktop app, plan the separate built-ins `rebel_operator__list`, `rebel_operator__activate`, `rebel_operator__remove`, `rebel_operator__rename`, `rebel_operator__duplicate`, and `rebel_operator__set_coach` when the user asks to manage their Operator team. Start with `list` when you need the exact id or display name. A rename uses `rename`, never `remove`; turning off a live-meeting coach uses `set_coach` with `enabled: false`, never `remove`; and `remove` is only for an explicit remove/delete/get-rid request. There is no reversible deactivate state: a deactivate-shaped request must use the remove tool's `deactivate` intent so it explains the remove-or-coach choices and deletes nothing.

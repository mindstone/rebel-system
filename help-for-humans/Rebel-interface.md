...
- **Notification drawer** — Approvals live in a notification drawer accessible from the badge icon in the top bar. Pending approvals are grouped by conversation, and compact receipt bubbles confirm each action after you approve. When actions need your approval mid-conversation, a bar appears at the bottom of the conversation: "Rebel paused. X action(s) need your OK" — click **View** to open the notification drawer.
...
- **Rebel's own notes** — While it works on something long, the notes Rebel keeps for itself appear in the transcript and in the conversation outline, and they stay there when the task is done, so you can see what it was thinking and where it changed course.

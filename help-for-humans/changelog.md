...
- **Memory approvals get a middle gear** — Rebel's memory safety ladder now has four plain-English choices for when Rebel asks; the new **Ask only when Rebel spots something specific** option handles routine saves while still stopping when Rebel spots something specific — and when it does, the card tells you what that is. If **Ask whenever Rebel is unsure** interrupts too often, you can ask less from the card, change it back any time, and audit automatic saves in the Activity log — shared-space password and key checks stay on throughout. Fewer shrugs, more useful interruptions.
...
<!-- detail: 260724_alex-cost-fixes Stages 2-4 (plan docs/plans/260724_alex-cost-fixes/PLAN.md; CE2 bug_mode). (S2) Sticky OpenRouter session affinity: conversation-scoped x-rebel-affinity-key (rendererSessionId ?? turnId, degenerate-key guarded) hashed by the proxy to an opaque x-session-id so implicit prompt caching can hit on the managed GLM path; raw session ids never egress. (S3) hasNativeContextEditing now a per-request function-of-model (mirrors supportsImageContent), so client-side tool-result pruning re-engages on non-Anthropic routes where the proxy strips context_management; prune-summarizer routed through the cheap BTS path. (S4) Cache-intelligent client pruning: fires when removable tool-result tokens ≥ 10% of context window, prunes to a low-water mark; emergency/pressure pruning never suppressed. Monitoring extends the existing Cost Incurred event (roundCount, orProviderCount, prunedToolPairs, compactionEvents, appVersion) — no new event volume. Do NOT claim measured savings — fleet telemetry not yet in. Keep public copy non-technical — no x-session-id/affinity/pruning/context-management internals. -->
- **Long conversations and automations cost less to run** — As a conversation or automation grows long, Rebel now trims the old tool results it no longer needs in tidy batches instead of carrying every last one along — and on Mindstone subscription plans, it keeps its working memory warm between turns so the heavy lifting doesn't start from scratch each time. Same answers, lighter work behind the scenes. Nothing for you to set up; it just behaves better as the thread gets long.

<!-- detail: 260724_alex-cost-fixes Stage 5 (plan docs/plans/260724_alex-cost-fixes/PLAN.md; CE2 bug_mode). Adds 'automation' to SKIP_MEMORY_UPDATE_KINDS so successful automation runs no longer spawn a post-turn memory agent. Other completion side effects in the same block (duration tracking, streaks, badges) still run — pinned by a handler-level test. Keep public copy non-technical — no SKIP_MEMORY_UPDATE_KINDS/handler internals. -->
...
<!-- detail: 260724_alex-cost-fixes Stage 1 (plan docs/plans/260724_alex-cost-fixes/PLAN.md; CE2 bug_mode). Recovery-ladder depth-4 was silently substituting a more-expensive model when an override-pinned background/automation turn overflowed its context (CONFIRMED fleet bug: haiku automations silently served opus on context overflow; recovery_depth_4_invocation 16/16). Gate now requires the candidate profile to be configured+enabled+credential-reachable, have a larger contextWindow, and a provider-aware weighted price scalar ≤ the pinned model's — otherwise the turn fails closed with a terminal user-visible error bridged into automation status on both desktop and cloud. CostTier deliberately NOT used (haiku and sonnet both mid-tier → would permit 3× increase). Telemetry: turnFallbacks/recoveryDepth/servedModelDiffersFromOverride on Automation Run Cost; recovery_depth_4_invocation enriched. Do NOT claim measured savings — fleet telemetry not yet in. Keep public copy non-technical — no recovery-ladder/depth-4/CostTier/override-pin internals. -->
- **Automations stop quietly upgrading themselves to a pricier model** — When an automation hit the limits of the model you'd picked for it, Rebel used to quietly swap in a much more expensive one and carry on as if nothing had happened — your budget none the wiser. Now it either stays on the model you chose or stops with a clear reason, instead of running up the bill behind your back. Nothing for you to do; the safety net just stops being made of money.

<!-- detail: 260726_meeting-capture-eval-redness (plan docs/plans/260726_meeting-capture-eval-redness/PLAN.md; CE2 bug_mode). Two background-automation fixes in the source-capture pipeline. (a) Retrieval visibility: the source-capture eval registered Fireflies tools under a generic "GoogleWorkspace" label with no agent-visible surface, so the agent made zero retrieval attempts across six consecutive runs — the meeting-transcript lane was effectively invisible. The eval now advertises Fireflies as a first-class package matching production connector discovery; the automation prompt now pins a mandatory meeting-transcript retrieval lane. (b) Malformed-note contract enforcement: a deterministic validator in core plus a fail-open one-repair hook at the production write path gives malformed source notes ONE bounced repair with actionable codes, then always allows/stages — never silently dropped, never denied forever. Cloud parity fixed (cloud never ran the validator at all). End-to-end capture correctness is measured but not yet fixed (R1 spin-out). Keep public copy non-technical — no eval/validator/hook/compound-key internals. -->
- **Rebel now actually looks for your meeting transcripts — and stops filing the broken ones** — Rebel's background capture had two quiet habits: in some setups it never went looking for your meeting transcripts at all (the lane was effectively invisible to the agent), and when a captured note came out malformed it filed it anyway — missing its source link or in the wrong folder shape, quietly degrading everything downstream. Now Rebel looks for transcripts properly, and if a note comes out malformed it asks itself to fix it once before filing — and if the fix doesn't take, it files it anyway rather than throwing your meeting away. You won't notice either unless you go looking; that's rather the point.
...
<!-- detail: 260725_async-session-persist (plan docs/plans/260725_async-session-persist/PLAN.md; CE2 bug_mode heavy, intent_critical). Every ordinary locked session write — including the 15s turn checkpoint — ran upsertSessionsSyncWithReload synchronously on the Electron main thread inside the process-wide global-index section (3.6MB index read+parse+validate, session write, backup re-read+re-validate+write, primary commit; up to three blocking fsyncSync). Measured on a user machine: 11.6s main-thread block, 12.3s section, co-occurring with an unresponsive cloud-drive file provider. Fix: thin async I/O twin (incrementalSessionStore.upsertSessionsWithReload) awaited by the ordinary locked writers, plus a neutral sessionPersistenceCoordinator owning one same-process transaction boundary with synchronous admission tracking; shutdown closes admissions, drains (3s), and SKIPS finalization on timeout. Sync variants retained for quit/beforeunload and the headless CLI only, lint-banned elsewhere (Stage 6). Also: mainThreadStallProbe now measures per-tick drift (retires a self-generated false-stall metronome), and new attribution telemetry (arrayBuffersMB, locked-reload phase timings, path/trigger tags, session_persistence_transaction_stalled). Wall time under filesystem distress is UNCHANGED — what's freed is the event loop; do not claim saves got faster. Keep public copy non-technical — no coordinator/fsync/checkpoint/telemetry internals, and platform-agnostic (no "macOS"/beachball). -->
- **Rebel no longer freezes while it saves your conversation** — Rebel saves your conversation as you go, and until now the whole app held its breath while it did: usually imperceptible, but if your files live on a cloud-synced drive having a slow morning, that pause could stretch into several seconds of nothing responding at all. Saving now happens quietly in the background — the window keeps scrolling, typing keeps typing, and a sluggish drive becomes the drive's problem rather than yours. Your work is kept just as carefully as before; it simply stops making a scene about it.

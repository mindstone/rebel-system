...
<!-- feature: bash-just-once | detail: Approving a command with Just this once re-checked whether commands are the kind of thing Rebel can hold a standing rule for (they never are, by design) instead of whether the person had just approved this run, so every such approval was refused at settlement with the rules-blocked message. It now honours the one-time approval; always-allow for commands is unchanged. -->
- **"Just this once" now runs the command you approved** - Since early September, approving a command that way came back with "That approval wasn't applied" and nothing ran, whatever your safety rules said. Rebel was checking whether it could remember your answer for next time (it never remembers commands, on purpose) instead of whether you had just said yes. It now runs the one you approved, including a command that uses one of your saved keys, which before could not be run by any route. Always-allow for commands stays off, as before.
...
<!-- feature: codex-bts-model-truth | detail: On a ChatGPT-subscription profile, a stored behind-the-scenes model the plan does not serve is remapped within its own class (a mini stays a mini) and Settings shows the model actually used, instead of a silent promotion to gpt-5.5 on every background call. -->
- **Background work on ChatGPT plans stays on the cheap model you chose** - If your plan does not serve the exact background model you picked, Rebel now uses the closest one of the same size and Settings says which, instead of quietly running every background task on the most expensive model and charging your plan for it.

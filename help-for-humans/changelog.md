...
<!-- detail: 260623_provider-status-probe. When a turn fails with an AI-service/provider server error (e.g. an Anthropic 529 overload storm), the error notice now offers a "Check <Provider> status" link (Anthropic → status.claude.com, OpenAI → status.openai.com, OpenRouter → status.openrouter.ai) so the user can confirm for themselves whether the provider is having an outage rather than assuming Rebel is broken. The link is a pure registry lookup (no live fetch) and corroborates only — it NEVER changes the verdict or claims "no outage". Separately, the diagnostics provider-reachability panel now surfaces public status-page info (incl. a live status check, diagnostics-only, surfaced only on major/critical incidents). Do NOT mention Statuspage / JSON / internal mechanics in public copy. -->
- **When the AI service has a wobble, see for yourself whose fault it is** — If a message fails because the AI provider is having a moment, Rebel now offers a "Check Anthropic status" (or OpenAI, or OpenRouter) link right on the error, so you can glance at the provider's own status page and confirm it's them having an outage — not Rebel quietly breaking. We don't claim innocence; we just hand you the evidence. The diagnostics panel shows the same status info too, for when you're digging in.

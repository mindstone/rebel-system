...
- **A friendlier nudge when a tool action needs tweaking** — When Rebel approves an action but the tool wants its inputs in a slightly different shape, you used to get an alarming red error full of developer jargon ("args must be an object", "use get_tool_details"). Now Rebel quietly says it's sorting it out and fixes the inputs itself — and only asks you for help if it genuinely can't work it out on its own. Less cryptic, more capable colleague.

...
<!-- detail: When the safety evaluator errored (provider rate-limit, outage, or transient failure), a memory write was staged for your decision with a "Do it once" card — but the staging path never recorded the publication authority the approval publisher requires, so clicking "Do it once" was mapped to a silent false success (optimistic removal + "Saved" toast) and the card reappeared ~60s later, indefinitely, for as long as the evaluator stayed unavailable. The fix persists the staged publication authority at every staging site, keyed to the file's own identity rather than the current attempt's, so one click publishes the captured bytes directly — no re-evaluation, no token, no retry. Files that genuinely can't be confirmed now return an honest, actionable error instead of a silent success, and the card stays put with the real reason. Fail-closed safety for unapproved writes is unchanged. -->

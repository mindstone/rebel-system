...
- **Windows: "not this one again"** — on a Windows command card you couldn't tell Rebel to stop asking about that action for the rest of the conversation, though you could everywhere else. Now you can, for commands Rebel can read and verify. One gap is still open and we'd rather name it than let you find it: a command carrying a credential — an API key, an access token — is still asked about every single time on Windows, however often you approve it. That needs a deeper repair than this release contains.

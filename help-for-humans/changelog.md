...
<!-- detail: 260818_low-ram-mode (internal planning doc; CE2 run). Efficiency Mode v2: (1) turning it on now also switches file indexing from a GPU worker window (~1 GB of memory) to the CPU path — search quality and availability unchanged, but indexing throughput drops, so newly added files can take longer to appear in search while the mode is on; the Settings copy says so honestly. (2) On machines with under 12 GiB of RAM, Rebel OFFERS (does not auto-enable) Efficiency Mode via a dismissible Home card — nothing changes until the user clicks Turn on; declining or later disabling stops the offer. Off-ramp is one switch: Settings → System → Efficiency Mode (findable by searching "low RAM", "memory", "slow"…). App-settings compatibility floor advanced 16→18 accordingly. Keep public copy non-technical — no gpuWorker/embedding/epoch/ledger internals. -->
- **Rebel now goes easy on computers with less memory** — On lighter machines it automatically switches on **Efficiency Mode**, trimming memory use and easing off background work so things stay smooth instead of sluggish. Search still works; newly added files can take a little longer to show up in it while the mode is on, and you can switch it off any time in **Settings → System**. The considerate houseguest.

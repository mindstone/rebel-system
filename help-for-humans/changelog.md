...
<!-- detail: Rebel can now use a different model for each step of a turn — a quick one for the easy parts, a stronger one for the hard parts — instead of running everything on the most expensive option available to you. It's still being tuned, so it's off unless you turn it on, and the thinking panel shows which model handled each step. -->

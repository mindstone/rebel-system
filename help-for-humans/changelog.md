...
- **Capture an action straight from the widget — even with no signal** — The widget gets a third button: **Add**. Tap it, type the thing, done. Rebel saves it on your phone first and syncs when it can, so a lift, a tunnel, or a dead spot doesn't cost you the thought — and if something genuinely can't sync, the action stays put with a tap to retry instead of quietly vanishing. ("Conversation" is now **Talk**, to make room.) Captured first, synced later.

...
<!-- detail: When the model you selected is not available on your ChatGPT plan, Rebel picks the closest one that is, says so once in the conversation, and keeps that choice for the session. You only see a refusal when your plan runs none of them. -->
- **If your ChatGPT plan cannot run the model you picked, Rebel now runs the closest one it can** — For two days some ChatGPT subscribers got a bare "model not found" for a model their plan does not serve, and picking GPT-5.6 Sol quietly ran GPT-5.5 instead. Rebel now switches to the nearest model your plan does run, tells you in the conversation which model actually answered, and remembers that for the rest of the session. Nothing to reconnect, no setting to change; your choice stays as you set it.

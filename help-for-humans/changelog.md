...
<!-- detail: 260702_fable-reenable. Anthropic restored Claude Fable 5 API access (withdrawn ~1-2 weeks in 2026-06). Re-enabled in the app: catalog isMainModel/isAuxiliaryModel flipped back to true on both the direct and OpenRouter Fable rows, featured add-a-model preset restored, RUN_FABLE_LIVE_TESTS gate removed. Deliberately kept OFF the quality slider (2× Opus price, often overkill) — reachable only via the Planner/Main work role pickers + add-a-model preset; Maximum stays Opus 4.8. Defaults unchanged. Keep public copy non-technical — no flag names, tier internals, or catalog details. -->
- **Fable 5 is back on the menu** — Anthropic's most capable model went dark for a couple of weeks; it's available again. You'll find it in your **Planner** and **Main work** model pickers, or add it from the model wizard. We've deliberately kept it off the Quality slider — it's our priciest model and usually more than a task needs — so it's there when you decide to reach for it, not something you drift into by nudging a slider.

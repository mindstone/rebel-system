...
<!-- detail: When a recording service handed Rebel the same meeting under a new identifier, or Rebel lost track of what it had already handled, the same transcript could be filed again and again — quietly repeating background work through the night. Rebel now recognises a recording by what it actually is rather than by the label the service gave it, and files each version of a transcript exactly once. Separately, meetings captured by the cloud meeting bot are now shared once the polished transcript arrives rather than at the moment of saving — sometimes a few hours later, and no longer twice. -->
- **A meeting transcript gets filed once, not on a loop** — If a recording service quietly renamed a recording between syncs, Rebel could treat it as brand new and file the same meeting all over again — every few minutes, all night, with nothing to show for it. Rebel now recognises a recording it has already handled and files each transcript exactly once. Meetings captured by the cloud meeting bot are also shared once their polished transcript arrives, which can be a few hours after the meeting — but only once. Once is plenty.

...
<!-- detail: Rebel's memory safety ladder now has four plain-language choices for when Rebel asks before saving. Shared Spaces can use the three asking options; "Save without asking" is for private Spaces. The new "Ask only for specific concerns" choice is opt-in, and existing Spaces keep "Ask when Rebel isn't sure" unless you change them. In one two-week sample on one setup, 29 of 147 saves to shared Spaces — about 1 in 5 — produced an approval card, and 19 of those cards came from uncertainty rather than a named concern. The new choice is designed to remove only those uncertainty-only interruptions when Rebel names no concern; the future reduction has not yet been measured and will vary. Built-in checks for passwords and keys continue to run on shared Spaces. -->
- **Memory approvals get a middle gear** — Rebel's memory safety ladder now has four plain-English choices for when Rebel asks; the new **Ask only for specific concerns** option handles routine saves while still stopping when Rebel can name what it is worried about — and when it does, the card tells you what that is. If **Ask when Rebel isn't sure** interrupts too often, you can ask less from the card, change it back any time, and audit automatic saves in the Activity log — shared-space password and key checks stay on throughout. Fewer shrugs, more useful interruptions.

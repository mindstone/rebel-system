...
<!-- detail: 260727_sentry-triage-beta-batch (plan docs/plans/260727_sentry-triage-beta-batch/PLAN.md; CE2 bug_mode). Reliability batch covering REBEL-6BE, REBEL-6DF, and REBEL-6EJ. Turns delayed while the computer sleeps or is busy now get one fresh wait period instead of being abandoned immediately; genuinely stuck setup remains bounded. Workspace/email connectors now get one automatic startup retry, except when the connector is missing or access is denied. The cloud-Space availability check now uses the same health window as the rest of Rebel, removing the still-healthy false alert while genuine offline/reconnecting states remain. Keep public copy non-technical. -->
...
<!-- detail: The eval exposed Fireflies tools under a generic package name, unlike production's named connector discovery, and recorded zero transcript-retrieval traces in six runs. The automation prompt now requires directly querying connected transcript services, while a shared note validator sends malformed paths, source types, or source links back for one repair attempt before allowing the capture through. Retrieval improved to 4/4 post-parity runs, but whole-fixture proof remained 0/3 on downstream key/appendix correctness, so end-to-end capture correctness remains open. -->
- **Meeting transcripts get an explicit check, and malformed notes get one repair attempt** — Rebel's background capture now treats connected meeting-transcript services as a source it must check rather than assuming the calendar was enough; notes with the wrong path or missing source details are sent back for one repair, then preserved even if the repair still fails. Better filing, without throwing away the evidence.

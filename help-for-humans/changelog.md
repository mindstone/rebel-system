...
<!-- detail: Source-capture recovery cards now use one plain explanation and the same action labels in the drawer, strip, and review dialog. Save appears only when filing remains possible; Keep note says where the note went and offers View when the saved path is available; Review or change opens on the instruction field; Discard is visually subordinate until its confirmation. Resolution telemetry is content-free. -->
- **Captured notes that need help now ask clearly** — When Rebel couldn't file a captured note automatically, the rescue card used to explain the same mishap twice and offer a muddle of **Save**, **Save privately**, and **Preview**. It now gives you one explanation and four honest choices: **Save to** the intended Space when that can work, **Keep note**, **Review or change**, or **Discard** behind a confirmation. Keeping a note tells you where it went and offers to open it. Fewer riddles; same safely kept note.

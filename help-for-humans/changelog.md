...
<!-- detail: 260807_rule-suggestion-reconfirm-loop (plan docs/plans/260807_rule-suggestion-reconfirm-loop/PLAN.md; CE2 bug_mode; internal incident report re conversation rebel://conversation/ec18955d-dcf8-4fd0-a9bf-c6f270f43d4f, screenshot of the drawer card). The "Remember this for next time?" consent card (DrawerRuleSuggestionCard) was essentially uncompletable: clicking Add rule regenerated the rule text via a fresh non-deterministic LLM call from the selected scope's label, then exact-string-compared the result against the text the user had just reviewed. Two independent high-temperature LLM samples from different prompts virtually never normalize equal, so the compare fired on every scope selection (expected) and every rephrasing (benign), the card showed "The rule changed since you last looked — review and confirm," and clicking Confirm add re-ran the same generation and looped again — the user gave up and the rule was never written (confirmed absent from safety-prompt.json). The shipped-but-unused generatedFromVersion CAS field (flagged in the 260703_approval_rule_suggestion_latent_defects postmortem's cross-family review, shipped unfixed) was the intended clobber detector that was dropped in favor of "always regenerate." Fix (6 stages, cross-family reviewed): a per-scope canonical draft (plain-language clauses; shown = stored, humanized at generation) is generated once and cached by a new useScopedPrincipleDrafts hook; Add commits those exact clauses through a dedicated safety-prompt:commit-principle channel whose .strict() request structurally refuses a renderer-authored document and whose response is a fail-closed z.discriminatedUnion('applied', …) — only {applied:true} resolves "Added," a missing/malformed field never reads as success. basePromptHash (SHA-256 of the live prompt) is the cross-surface CAS token (numeric revisions diagnostic-only — desktop and cloud revision domains are independent); a hash mismatch rejects without writing and refreshes one fresh draft for re-consent (a second conflict is terminal with Skip). Consent writes skip the immediate post-update consolidation that drives version drift on the ordinary path, so conflicts are rare here. The string-compare reconfirm machinery (state, normalizePrinciple, confirm-add UI, banner, three pinning tests) was deleted. Also: stripMarkdown no longer leaks list markers (- / * / 1.) into card copy; the shared humanizeToolAction formatter moved to @rebel/shared; an ESLint rule structurally enforces no side effects after a rejected safety-prompt write. Mobile/cloud consent UI still deferred (safe no-op — no silent write on those surfaces; the core fix is surface-agnostic). Keep public copy non-technical — no CAS/discriminated-union/clauses/consolidation internals; say "the rule you reviewed is the rule that's saved," "no longer asks twice." -->
...
<!-- detail: same run (260807_rule-suggestion-reconfirm-loop, Stage 6). The persistence-intent detector prompt (rebel-system/prompts/safety/eval-system.md) treated bare factual/preference "remember" language ("remember that's what we use for WhatsApp in future") as standing permission to approve-and-stop-asking, so it minted a "Remember this for next time?" card for a preference the user never asked Rebel to act on — the most baffling class of card. Amended prompt: factual/preference "remember" language is NOT standing permission unless it explicitly concerns future approval/checking behavior; negative fixture is the incident wording; true positives are "stop asking," "always allow," "remember this approval." Eval-gated: npm run eval:safety-chat-intent-gate baseline must hold or the stage doesn't ship. Does NOT modify buildSelectedLabel, add memory-write suppression, or touch dormant auto-persist. Keep public copy non-technical — say "fewer confusing remember cards." -->

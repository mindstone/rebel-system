...
<!-- detail: 260810_eval-error-approval-loop (plan docs/plans/260810_eval-error-approval-loop/PLAN.md; CE2 bug_mode). When the safety evaluator errored (provider rate-limit, outage, or transient failure), a memory write was staged for your decision with a "Do it once" card — but the staging path never recorded the publication authority the approval publisher requires, so clicking "Do it once" was mapped to a silent false success (optimistic removal + "Saved" toast) and the card reappeared ~60s later, indefinitely, for as long as the evaluator stayed unavailable. The fix persists the staged publication authority at every staging site, keyed to the file's own identity rather than the current attempt's, so one click publishes the captured bytes directly — no re-evaluation, no token, no retry. Files that genuinely can't be confirmed now return an honest, actionable error instead of a silent success, and the card stays put with the real reason. Fail-closed safety for unapproved writes is unchanged. Keep public copy non-technical — no staging/authority/evaluator/tombstone internals. -->
...
<!-- detail: 260808_live-voice-mode (plan docs/plans/260808_live-voice-mode/PLAN.md; CE2). Live mode: hold the mic ~700ms to enter a duplex OpenAI Realtime session (gpt-realtime-2.1, GA endpoint) — PCM16/24kHz, semantic VAD for turn detection, server-side barge-in. Tap still means ordinary dictation. Rebel stays the brain: the realtime model holds function tools that dispatch INTO Rebel, so Live reaches Rebel's MCPs, tools and context rather than running a parallel agent. Spoken answers carry honesty state (provisional | verified | unverified | corrected); corrections link via supersedesResponseId rather than silently replacing, so a fast provisional answer can't leave the user confidently wrong without a trace. speechChannelCoordinator arbitrates the three audio-output regimes (TTS queue, meeting-coach relay, realtime) first-owner-wins — a channel model, not a device model; Live and the meeting coach are mutually exclusive. Missing OpenAI key surfaces a plain error pointing at Settings. Grounding is prompt-and-eval enforced rather than construction-enforced (accepted residual risk of the locked design); the live-voice-grounding eval (20 fixtures) probes it but has NOT yet run against a live model. Keep public copy non-technical — no realtime/VAD/WebSocket/PCM internals. -->
...
<!-- detail: Stage 1b of the same run. Planning was previously coupled to configuring a DISTINCT Thinking model, so the whole planner design — and its three outcomes direct_answer / answer_then_verify / plan — was inert for single-model users, which is almost everyone. Planning-as-a-phase is now decoupled from planning-as-a-separate-model: the working model is promoted to planner when nothing distinct is configured, in the centralized resolver rather than in any caller. Unchanged: configured distinct planners, explicit suppression, profile gates, admin locks. Also adds voice-aware planning — an interaction profile (standard | conversational), separate from response modality, raises direct-answer probability and shortens plans while a voice is active. Keep public copy non-technical — no planner/provenance/model-role internals, and do NOT mention the extended-context billing guard. -->
...
<!-- detail: 260810_widget-add-action (plan docs/plans/260810_widget-add-action/PLAN.md; CE2 heavy). Adds a third capsule to the iOS medium widget (Talk / Record / Add) deep-linking rebel://action/add into the existing add-action sheet, and converts the mobile inbox-add from a single-shot online ipcCall into a durable queue-backed path: persist-before-accept, retry with backoff, survives offline/background/app-kill, reached by every wake including the OS background wake. Asking the sync question first surfaced a LIVE pre-existing bug, fixed here as its own standalone shippable stage: addInboxItem was not idempotent by client id while the transport already retried the same payload up to 3x, so a lost response could duplicate an action AND overwrite the stored item with a pristine capture snapshot (reverting interim edits, archive state and space assignment), and could resurrect a deleted item. The guard is check-before-write first-write-wins on both store paths, plus read-repair for stores already corrupted through the live window. Version skew is closed technically rather than by process: the phone probes /api/health for inbox-add-idempotent-v1 before any mutating add and parks the capture attempt-neutral if absent, so an un-upgraded server is never sent a retryable mutation. Pending captures live in a mobile-local projection (never the shared store, whose cache validates all-or-nothing), filtered by the queue's boundCloudUrl and cleared on unpair so one account's capture titles cannot render in another account's list. Per the user's explicit ruling, captures reach the widget only via the normal act-tier route — no eligibility widening, tier classifier untouched. Keep public copy non-technical — no queue/capability/idempotency/projection internals. -->
...
<!-- detail: The Desktop | Cloud run-location control on automation cards (shipped in v0.4.46, commit 37aabc9f29) was hidden entirely unless Cloud continuity was already connected — so desktop-only users couldn't discover the option existed, and a colleague asking Rebel about it was told (wrongly) that no such toggle shipped. The control now renders for every eligible automation (still hidden for built-in system and event-triggered automations, which always run locally); without Cloud continuity the Cloud segment is aria-disabled with an explanatory tooltip ("Runs in the cloud, even when your laptop's closed — once you set up Cloud continuity (Settings → Cloud)") instead of the whole control disappearing. The RebelAutomations MCP server (resources/mcp/rebel-automations/server.cjs) now exposes executeIn (local|cloud) + timezone on create/update and reports "Runs on" in list output, capturing the desktop's IANA timezone when cloud is chosen (mirroring the panel) — previously the agent-facing tool surface had no run-location field at all, which is why Rebel itself couldn't see the feature. The bridge already accepted both fields (AutomationDefinitionPatchSchema); a pass-through contract test pins that. Cross-family review (GLM-5.2 + GPT-5.6-Sol) caught and this change also fixes: bridge (MCP) automation mutations now dual-write to the cloud exactly like the IPC path (previously an MCP-created cloud automation would have existed only on desktop — owned by the cloud, unknown to it, silently never running; deletes had the same gap); invalid IANA timezones are rejected at the MCP boundary and cleared at both store upserts (a bad zone would have produced NaN schedule times); a stale timezone is cleared whenever an automation isn't cloud-marked (the cloud→local switch previously left the old zone behind, firing runs at the wrong wall-clock) via a shared coercion helper used by the desktop scheduler and the cloud store; and MCP-created hourly automations now get the scheduler's type-aware catch-up default instead of always-on. help-for-humans automations.md gains a "Where automations run: Desktop or Cloud" section (incl. no-mobile-app-required and timezone behavior); cloud-continuity-and-mobile.md mentions 24/7 automations; the create/edit-automation skills document run location so Rebel knows it can make the switch for you. Scheduler fail-safes unchanged: system/event automations are forced local, and a cloud-marked automation with no cloud configured still runs locally. Keep public copy non-technical. -->
...
<!-- detail: 260806_single-attempt-answer-stacking (plan docs/plans/260806_single-attempt-answer-stacking/PLAN.md; CE2 bug_mode). When the model wrote its answer, took a small promised action mid-response (e.g. saving a note), then wrote the answer again, both full versions stacked in one message. Rebel now keeps only the last complete answer — matching what the system already treats as "the answer" — and moves the earlier draft into the collapsed work trace; single-answer turns are unchanged. A cross-family review caught that the first fix was a no-op on the live UI (the renderer store dropped the new field), driving the live-store and replay-batch follow-ups. Keep public copy non-technical. -->
...
<!-- detail: Sparked by a user report (internal Sentry ticket) where Rebel, unable to read a web page it wanted to quote, described its contents anyway. Two new ground rules in the agent's core instructions govern this: specific factual claims must be grounded in content Rebel actually retrieved, and a failed retrieval counts as knowing nothing about the source — the honest move is to say so, then try again, use another source, or mark the claim unverified. Page reading itself is more resilient too: transient network failures get one automatic retry, and when Rebel's usual DNS lookup can't answer (a failure class that broke page reading on some VPN and corporate networks), it falls back to the operating system's own lookup. Error pages (404s and the like) are now reported as unreadable instead of being passed off as content. Keep public copy non-technical — no DNS/retry/resolver internals. -->
...
<!-- detail: 260806_conversation-index-stale (plan docs/plans/260806_conversation-index-stale/PLAN.md; CE2 bug_mode; internal Sentry + Linear tickets). A crash (likely at the 0.4.58 auto-update boundary) killed a process between creating an empty sessions-locks/index.lock and writing its payload, leaving a lock the stale-lock recovery couldn't reclaim — recovery needs a parseable payload (it reads startedAt for age and pid for liveness) and is skipped entirely on an unreadable file, so every session write (checkpoints, upserts) timed out on LockAcquireTimeout while the conversation list kept reading a frozen-but-healthy index, silently freezing history for ~7 days. Fix: locks now publish atomically via a same-directory temp write + close + link() publish-if-absent (a crash mid-publish leaves no canonical lock; a late publisher can never overwrite a winner), and a discriminated lock read reclaims proven-malformed locks past a 5-minute mtime fence with identity revalidation, while unreadable I/O states (permissions, fd exhaustion) stay fail-closed rather than being wrongly stolen; lock timeouts now carry holder/state diagnostics and every performed malformed reclaim emits a structured warning + a process-deduped session_lock_malformed_reclaimed known condition so the fleet pattern is visible; releaseIfOwned warns when its own payload is malformed/unreadable at release. Existing poisoned installs self-heal on the first write after upgrade — no user action needed (the lock's mtime is days old, well past the fence). Cloud shares the same core lock code and gets the fix automatically. Arbitrator (gpt-5.6-sol) synthesized the hardlink publish-if-absent + malformed-only healing design after the Devil's-Advocate showed the original claim-ticket design rested on a false crash-window premise and named real hardlink production precedents. Keep public copy non-technical — no lock/hardlink/payload internals. -->
...
<!-- detail: 260806_staging-copy-sweep (plan docs/plans/260806_staging-copy-sweep/PLAN.md; CE2; residue of the 260806 source-capture recovery-card run, chief-designer copy consult). Fifteen staging sites shared one approval-card sentence, "This space requires approval for all saves.", which misattributed the cause on every mechanical lane (credential scanner errored, unreadable pending duplicate, safety system still loading, Privacy Mode, path outside every Space, same-session replacement) and was impossible in a Private space, where no such setting exists. The real cause is now recorded at the moment the save is held and rendered per-cause; the false blanket sentence is deleted; a row with nothing honest to say states what is happening rather than rendering nothing; three mechanical causes get a short "Why is Rebel asking?" body answering the user's actual question (is my note OK, do I need to do anything?); and two cards that claimed "Possible credentials detected" where nothing had been scanned are corrected. Separately, a captured note stranded because the model wrote the source system's name where its kind belonged is repaired at the moment the user saves it, but only when the note itself corroborates the fix — anything ambiguous is still refused rather than guessed. Keep public copy non-technical — no frontmatter / staging / structural_policy / precedence / enum internals; say "held it for you to look at", "the real reason", "kept your copy". -->
...
<!-- detail: same run (260806_staging-copy-sweep, Stage 4 — publish-time source_type repair, plan R4 remainder). Repair happens only at user-authorised publish of a stranded recovery copy, only when the note's own source_system corroborates the wrong value, and only for a small SSOT-derived table; a contradicting or absent source_system, or an unlisted system, is declined by construction and the existing fail-closed refusal stands. The old value is recorded in the note. Keep public copy non-technical. -->
...
<!-- detail: 260807_read-aloud-uncontrolled-audio (plan docs/plans/260807_read-aloud-uncontrolled-audio/PLAN.md; CE2 bug_mode). Rebel's read-aloud feature was uncontrolled: a bundled helper script played audio through a shell command (afplay/say) that Rebel couldn't pause or stop, so once it started you were committed. The fix introduces a built-in `rebel_read_aloud` tool that plays through Rebel's own controllable audio player with a visible pause/stop bar (SpeechControlBar), a deterministic audioPlaybackGuard that blocks shell audio playback attempts (afplay, say, paplay, mpg123, aplay, ffplay) at PreToolUse with a redirect message naming rebel_read_aloud when the capability is available, and a defense-in-depth Bash pre-spawn backstop in builtinTools.ts. The bundled ElevenLabs helper script no longer plays audio — it generates files only. Behavior change: the ElevenLabs voice branch no longer silently swaps a manually-set short/invalid ttsVoice to Rachel — it now sends the configured value as-is, so a misconfigured voice (default unset/null still falls back to Rachel) returns a provider 400 instead of silently substituting, and Voice replies for that narrow misconfigured case will surface an error rather than play Rachel. Safety-prompt and tool-safety-classifier eval fixtures added for read-aloud behavior. Desktop renderer only. Keep public copy non-technical — no guard/hook/backstop/eval internals; say "plays through Rebel", "pause/stop bar", "blocked". -->
...
<!-- detail: The connector improvement wave landed entirely in the public mindstone/mcp-servers repo (github.com/mindstone/mcp-servers). Released and pinned in the app: google-workspace@0.3.0 (contacts create/update, Gmail vacation responder + send-as read, auth-reconnect signal fix) and microsoft-mail@0.3.0 (get/set_automatic_replies out-of-office, BCC on send/draft/compose, Zod-validated draft creation). Also in the public repo but not yet pinned into the app: microsoft-teams search/reply/presence, microsoft-calendar find_meeting_times + RSVP status, microsoft-sharepoint page authoring + uploads, microsoft-files docx/pptx read + binary upload, google-analytics audience exports + async reports. -->
...
<!-- detail: 260730_org-openrouter-provider-switch + 260731_org-openrouter-proxy-start-hardening (two CE2 runs). An admin-assigned organisational OpenRouter key was stored but never became a routing candidate — planner and behind-the-scenes kept running on the previous provider while the UI asked for OpenRouter. The org claim now visibly switches the provider (toast + "in use through your organisation" attribution), admin-pushed default models apply once servable, and proxy startup is keyed off the actual route decision so a proxy-required turn can't silently proceed without its proxy. -->
...
<!-- detail: 260806_conversation-rename-titles (plan docs/plans/260806_conversation-rename-titles/PLAN.md; CE2). Three related conversation-title work items from one user report. (1) Rename-via-context-menu opened the inline edit box and immediately cancelled it: the sidebar row's mouseleave handler blurred whatever was focused inside the row, and once the right-click menu (a floating-ui portal) unmounted and the rename input mounted+focused in the same commit, the pointer was left hovering over the row — the next pointer movement off the row fired mouseleave → blur → commit-with-no-change → silent close. Fixed by guarding the blur so it never fires on a rename input (and more broadly on any input/textarea/contenteditable, so the same handler is safe for any future inline editor), and by making focus resilient via a callback ref so the input re-focuses after the plain↔virtualised list switch at 50 items unmounts all row DOM (the old useEffect([editingSessionId]) did not re-fire on remount); a deferred-blur commit is now guarded by an edit-generation token (a ref counter incremented on every startRename/cancelRename/commitRename) so a stale deferred commit can never clobber a newer edit session. (2) Double-click on a main sidebar list row now opens rename, scoped to the row's title button so star/pin/cloud/more action siblings don't trigger it, skipped on corrupted/trash/no-content rows and when a rename is already in progress; search-result rows keep context-menu rename only (their first click clears search and unmounts the row before a second click could land, and deep-search rows have no inline rename input). (3) Auto-generated titles are now up to ~6 words (TITLE_MAX_WORDS 3→6; TITLE_MAX_CHARS=48 unchanged with word-boundary truncation so a too-long title cuts at the last full word that fits rather than mid-word), the system prompt (rebel-system/prompts/conversation/title.md) and the inline user prompt are rewritten to lead with the most informative noun/proper name and include the distinguishing qualifier ("Q3 budget review" not "Budget review"), and a single shared TITLE_MAX_TOKENS=32 now drives both production generation and the eval harness (was 24 vs 64), killing the production-vs-eval token-budget drift class. Eval word-count gates updated 2–3 → 2–6 and the title eval re-run for the new baseline. Desktop renderer only; mobile/web sidebar has its own component tree. Keep public copy non-technical — no mouseleave/blur/callback-ref/virtualised/token-budget internals; say "rename box", "double-click", "a few words longer". -->
...
<!-- detail: 260806_use-tool-args-retry-loop (plan docs/plans/260806_use-tool-args-retry-loop/PLAN.md; CE2 bug_mode; internal Sentry + Linear tickets). A user running the feedback skill hit a retry loop: the agent called a no-argument tool but tucked one of the router's own call-shaping parameters (max_output_chars) inside the tool's arguments. The router correctly rejected the unknown field, but the error only said the field was unknown — it never said the parameter was legitimate and merely in the wrong place, so the model re-sent the same broken shape every time. After three tries the terminal message told the user to supply specifics they could not possibly supply, for a mistake the agent could fix itself. Three misplaced parameters (max_output_chars, output_offset, schema_hash) are now rejected at the envelope layer on the first attempt, before any schema lookup, with an error naming the misplacement and the exact corrected call shape; the two softer cases (dry_run, result_id) are named by the repair ticket instead. The 'ask the user for specifics' terminal message is replaced with accurate guidance, and the inbox classifier was updated in the same change so the exhausted-retries state stays reachable. Keep public copy non-technical — no envelope-layer / meta-param / repair-ticket / classifier internals. FOLLOW-UP (260807_arg-validation-residue, plan docs/plans/260807_arg-validation-residue/PLAN.md; CE2): the two soft cases (dry_run, result_id) were only taught when the tool's schema would have stripped the unknown field; on a permissive schema a nested dry_run validated clean and the call DISPATCHED — a model asking to preview got the real, unreverted action, with only a log warning. The gate now keys on whether the tool declares the property — a literal declaration passes through, a canonical-form twin (declared dryRun, nested dry_run) is renamed to the declared spelling before dispatch so the tool actually honors it, and an undeclared one throws with the corrected call shape instead of dispatching — and the renderer classifies these failures on the -33003 code token rather than prose wording. Plus reliability plumbing: contradictory advice suffix suppressed, collision warning once-per-tool, dead duplicate predicate deleted. Keep the added sentence non-technical — no schema/declared-property/error-code internals; say "test", "dry run", "held back". -->
...
<!-- detail: 260806_rebel-7gv-nonvision-image (plan docs/plans/260806_rebel-7gv-nonvision-image/PLAN.md; CE2 bug_mode; internal Sentry + Linear tickets, urgency critical). A user on a managed non-vision working model (z-ai/glm-5.2) attached an image and asked Rebel to review it; the agent replied "this model doesn't read images" and the user was stuck. The wire gate was correct (translators substitute a text placeholder before the API call when modelSupportsImageInput(model) is false, so the provider never 404s), but the actionable image_input_unsupported banner was unreachable because no provider rejection ever fired, and there was no attach-time affordance — the user hit a silent dead end with no path forward. Fix: a shared pure resolver (src/shared/utils/imageCapabilityRecovery.ts) answers "can this conversation read an attached image, and if not, what named option can?" from the same selectable set the model picker uses (managed allow-list tri-state passed directly; admin-locked and route-unproven candidates fail closed; uncatalogued current model inherits modelSupportsImageInput's fail-open). The composer (and in-conversation question replies) render a compact Notice below the attachment thumbnails when the current setup can't read an attached image, intercept Send before it can dead-end (preserving draft + attachments + thinking effort), and offer a one-tap per-conversation switch to an in-allow-list image-reading option with a true Undo that restores the exact prior override tuple (persisted both directions, invalidated by any newer selector write). Conversations that already dead-ended get a deterministic post-refusal CTA after the assistant response, detected structurally from the turn's persisted observed working model + image attachment (never refusal-prose matching), offering the same switch and asking the user to attach the image again (persisted history can't reconstruct the original — no fake resend). Honest no-candidate state for accounts with no image-reading option (org-policy vs none-available vs allow-list-unknown vs route-unconfirmed). Automatic per-turn routing of image-bearing turns (direction #22a) deliberately deferred — cost/billing/consent implications; tracked in MODEL_AND_PROVIDER_DIRECTION #15/#22. Desktop renderer only this run; mobile/web composers keep the un-fixed behaviour (the resolver is in src/shared so adoption is a UI-wiring job). Keep public copy non-technical — no "vision-capable model"/"wire gate"/"translator"/"allow-list"/"override tuple" internals; match the in-product "Rebel can't read this image with the current setup" copy family. -->
...
<!-- detail: 260806_openai-rate-limit (plan docs/plans/260806_openai-rate-limit/PLAN.md; CE2 bug_mode; internal Sentry + Linear tickets). When an AI provider asked Rebel to slow down (a heavy day on one account), Rebel would say only "try again shortly", keep knocking every second anyway, and — if you'd connected a second provider like Anthropic — leave you to dig into Settings and switch by hand. Worse, the "shortly" was a guess: Rebel ignored the actual wait time the provider sent. The fix reads the real time when the provider gives one (and falls back to its "resets at" time otherwise), backs off for short windows instead of hammering (and for longer ones beyond a sensible cap, surfaces the true time and the switch instead of waiting), shows the true time in the message ("try again in about 2 minutes"), and — when you have another provider connected — offers a one-tap switch right there in the message. A multi-hour wait never holds a turn hostage: beyond a sensible cap, Rebel tells you the real time and offers the switch instead of waiting silently. No automatic failover: the switch is an offer, not a forced change, and the broader question of when Rebel should switch on its own stays parked. Keep public copy non-technical — no Retry-After/429/header/backoff internals. -->
...
<!-- detail: 260805_fix-plan-card-flicker (plan docs/plans/260805_fix-plan-card-flicker/PLAN.md; CE2 bug_mode). Active-mode task filtering emptied the live plan list when all tasks were momentarily completed mid-turn, and both plan-card hosts (phantom row + message card) were gated on projected liveness, which flaps false mid-turn — so the plan could shrink to one activity line or vanish entirely, then reappear. The fix: one shared full-snapshot derivation for both hosts (completed steps stay checked while the turn runs; badge/collapsed counts truthful), card visibility/expansion keyed to the authoritative runtime turn identity (attempt-aware — a superseded attempt's result can't collapse the card), silent-stop classification yields to that protection, the phantom host is preserved across projected-idle blips (incl. the primary-MCP-App host-capability edge), and protection releases at the canonical 5-minute staleness boundary so a dead turn can't pose as live. Mobile parity: full checked history with the live headline derived from active (never completed) tasks; all-checked running plans keep live (not Done) treatment. Production plan-visibility telemetry (hashed ids, no content) makes the class diagnosable. Keep public copy non-technical — no liveness/host/telemetry internals. -->
...
<!-- detail: 260806_automation-quarantine-notification (plan docs/plans/260806_automation-quarantine-notification/PLAN.md; CE2; internal Sentry + Linear tickets). Follow-up to the v0.4.59 weekly-automation-fires-daily fix below: automations paused into the quarantine surface were only discoverable by opening the Automations panel, showed a developer-vocabulary reason string, and offered only delete-and-recreate recovery — so a non-technical user whose weekly/monthly automation was paused for an empty-days schedule had no way to learn it had paused, and had to rebuild the automation from memory to fix it. The panel section is rewritten in plain language ('Needs your attention'); the cause is derived at read time by a pure classifier over the retained definition (substitute a sentinel day and re-run full schedule validation — if it then validates, the only defect is empty days → empty-days-weekly/monthly; anything else → generic; never inferred from message strings); and empty-days rows offer a narrow 'Choose days' in-place repair that upserts the full retained definition with only the schedule corrected (generic-corrupt rows keep Delete-only). A race-safe toast (fetch-then-subscribe; dedupe key recorded only after showToast returns a real id) fires once per (automationId, quarantinedAt) — a summary toast when several pause in one load — and a single aggregate dismissible Today card on the homepage persists until repaired/deleted/dismissed. Toast and card copy derive from one shared helper so the three surfaces never drift. Desktop-only this run; cloud/mobile have no toast/Today-card equivalent. ~10 affected automations install-wide. Keep public copy non-technical — no classifier/quarantine-entry/dedupe-key/Today-card internals; say 'paused', 'notification', 'homepage reminder', 'pick a day'. -->
...
<!-- detail: 260806_source-capture-recovery-card (plan docs/plans/260806_source-capture-recovery-card/PLAN.md; CE2 bug_mode). The Source Capture automation stages a no-loss "recovery copy" whenever a captured note fails its formatting check (e.g. the note's source_type field held the source system name instead of its kind — "slack" instead of "thread"). That recovery copy is transient and self-resolving: the model gets one rewrite, and on success the copy is deleted. But it rendered as an ordinary approval card with the generic one-liner "This space requires approval for all saves" — wrong attribution (nothing about the space requires approval) and confusing, because it asked for action that wasn't needed and disappeared on its own ~4 minutes later when the retry landed. Two root causes, both fixed. Producer: the closed list of allowed source_type values lived one file-read away from the prompt that actually fires, and the most confusable wrong value (the source system name) is the value the model had just written into the adjacent valid field — so the formatting check was the first and only pushback. The list is now pinned at three points of use (the prompt, the one-rewrite feedback, and a new automated check), all driven by one source of truth, and the feedback tells the model the allowed values plus which field the system name actually belongs in. Consumer: the recovery copy was an ordinary pending file with no structured marker, so it inherited the wrong copy, an unsafe publish path (publishing bytes the formatting check had already denied), and could be swept by bulk gestures. Host self-healing state now carries a structured marker plus a durable lifecycle — invisible while Rebel retries (recorded at the moment of staging so the retry window itself is covered), revealed promptly as a plain filing task once self-healing stops (retry failure, turn end, or after a bounded age on restart — whichever comes first), never as an approval, never bulk-sweepable, with honest copy. A single shared liveness check drives both the list filter and every action, so they can't disagree; the host's own cleanup is never blocked while every user-facing action is guarded; and publishing a note that still fails its formatting check is refused (the note is kept private or discarded instead — nothing lost). Keep public copy non-technical — no frontmatter / recovery-copy / staging / structural_policy / eval-gate internals; say "couldn't get its details right", "couldn't file it", "kept your copy". -->
...
<!-- detail: 260804_weekly-automation-fires-daily (plan docs/plans/260804_weekly-automation-fires-daily/PLAN.md; CE2 bug_mode; internal Sentry + Linear tickets). The weekly and monthly cadences had the same defect class as the April every_n_days bug: AutomationScheduleSchema accepted empty daysOfWeek/daysOfMonth arrays, and calculateNextRunAt/calculateMostRecentScheduledTime fell back to today's weekday/day-of-month when the array was empty — so the launch/wake catch-up check (which re-derives "today" on every evaluation) admitted a run on every app launch, turning a weekly automation into a daily one. The schema now requires .min(1) on both arrays, the scheduling math fails closed (returns null) for empty arrays, and existing broken definitions are routed to the existing quarantine surface in the Automations panel (delete-and-recreate recovery) instead of silently guessing user-visible weekday intent — a weekday is explicit user intent, not a hidden anchor. Cloud stores get the same repair via a cloud-local STORE_VERSION bump (2→3) so the version-gated eager migration revisits existing definitions; desktop eager migration already re-persists on load. Monthly had the identical latent bug and is fixed in the same change. ~10 affected automations install-wide will pause and surface in the Automations panel after the update; recovery is delete-and-recreate (or support-assisted same-ID restore). Keep public copy non-technical — no schema/catch-up/quarantine/store-version internals; say "pause and show in the Automations panel", "recreate". -->
...
<!-- detail: 260805_midstream-fallback-recovery (CE2 bug_mode). When a provider overloaded after a task had already started, Rebel either retried the same unhealthy route or ended the whole turn; partial text could then compete with the eventual result and the same failure could appear twice. Rebel now cools the exact failed route, hands only that step to a configured fallback model inside the existing turn, retracts the failed model's unfinished fragment without replaying completed tools, and keeps one canonical result. If every fallback is exhausted, the saved partial work and structured cause produce honest terminal copy. Keep public copy non-technical — no lease/lifecycle/CAS/cooldown-key/usage-ledger internals. -->
...
<!-- detail: 260803_rebel-7f9-swifteq-oauth-callback (plan docs/plans/260803_rebel-7f9-swifteq-oauth-callback/PLAN.md; CE2 bug_mode; internal Sentry + Linear tickets). One connector's sign-in service (Swifteq, fronted by Auth0) accepts returns only to two hard-coded local addresses (both on port 8080), and its registration endpoint is a facade that echoes Rebel's registered return address without saving it — so every sign-in from Rebel's usual ports (5173–5182) died on the provider's own page with "Callback URL mismatch" and surfaced minutes later as a generic "Request timed out". Rebel now headlessly checks the provider's verdict on the sign-in address before opening the sign-in window (ambiguous answers fail open to the old behavior; env kill switch), and on a classified rejection retries on the port the provider actually accepts — 8080 joins the candidate list at position 2 on a fresh sign-in and first on a retry, so a saved broken registration self-corrects on the next authenticate, with full per-attempt isolation (fresh registration, callback listener, and state each time; ≤3 attempts). If no address is accepted, Rebel still opens the sign-in window as before (never worse than today), and the message now says plainly that the provider's sign-in page rejected the connection — a fix on their side, not the user's. Also shipped: stricter staleness comparison on saved registrations, ambient connections adopting the saved port, a diagnostics envelope (chosen port, registered vs requested return addresses, probe verdicts) riding error reports so the next report of this class self-diagnoses, and the desktop authenticate budget raised 500s→620s. Vendor ticket text prepared (correct permanent fix is theirs: Native-app registration per RFC 8252, or real per-client registration persistence). Keep public copy non-technical — no OAuth/redirect_uri/DCR/Auth0/probe internals. -->
...
<!-- detail: same run (260803_rebel-7f9-swifteq-oauth-callback, Stage 4 refinement + safeServerName follow-up). Connector/server ids can carry account-derived slugs (e.g. GoogleWorkspace-<name>-<domain>); bug-report attachments now pass the id only through the slug-stripping display-name transform, and Sentry/PostHog egress strips unknown compound ids outright. Non-PII diagnostic fields (timeout flag, callback port) are newly allowlisted so reports stay diagnosable. Keep public copy non-technical. -->
...
<!-- detail: 260802_credential-write-rereview-loop (plan docs/plans/260802_credential-write-rereview-loop/PLAN.md; CE2 bug_mode; internal Sentry + Linear tickets). A user building a custom MCP server saw repeated "save to MCP Servers" approval prompts for the same write — the agent itself reported the write being queued again after approval. Two publication contracts had coexisted without alignment: staged (approve = publish the captured bytes, no retry) and blocking (approve = mint a token and ask the agent to replay). Credential redaction makes the blocking contract unsound by construction — the approved content is deliberately withheld from the continuation, so the agent regenerates, the regenerated bytes fail the byte-exact digest, the token is not consumed, and the preflight re-detects and re-blocks. The fix makes every desktop staged approval honour the staged contract (the main process fails closed for staged records before any side effect, so a staged approval can only resolve through the staged-publish channel; the publisher re-checks the persisted approval's exact operation identity under the destination lock), and moves the ~/mcp-servers destination class onto staging with a fixed, fail-closed validation context (symlink-aware containment, sensitive-root carve-out, protected-path checks, revalidation under lock). Rejected: mint-on-publish, generic publish-on-approval for the blocking flow, and loosening the byte-exact digest. Keep public copy non-technical — no digest/token/staged/blocking/lane internals; say "approved write" and "save to MCP Servers". -->
...
<!-- detail: 260802_nudge-aside-ui (CE2 bug_mode). A Nudge used to appear twice in the transcript: once as an ordinary user bubble and again inside the run, while acknowledgements depended on a synthetic task carrier that could fail during a mid-turn provider switch. Nudges now use a request-local acknowledgement channel, transactional provider handoff, accepted-attempt-only replies, and one shared run-attached aside across desktop, current mobile caches, and authenticated web. Incomplete legacy/read-only histories stay visible as "Earlier nudge" bubbles rather than claiming delivery. Keep public copy about transcript clarity and truthful status; no protocol/lease/journal internals. -->
...
<!-- detail: 260801_invoke-layer-liveness (plan docs/plans/260801_invoke-layer-liveness/PLAN.md; CE2, structural prevention for the renderer_unbounded_ipc_await class named in the 260729 bug-report send-hang postmortem, rec #3, and confirmed live a second time by 260728_settings_load_silent_failure_blank). Every request the app's screen makes of its own engine room now carries a deadline policy: all of them are observed and reported if they stay pending past a finite budget (120s default, 300s for known-slow work, 30 minutes for anything legitimately waiting on the user — a sign-in window or an open file picker — which is exempt from being cut off but never exempt from being watched), and three pure-read diagnostics/health lookups now actually give up at 5 minutes with an honest "couldn't confirm, try again" state rather than spinning. Remaining channels stay observe-only and migrate per-family as their call sites are verified. Keep public copy non-technical — no IPC/invoke/preload/renderer/deadline/telemetry internals; the user-visible surface is genuinely small (an honest unconfirmed state on a few diagnostics reads, and no more invisible infinite spinners). -->
...
<!-- detail: 260731_cloud-outbox-stuck-watchdog (plan docs/plans/260731_cloud-outbox-stuck-watchdog/PLAN.md; CE2 bug_mode; internal Sentry ticket, 7.4k events / 35 users over 3 months). Three different conditions were being reported under one "sync appears stuck" alarm. (1) The alarm fired whenever anything was queued rather than when work was actually DUE, and its clock was never refreshed on cycles that legitimately did nothing — so backoff waits, a deliberately-held-off connection, a sleeping laptop, and a fresh restart each reported a stall hourly. Pure false positive, and the dominant volume. (2) Behind that noise sat a real wedge: a chronically rejected item retried forever, and because the connection verdict was collective, one failure among many successes counted as a connection failure every time — so the circuit stayed open and starved delivery of everything else (one machine: 122 items undelivered for 4.16 days). Such items now park REVERSIBLY — immediately only for producer-audited structured rejections, otherwise after a 7-day AND 50-identical-failure budget — with manual Retry plus app-version, cloud-version/capability, and credential wakes. Nothing is deleted and local data is untouched. (3) Repeated credential rejections now park until credentials are demonstrably repaired, ending a 15-minute/30-minute steady-state loop. Because parking removes work from "pending", the sync status gained paused-replication and needs-reauth counts with Reconnect/Retry, so parked work can never render as synced. 10 commits. Keep public copy non-technical — no outbox/watchdog/circuit-breaker/backoff/parking internals; say "sync", "set aside", "paused". -->
...
<!-- detail: 260731_offline-misdetection (plan docs/plans/260731_offline-misdetection/PLAN.md; CE2 bug_mode; internal ticket, reported by a team member whose internet was demonstrably fine at the time). Three independent connectivity detectors all read "we got no answer" as "nothing out there is reachable". When the app's main thread stalled, the detectors' own response budgets were starved, so probes that failed fast on a managed network became indistinguishable from a machine with no internet — producing the "You appear to be offline" message, an all-providers-unreachable support verdict, and a tripped cloud circuit that quietly parked background delivery for minutes. Now: stall-corrupted probes read as inconclusive and fail open; a TLS certificate rejection (a work network inspecting traffic with its own certificate) is recorded as its own diagnosable condition rather than a false offline, because completing a TLS handshake proves the network is up; the support-facing verdict only fires on definite evidence; and the cloud circuit has a 60-second recovery hold-down so it stops flapping, with a latch so a sustained re-outage still escalates the moment the hold-down expires. Genuine offline is unchanged — real offline users still get the honest message, fast. Does NOT fix: genuinely going offline during a stall (indistinguishable by design; fails open), or partial unreachability where the user's own provider is down but a canonical endpoint is up. 7 commits. Keep public copy non-technical — no probe/budget/stall/circuit-breaker internals. -->
...
<!-- detail: 260730_rebel-52b-no-route-gemini (plan docs/plans/260730_rebel-52b-no-route-gemini/PLAN.md; CE2 bug_mode; internal Sentry tickets; internal Linear tracking). When Rebel switched models mid-turn to a Google (BYOK) model, the local model router's resolveRouteProfile could lose track of which account/profile was supposed to serve the request and hard-failed the turn with the generic "The AI service ran into trouble. Your message is safe — try again." The router now binds each request to the account that authorized it (the route-facts carrier is consumed after the Anthropic-passthrough step, before the base-profile fallback, so a mid-turn switch to a Google profile routes to the right account instead of falling through to a missing route). When a route is genuinely missing, the error is now classified as routing/route-miss (new classifyByStructuredProviderError arm for code:'route_miss' in src/core/rebelCore/modelErrors.ts) and surfaces honest, specific copy — "Rebel couldn't reach the model this request was routed to. Try again — if it happens again, pick another model in Settings." — instead of the generic AI-service message, and retrying actually makes sense. The failure is now reported with enough detail to diagnose remotely. Keep public copy non-technical — no router/profile/route-facts internals; say "model router"/"account" not "proxy"/"profile". -->
...
<!-- detail: 260729_bug-report-send-hang (plan docs/plans/260729_bug-report-send-hang/PLAN.md; CE2 bug_mode). The Feedback & Bugs dialog's submit awaited the bug-report IPC invoke with no renderer-side deadline; isSubmitting cleared only in finally and the dialog closed only on outcome:'accepted', so any unsettled invoke (a main-thread stall defeats every setTimeout-based main-side bound, since the timers run on the stalled loop) left the user on an infinite "Sending…" with no error, no timeout, and no recovery — a colleague's 2026-07-19 report was durably saved and auto-delivered next morning, but the UX gave no sign of it. Stage 1 wraps submitBug in the shared withRendererTimeout at 25s with a typed stalled sentinel; on timeout the dialog shows a warning (not an error), the text is preserved, Send re-enables, and the dialog stays open — the durability contract is unchanged (main still returns accepted only after the fsync'd write), so a timed-out submit is "unconfirmed", not "failed", and the existing queued/delivered status broadcasts reconcile the state when the late write lands. A monotonic submitAttemptRef (modelled on screenshotSessionRef) fixes the shared isSubmitting/resetForm leak that wedged the dialog on reopen after an X/Escape mid-submit. Retry-during-in-flight returns an additive optional reason:'in_progress' on the existing failed branch and renders calm copy as a warning instead of the raw 'Bug report already in progress' developer string. Late-settling submits emit one structured console.info (bug-report-submit-late-success / -late-error, no report content) — the first observability this class has had. Stage 2 (main, defense-in-depth for the disk-wedge-with-healthy-loop case) bounds fsp.mkdir and fsp.stat inside their existing timeout budgets and mints identifiers before acquiring the in-flight guard, closing a 90s guard-leak window. Stage 3 applies the same deadline pattern to ConversationFeedbackDialog (45s, unconfirmed-not-failed), consolidates BugReportStatusPayload into one shared discriminated type in src/shared/bugReportStatus.ts (was hand-synced across 3 places), and makes the unknown-status silent drop warn once per session. bug-report:submit-bug is deliberately desktop-only; cloud/mobile POST /api/feedback is untouched. Keep public copy non-technical — no IPC/invoke/renderer/deadline. -->
...
<!-- detail: 260729_dedupe-file-access-grants (plan docs/plans/260729_dedupe-file-access-grants/PLAN.md; CE2 bug_mode). Settings → Privacy & Safety → "Folders Rebel can read" listed the same folder multiple times because repeated approvals of the same folder each persisted a fresh grant with no equivalence check, so identical grants differing only in id/createdAt accumulated. Worse, clicking Remove on a duplicated folder dropped only the clicked row while a hidden twin kept the folder authorised — the UI optimistically said "removed" but access silently continued (first-allow-match-wins at check time). The grant store now dedupes by equivalence at write time and collapses duplicates in the read view, and Remove sweeps every equivalent row so revocation actually de-authorises. No schema or store-version change; existing on-disk duplicates become invisible immediately and are pruned the first time the folder is revoked. Keep public copy non-technical — no grant/store/equivalence-key internals. -->
...
<!-- detail: 260729_citation-sanitizer (plan docs/plans/260729_citation-sanitizer/PLAN.md; CE2 bug_mode; the "for another day" glitchy-Unicode issue the v0.4.58 attempt-supersession entry above teased). Raw grounding-citation control markup — Unicode private-use-area delimiters wrapping `cite...turn<N>search<M>` tokens, emitted by some search connectors and occasionally reproduced by the model from training data — flowed unsanitized into visible assistant text and rendered as glitchy Unicode boxes and stray fragments like `citeurn0search0`. Rebel now strips that exact grammar at the shared event-sanitation boundary for complete answers (with defense-in-depth at the tool-result funnel so the model stops seeing and copying the markers), so it never reaches display, persistence, the collapsed work trace, exports, or bug reports; split streaming markers are filtered centrally before CLI/cloud fan-out; and older conversations already saved with the markers are cleaned at read time — their stored bytes stay untouched (no migration, rollback-safe). Count-only telemetry reports unique removals and any malformed survivors without logging conversation content. 5 commits, cd17ed3582..b6b0733a68. Keep public copy non-technical — no PUA/delimiter/funnel/manifest/sanitization-strategy internals. -->
...
<!-- detail: 260728_slow-response-bug-sweep Stage 2 (plan docs/plans/260728_slow-response-bug-sweep/PLAN.md; CE2 bug_mode). When an AI reply paused mid-stream after it had already started, the "still waiting" notice used to disappear the moment the first token arrived, leaving a dead spinner for up to 10 minutes until the turn was aborted. A streaming-phase soft-stall predicate now re-surfaces the notice when the stream goes quiet past 30s and clears it when tokens resume; the awaiting-api path is unchanged. Keep public copy non-technical. -->
...
<!-- detail: 260728_slow-response-bug-sweep Stage 3 (plan docs/plans/260728_slow-response-bug-sweep/PLAN.md; CE2 bug_mode). The 120s pre-dispatch liveness guard could kill a turn that was legitimately waiting on a rate-limit cooldown (≤300s, persisted across restarts), and the terminal copy hardcoded "an unresponsive cloud-storage folder" as the cause. The guard now pauses its active-setup budget through declared cooldown waits and resumes the remaining budget after, instead of killing; the terminal copy no longer names an unverified cause. Keep public copy non-technical. -->
...
<!-- detail: 260728_slow-response-bug-sweep Stage 5 (plan docs/plans/260728_slow-response-bug-sweep/PLAN.md; CE2 bug_mode). The promptFilesRender health check false-warned "unreferenced variable" on every install because its case-sensitive body.includes test didn't recognise the single-brace {VAR} substitution convention the prompt producers use, so health_status read "degraded" fleet-wide. The check now recognises every producer convention while still warning on a genuinely absent variable. Keep public copy non-technical. -->
...
<!-- detail: 260729_rebel-6cs-route-tag Stage 1 (plan docs/plans/260729_rebel-6cs-route-tag/PLAN.md; CE2 bug_mode; Sentry (internal ticket), 26,902 events/29 users since 2026-06-23). An internal watchdog that flags "you asked for model X but model Y actually served" compared model names letter-for-letter, so the same model written two ways (claude-haiku-4.5 vs claude-haiku-4-5 — dotted vs dashed version) tripped it constantly. The comparator is now spelling-aware (case, dot/dash version segments, provider prefix, trailing date snapshots, [1m] markers) and still fires on a genuinely different model or a different billing provider. Telemetry now records transport, credential source, and consumer class so future anomalies can be characterised. Keep public copy non-technical — no route-tag/gate/comparator internals. -->
...
<!-- detail: 260729_rebel-6cs-route-tag Stage 2 (same plan; CE2 bug_mode). The abort-estimate cost row in agentQueryRunner.ts now resolves to the serving/routed model when route attribution is available (reusing resolveReportedModelForRequest semantics) and prices from that model, and never writes a row under the literal "unknown" sentinel. NOTE: this branch is currently unreachable in production — the zero-cost-abort producer gap is tracked as an internal ticket (adapter never emits assistant-level usage; estimate branch dead since the Rebel Core cutover) — so this is latent hardening, not a live user-visible change. Phrased honestly per run instructions. Keep public copy non-technical. -->
...
<!-- detail: 260729_zero-cost-abort-usage (plan docs/plans/260729_zero-cost-abort-usage/PLAN.md; CE2 bug_mode; internal ticket). Aborted turns booked zero cost because the per-round usage accumulator in agentQueryRunner.ts read assistant-message usage that the Rebel Core adapter never emits — the accumulator died silently at the Rebel Core cutover (April 2026) and nothing noticed because cancelled turns are easy to forget. Revived the producer: the adapter now fills AgentAssistantMessage.usage per-round via the existing toAgentUsage helper, and a contract test pins the loop→adapter flow that would have caught the cutover death. Moved the abort-estimate write out of the per-attempt finally block and into completeTurnCleanup — one write after all retries, gated on !hasCostRecorded — with per-turn usage accumulating across attempts in agentTurnRegistry, so an abort→retry→complete turn books exactly one ledger row (the completed attempt's exact row suppresses the deferred estimate), never both. Completed-iteration usage is exact; the in-flight iteration at abort contributes zero (no provider evidence at any layer for an interrupted first response), so the estimate stays est:true and covers only completed iterations plus known input. Per-model attribution uses the serving-model machinery already shipped (modelUsage keys / executed-route attribution), never the literal "unknown" sentinel. Keep public copy non-technical — no adapter/accumulator/registry/est-flag internals. -->
...
<!-- detail: 260801_session-scoped-tool-approvals (plan docs/plans/260801_session-scoped-tool-approvals/PLAN.md; CE2 heavy). Eligible tool approval cards can now grant either the raw tool or all eligible tools in its connector for the current conversation. Enforcement is deterministic (no LLM match), in-memory per executor, and bounded by the trust-eligibility ladder plus credential-Bash, Actions rule-write, and durable-block precedence. Active grants appear in the conversation strip and can be revoked there; user negation also revokes; teardown/restart clears. Tool scope is argument-independent by design and Stage 6 pins zero follow-up cards even when recipients/arguments change. Keep visible copy explicit about that breadth and expiry; do not describe Settings management or persistent new permissions. -->
...
<!-- detail: 260804_session-write-grants (plan docs/plans/260804_session-write-grants/PLAN.md; CE2 feature, medium). Write approval cards gain a visible secondary "Allow writes here" action that mints a session-scoped, destination-subtree write grant (routine authority only — destination authorisation, never credential or sensitivity review). A separate explicit elevated action on credential cards at server-derived ~/mcp-servers/<project> roots attaches the credential capability (informed consent for future credential-shaped writes there); a deterministic credential scan runs before EVERY grant-based allow, fail-closed, even where the universal preflight is skipped. The credential lane publishes the exact staged bytes (never a tool re-execution). Conversation permissions strip (desktop) lists active grants with routine/elevated distinction and atomic revoke; every credential auto-publication emits a collapsible user-visible audit event. Desktop-only v1 (cloud/mobile get one-time approvals only); grants are in-memory, keyed to the conversation, cleared on conversation end or app restart; Bash, delete, durable, out-of-zone, team/shared/public and explicitly-cautious destinations excluded. Keep public copy non-technical — no grant/capability/seam/staging internals. -->
...
<!-- detail: 260729_auto-reopen-done-conversation (plan docs/plans/260729_auto-reopen-done-conversation/PLAN.md; CE2). A conversation marked Done stayed filed under Done even while the user kept sending messages into it — the sidebar showed "finished" for a conversation the user was actively using, and the only way back to Active was a manual reopen. Any new user-authored turn (composer send, voice, queue drain, edit-and-resubmit, retry-after-error) now clears doneAt → null before the turn dispatches, on every surface: desktop via the renderer reopenSessionIfDone hook at initiateAgentTurn and rerunEditedMessage, cloud/mobile via an atomic read-modify-write in submitAgentTurnInternal. Inbound external messages into an existing conversation (e.g. Slack) inherit the desktop hook; meeting companions reopen (they are NOT background); system continuations (tool/memory/automation approval retries, compaction), excluded-from-Active session kinds (automation, meeting-analysis, use-case-discovery), scheduled automation runs, and the legacy inbound-polling path (only ever creates new Active sessions) are deliberately excluded. Single decision rule: shouldReopenOnNewTurn() in packages/shared/src/sessionLifecycle.ts (import-free, consumed by both surfaces). Silent by design (no toast/banner — the sidebar regroup is the entire signal, mirroring the star-reopen precedent); the post-turn auto-done safety evaluator remains authoritative (a reopened conversation that genuinely completes may be re-marked Done — designed, not a fight). Cloud hook uses outcome-aware persistence (updateSessionWithOutcome) so dropped read-only/tombstoned writes are observable rather than silently swallowed. reportDoneClearedByMerge WARN volume is expected to rise (every cross-surface reopen trips it) — observability-only, not silenced. No store-version bump, no schema change (doneAt was already allowlisted in AgentSessionMetadataPatch and the conflict-detector tiebreaker). Works on mobile via the cloud path. Keep public copy non-technical — no doneAt/lifecycle/file names in the visible line. -->
...
<!-- detail: 260729_policy-sync-ack-guard (plan docs/plans/260729_policy-sync-ack-guard/PLAN.md; CE2). Inbound-author-policy changes (who can message Rebel) were dual-written to the cloud fire-and-forget: a failed or cooldown-skipped forward left the cloud enforcing the previous policy with no signal anywhere. A policy sync observer now tracks an enforcement fingerprint (mode + allow/block/trusted/agent lists — not notices or the device-local counter) through each forward's settlement, derives pending/synced/unsynced state (never stored), retries with bounded backoff on focus/reconnect/manual sync, and surfaces a non-dismissible inline Notice ("Messaging access hasn't synced" / Try again) at the messaging-policy subsection until the cloud confirms. Desktop forwards are serialized FIFO; cloud PATCHes that omit the policy now preserve the stored one (single-key re-injection, not deep merge); the mobile Help toggle PATCH was narrowed so it stops being an accidental stale-policy writer; the Slack bypass writer reads fresh state immediately before writing. cloudMigrationService is exempt (blocking wizard + always carries the full policy), pinned by an invariant test. Cross-device ordering (CAS) deliberately out of scope. Keep public copy non-technical. -->
...
<!-- detail: 260728_liquid-glass-app-icon (plan docs/plans/260728_liquid-glass-app-icon/PLAN.md; CE2). Rebuilt the macOS app icon as a layered Icon Composer source so macOS 26 (Tahoe) derives Light, Dark, Tinted, and Clear treatments from one source with the Liquid Glass look; macOS 25 and earlier keep the existing .icns unchanged. Windows, Linux, and the DMG icon are untouched. Suggested by a Rebel user on the community forum. Keep public copy non-technical — no Icon Composer/Assets.car/CFBundleIconName/actool internals. -->
...
<!-- detail: 260729_approval-decline-only (plan docs/plans/260729_approval-decline-only/PLAN.md; CE2 bug_mode). When Rebel asked to look through a folder it could never be given one-click access to (a top-level location like /workspace, /Users, or a drive root), the approval card used to show a greyed-out "Allow just this once" with no explanation — the only working button was "Don't allow," which read as a dead end. Two paired fixes: (1) new asks of that kind no longer become cards at all — Rebel's file-access policy now blocks them at the source and tells the agent to use a more specific folder instead of waiting for an approval that could never come; (2) older saved requests of that kind now render as an honest blocked-status card with a plain-language explanation, a Dismiss action, and "Tell Rebel what to do instead" — never a dead button. The 260712 never-offerable-root floor is preserved; this is the 260505 "actionable or explains itself" invariant applied to the empty-scopes case. Keep public copy non-technical — no scopes/policy/subtree/producer language. -->
...
<!-- detail: Rebel's memory safety ladder now has four plain-language choices for when Rebel asks before saving. Shared Spaces can use the three asking options; "Save without asking" is for private Spaces. The new "Ask only when Rebel spots something specific" choice is opt-in, and existing Spaces keep "Ask whenever Rebel is unsure" unless you change them. In one two-week sample on one setup, 29 of 147 saves to shared Spaces — about 1 in 5 — produced an approval card, and 19 of those cards came from uncertainty rather than a named concern. The new choice is designed to remove only those uncertainty-only interruptions when Rebel names no concern; the future reduction has not yet been measured and will vary. Built-in checks for passwords and keys continue to run on shared Spaces. -->
...
<!-- detail: 260728_dismiss-teach-custom-rule (plan docs/plans/260728_dismiss-teach-custom-rule/PLAN.md; CE2 medium, cross-family reviews). The Dismiss/Teach dialog's note field was feedback-only and the suggested-rule ladder offered only single-axis deterministic options relabelled by an LLM pass that could not introduce new axes. The note field is now rule material: type why an action missed, press the new "Remember this" button, and Rebel grounds the note into a structural rule in one non-mutating LLM pass, shows the interpreted rule in a toast with Undo, and if it cannot honestly ground the note (e.g. a concept with no matching facet on the item) it says so and keeps the note as feedback instead of inventing a loose approximation. The suggested-rule ladder can now propose compound (multi-axis) rules (e.g. meeting-prep actions tagged "personal"), not just single-axis ones. Copy reframe: the note label now asks "Why should Rebel skip these?" (create) / "Why should Rebel leave these alone?" (archive/complete) with a discoverable hint before typing. Grounding is fail-closed by design: a wrong-but-plausible rule is worse than no rule, so the motivating "family" concept (no current facet) intentionally takes the visible refusal path. No store-version/ledger change. Keep public copy non-technical — no matcher/axis/grounding/dual-write internals. -->
...
<!-- detail: 260727_sentry-triage-beta-batch (plan docs/plans/260727_sentry-triage-beta-batch/PLAN.md; CE2 bug_mode). Turns delayed while the computer sleeps or is busy now get one fresh wait period instead of being abandoned immediately; genuinely stuck setup remains bounded. Workspace/email connectors now get one automatic startup retry, except when the connector is missing or access is denied. The cloud-Space availability check now uses the same health window as the rest of Rebel, removing the still-healthy false alert while genuine offline/reconnecting states remain. Keep public copy non-technical. -->
...
<!-- detail: 260727_safety-quota-honesty (plan docs/plans/260727_safety-quota-honesty/PLAN.md; CE2 bug_mode + feature). When the AI plan Rebel uses for safety checks hit its usage limit (Codex usage_limit_reached, multi-day reset), approval cards and the agent-facing tool result said "temporary glitch" and the only honest surface was a 4-second toast; the diagnostics ledger also silently dropped every safety-eval-degraded event because its zod schema rejected them. The ledger schema now derives its cooldown shapes from one focused contract module (so the same drift can't recur), reports the actual evaluator attempt count, and threads the structured cause into Sentry. The persistent surfaces — staged-approval cards (tool and memory), agent-facing tool result, toast, and diagnostics hub row — now carry deterministic, provenance-framed copy with the estimated reset as an absolute local datetime ("expected to reset Sun, Aug 2 around 5:13 PM"), baked at queue time so they stay truthful days later; "Safety checks resumed." appears only on confirmed recovery, never because an estimate elapsed. The fail-closed posture is unchanged: nothing auto-runs, every action still needs per-action approval, no auto-failover was added. Copy is provider-agnostic and ownership-neutral, and only deterministic builder output ever populates eval-error reason — never raw provider text. Desktop + mobile parity via the existing reason carrier; cloud catch-up preserves it. Keep public copy non-technical — no zod/carrier/contract-module internals. -->
...
<!-- detail: waitForModelReady could wait forever when the GPU backend had been disposed while the CPU fallback was ready, freezing the RebelSettings package across queued calls. Readiness now delegates to the bounded initializer and rejects during shutdown. The same fix reports embedding degradation honestly, rejects persistence failures, caps add batches at 15 before work begins, removes redundant readiness waits, and fixes list_model_profiles payload formatting. -->
...
<!-- detail: Ordinary conversation writes, including 15-second checkpoints, used synchronous file operations on the app's main event loop. One measured save blocked it for 11.6 seconds on a distressed cloud-synced drive. These writes now use awaited asynchronous file I/O coordinated across the process; save wall-clock time under a slow filesystem is unchanged, but the app remains responsive while the write completes. -->
...
<!-- detail: 260721_tiptap-blank-line-roundtrip Stages 4+5 (CE2 bug_mode, second session). The Library editor's content-sync compared serializer-normalised markdown strings to decide "did the document change", so opening a file whose markdown doesn't round-trip canonically (blank-line runs, <br> tags, etc.) made the editor's own echo look like a user edit — the 1400ms autosave and the unmount cleanup then rewrote (and normalised/corrupted) files the user never touched; 45 files hit on one machine, 35 on a company shared drive. Replaced with explicit delivery provenance: an atomic {documentId, content, revision} envelope where the revision advances only on genuine external application, emitUpdate:false + addToHistory:false on every programmatic apply, and a source-aware write chokepoint (two-lane generation watermarks) so every disk write requires an unpersisted user-origin mutation. Also kills the false "changed elsewhere" conflict on untouched docs and no longer discards an external restore whose bytes equal an older editor state. Keep public copy non-technical — no envelope/revision/serializer internals. -->
...
<!-- detail: 260721_tiptap-blank-line-roundtrip Stage 1 (CE2 bug_mode). Upstream @tiptap/extension-paragraph parseMarkdown unwraps a single-image paragraph into a bare inline image node, so a standalone ![alt](path) line produced a schema-invalid document (doc = block+ violated). ProseMirror validates lazily: the document rendered and plain typing worked, but every structural edit (Enter, Backspace-join, multi-line delete) threw "Called contentMatchAt on a node with invalid content". Fixed via a per-instance LibraryParagraph parse override in the new libraryEditorFactory.ts (single source of truth for production + test extension arrays), plus a post-parse doc.check() that degrades loudly instead of silently accepting an invalid document. Round-trip stays byte-identical, so no new diffs in user files. Keep public copy non-technical — no ProseMirror/schema/node/parse internals. -->
...
<!-- detail: The eval exposed Fireflies tools under a generic package name, unlike production's named connector discovery, and recorded zero transcript-retrieval traces in six runs. The automation prompt now requires directly querying connected transcript services, while a shared note validator sends malformed paths, source types, or source links back for one repair attempt before allowing the capture through. Retrieval improved to 4/4 post-parity runs; the reproducible correctness proof then passed 3/3 with the exact source account and a verbatim transcript appendix in every note. Capture correctness is fixed; the full-capture behavior's cost and context envelope is tracked separately as efficiency work, not a correctness gap. -->
...
<!-- detail: Real-time voice companion plan docs/plans/260723_voice-companion/. Long-running turns now have one busy-state grammar: Queue (after), Nudge (into), Stop & send (instead). Directive Inbox injects nudges only at safe model-request boundaries, with visible On its way/Picked up/missed states and no-loss fallback. Journal-grounded check-ins answer progress questions without interrupting the worker. Desktop voice uses the same routing and can speak gated proactive updates; mobile v1 can send/track text nudges but does not add voice companion controls. Keep public copy non-technical. -->
...
<!-- detail: enablePriorTurnsHeader default flipped false->true (src/core/services/settingsStore/index.ts). Cross-turn awareness header (Layer 1, built 260525) that lists prior turns' file reads / searches / materialized outputs so a follow-up turn doesn't redo them. Validated by deterministic real-session replay (readPriorTurns over real transcripts+session store, REBEL_USER_DATA): 100% coverage — the header surfaced 175/175 of the file paths actually redundantly re-read across 35 real multi-turn conversations. The knowledge-work eval could NOT validate it (loadSession returns null in the eval's synthetic sandbox), so the go decision rests on the real-session replay, not the eval. Guards (legitimate re-read after a change; prompt-injection resistance via ZWSP sentinel transform) are deterministic + unit-tested. Also covers the clarifying-question continuation path (disk+accumulator merge) so the same recall works right after Rebel asks a question, not just on plain follow-ups. Keep public copy non-technical. -->
...
<!-- detail: New direct connector opusclip-official ("OpusClip (Official)"): vendor-hosted MCP at https://mcp.opus.pro/mcp with OAuth via WorkOS AuthKit (DCR registration_endpoint verified 2026-07-18, so no pre-registered oauthClientId needed; same shape as the notion entry). 26 documented opusclip_* tools (12 read / 14 write) + thumbnails; setupNotice covers the Pro Beta/Max/Business plan requirement for tool calls. Sits alongside our own bundled-opus-video-clip wrapper (unchanged); possible future deprecation of the wrapper flagged in docs/project/mcps/OPUS_CLIP_MCP.md. Vendor-official hosted service, so the 3rd-party security-review gate doesn't apply. Keep public copy non-technical — no OAuth/DCR/endpoint internals. -->
...
<!-- detail: New direct connector replit-official ("Replit"): vendor-hosted MCP at https://replit-mcp.com/server/mcp (streamable HTTP) with OAuth 2.1 + PKCE; DCR registration_endpoint verified 2026-07-18 via the https://replit.com/oidc openid-configuration, so no pre-registered oauthClientId needed (same shape as the notion entry). 3 tools (create_app_from_prompt / update_app_using_prompt / ask_question); builds are async and a -32001 tool timeout means still-building, don't retry. maturity: beta with setupNotice carrying the vendor's beta warning. Existing bundled-replit-ssh display-renamed to "Replit (SSH)" (id unchanged, no migration) so the official card takes the plain "Replit" name. Vendor-official hosted service, so the 3rd-party security-review gate doesn't apply. Keep public copy non-technical — no OAuth/DCR/endpoint internals. -->
...
<!-- detail: 260713_fix-voice-provider-override (plan docs/plans/260713_fix-voice-provider-override; internal ticket). applyAuthConfigToSettings validated the local voice provider against a hardcoded allow-list that had drifted (missing 'openrouter'), so a routine auth-config sync overwrote an OpenRouter user's chosen provider with the server default 'openai-whisper'. Fix derives the allow-list from the canonical VoiceProviderSchema enum (exported from src/shared/ipc/schemas/settings.ts, compile-time pinned to the VoiceSettings union) plus an enum-driven drift-guard test asserting final persisted state. Keep public copy non-technical — no allow-list/auth-config/schema internals. -->
...
<!-- detail: web-search CAPTCHA card confusion fix (internal ticket; CE2 bug_mode, cross-family design + code review). Rebel's built-in web search uses DuckDuckGo on a shared outbound connection, which occasionally returns a CAPTCHA under load. A prior change turned a latent plain-text narration into a prominent deterministic "Upgrade your search / add Brave Search" card that ALSO fired on Rebel's OWN per-turn 5-search cap — non-technical users read it as "my internet is broken and Rebel is upselling me". Fix: replaced the hard 5-search/turn cap with a process-wide token-bucket PACER (burst 5, ~1 search / 2.5s sustained, proceeds after an 8s ceiling — a pacer, not a wall) plus a generous neutral 20/task runaway-loop backstop that returns plain prose, never a card. The connector card now fires ONLY on a genuine external DuckDuckGo CAPTCHA, with blame-free, cause-first copy ("Search hit a snag"; the cause sits on the always-visible line since the card collapses its detail line; options "Set up search" / "Wait it out"). Also corrected the stale Brave Search help doc (the free tier ended Feb 2026 → metered pay-as-you-go). Keep public copy non-technical — no pacer/backstop/CAPTCHA-cooldown/Sentry internals. -->
...
<!-- detail: 260710_deprecate-teams-license-gate (plan docs/plans/260710_deprecate-teams-license-gate/PLAN.md). Removes Teams-licence gate on creating additional Spaces (including shared-drive Spaces); now available to all users. -->
...
<!-- detail: 260709_completion-autoresolve-default-on (plan docs/plans/260709_completion-autoresolve-default-on/PLAN.md). Makes the agentic completion-checker (built behind-the-scenes flag-off in 260707 M2) always-on for everyone by removing the inboxCompletionAutoResolveEnabled master gate (Greg's product call). Auto-completed actions surface under "Handled by Rebel" (completedBy:'rebel' auto-archived view). Behaviour-only change: the read-only + fail-closed + safety-gated probe path and fail-open silent-failure discipline are unchanged — Rebel only marks something done when it has real evidence, never on a check that couldn't run. Settings field kept as a deprecated persisted no-op for rollback compatibility. Keep public copy non-technical — no flag/probe/ledger/store internals; focus on "Rebel now closes actions it has genuinely finished, and tells you." -->
...
<!-- detail: 260708_library-browse-rearch (plan docs/plans/260708_library-browse-rearch/PLAN.md; CE2 bug-fix + small feature). Fixes the reported "the Library dumps me back to the main screen when I close a tab or pane." Root cause: the Library surface fully UNMOUNTS whenever you leave its tab (FlowPanelsShell renders inactive panels as null for perf), destroying all in-memory browse state (folder-tree expansion, selected node, active Space scope) and reloading the tree collapsed to the top level — only the lens survived, because it alone was persisted. Fix (renderer-only; no IPC/store/prompt/persisted-contract change): a new durable browse-position record library.nav.v1 in localStorage (mirroring useLibraryLens/library.lens.v1) holding spaceFilter + focusedPath + expandedDirectories, written on change and restored on remount. Restore seeds Space scope synchronously (useState initializer), seeds folder expansion, and queues the last-focused node into the proven pendingFolderNavigation reveal path; the mount tree-load's resetExpanded now fires ONLY on a genuine mid-session workspace-root change, not on every remount. Fail-open throughout: a stale/deleted/outside-workspace focused path is skipped (no phantom selection, no nag), a corrupt localStorage entry is discarded, quota/private-mode write failures are swallowed observably via ignoreBestEffortCleanup; expansion set capped at 500 dirs. Cross-family review (GLM-5.2) caught + fixed a phantom-directory-selection regression on a deleted focused node in a complete tree (F2, pinned by test), a marker-leak that could suppress a later explicit reveal's toast (F1), and a stale-Space-scope-on-workspace-switch path (S1). DEFERRED: decoupling the explicit "Back to overview" button from the reset (plain editor-close already preserves position); per-workspace re-restore without unmount; broader IA rework. Keep public copy non-technical — no localStorage-key/unmount/reveal-effect/IPC internals. -->
...
<!-- detail: plan docs/plans/260710_fix-preflight-timeout-false-missing (CE2 bug_mode — cross-family GPT+Opus diagnosis, 3 adversarial review rounds, mandatory postmortem). Windows-only. On slow / AV-scanned / cloud-synced (OneDrive) machines the startup health checks timed out; the preflight collapsed "timed out (unverified)" into "confirmed missing/failed" via one predicate → false hard-blocker "a required component is missing" cards (Git / Node runtime / MSVC), a false "MSVC runtime missing" Sentry error, and possible onboarding lockout — components were present and functional. Fix: discriminated CheckOutcome union (timeout ≠ missing, contradictions unrepresentable by construction); timeouts render a non-blocking "couldn't check in time — Check again" warning while genuinely-critical checks stay fail-closed on a *real* fault (which fails fast, not via timeout); MSVC Sentry fires only on a confirmed-missing DLL; health-check process spawns de-synced off the main thread (async execFile) and the onboarding gate made spawn-free so one slow/AV-held spawn can't starve the checks into false timeouts. Recurrence of 260704; drained the kill-by-construction prevention (ESLint ban on sync spawns in the health path). Keep public copy non-technical — no predicate/union/execFile/CreateProcessW/Sentry internals. -->
...
<!-- detail: 260708_action-context-menu-copy-link (plan docs/plans/260708_action-context-menu-copy-link/PLAN.md; feature build). A right-click / ⋯ context menu on every Action card, plus a shareable rebel://tasks/{id} link. Menu items are state-aware and gated per view (Go/Open details/Open conversation/Open file/Copy link/Mark done/Teach Rebel/Restore/Dismiss): run/mark-done hidden in restore views, Go disabled while executing, Open-conversation prefers the completing session in Done view (registry getInboxMenuItems in inboxCardMenuActions.ts, cursor-anchored @floating-ui menu in InboxCardContextMenu.tsx, right-click + Menu-key/Shift+F10 wiring in InboxCardFrame.tsx). Copy link writes the canonical rebel://tasks/{id} (buildInboxItemLink → formatNavigationUrl). Pasting that link into the composer and sending attaches the Action's content (title/details/source/draft/references) as a bounded markdown attachment so the agent can act on it — extractInboxReferenceIds + prepareInboxAttachments in useInboxMentions.ts, hydrated desktop-locally via inbox:load-items (non-UUID/missing ids logged, never silently dropped), wired into App.tsx buildAttachmentsForPrompt + the preparing-context spinner gate. Clicking/OS-opening a link navigates to Tasks and focuses the Action (focusApprovalId consumed in InboxPanel with observable already-done / not-here toasts, one-shot cleared via onFocusApprovalIdConsumed). No store-version bump, no new persisted fields, no IPC contract changes. Keep public copy non-technical — no registry/IPC-channel/floating-ui/attachment-payload internals. -->
...
<!-- detail: 260708_action-space-surfacing (plan docs/plans/260708_action-space-surfacing/PLAN.md; medium feature build; builds on 260707_action-space-tags). Makes the per-Action Space tag a usable filter/scope dimension beyond the action card. Renderer-only — no IPC/store/prompt/persisted-contract change. (Actions) a removable Space-scope chip at the front of the Actions search input + a "Spaces" section in the search dropdown, bound to the SAME selectedSpace state as the sidebar Space filter (one source of truth, no forked copy); result filtering already AND-combined with the sidebar filter, so this is visibility + control placement only. (Library) activating a Space (via the existing Spaces lens) now actually scopes browse (Cards + Folders) AND the rail search corpus by path-prefix, with a shared SpaceScopeChip in the lens bar and scoped, one-click-exitable empty states — previously it set state but filtered nothing. Cross-surface shared vocabulary: cleared = "All Spaces"; Actions-only null bucket = "Unfiled" (Library omits it by design). Fail-open on an unresolvable/renamed/deleted Space (chip hides, everything shows — never a dead surface). Designer defaults flagged to Greg: Library scope persists within-session/resets on restart; Library omits the "Unfiled" scope. Deferred (R7): the Library rail's non-search browse tree isn't scoped yet. Keep public copy non-technical — no selectedSpaceFilter/path-prefix/chip-component/IPC internals. -->
...
<!-- detail: 260708_quality-tier-3-level-planmode (plan docs/plans/260708_quality-tier-3-level-planmode.md; diagnosis docs/plans/260707_plan-mode-regression-hunt/PLAN.md). Fixes a reported "planning mode isn't showing" via two root causes: (1) the 4-level quality slider silently disabled plan mode on 3 of 4 tiers (plan mode is config-derived — engaged iff planner model ≠ working model — and only "Thorough" engaged it, with no UI signal), (2) planner output was clamped to 4096 tokens, truncating large plans mid-JSON so zero tasks seeded. Collapsed 4 tiers → 3 (Quick/Balanced/Maximum); Balanced is now the default and the plan-mode tier (Sonnet working + Opus planning); added a plain-language plan-on/off indicator beside the slider (derives from resolved model ids, mirroring the engine); raised the planner output ceiling to 16384. Keep public copy non-technical — no tier-internals/token-ceiling/config-derivation details. -->
...
<!-- detail: 260707_up-next-actions-shortlist (plan docs/plans/260707_up-next-actions-shortlist/PLAN.md; medium CE2 build). Opt-in "Up Next" shortlist shelf pinned above the Actions list. Deliberately manual — NO auto-scoring, NO auto-seeding, NO ranker: the user adds actions themselves. Cap 5 saved / 3 visible; finishing one triggers a completion moment + reshuffle; an opt-in "fill the shelf" seed and a reshuffle are offered as gentle momentum. Content-free telemetry (counts/enums only, never titles). Additive-optional upNext/upNextAddedAt threaded through InboxItem + IPC + inbox:update-fields + core store (no store-version bump; fingerprinted rollback assertion). Grounded in procrastination-psychology + task-shortlist-UX research. Keep public copy non-technical — no field/IPC/telemetry/cap internals. -->
...
<!-- detail: 260707_action-space-tags (plan docs/plans/260707_action-space-tags/PLAN.md; medium feature build). Each newly-created action (InboxItem) is classified by the LLM into its most-likely related Space (or None) as async background enrichment AFTER persistence, surfaced as a badge on the action card + a Space filter in the Actions sidebar (mirrors the "From" source filter). Structured additive field spaceAssignment {path:string|null, source:'llm'|'user', status:'pending'|'classified'|'failed', at:number} on InboxItem; INBOX_STORE_VERSION 6→7, DATA_SCHEMA_EPOCH 139→140, safe/additive_unknown_field_preserving ledger entry. New actions only (no backfill); manual override is sticky (source:'user' → never re-classified); deterministic 0/1-space short-circuit skips the LLM. Card render: undefined→no badge (legacy); pending→"Sorting…"; classified+resolvable→space name; classified-null/failed/unresolvable→"Unfiled". New inbox:set-space IPC for the override. Keep public copy non-technical — no spaceAssignment/IPC-channel/store-version/enrichment-orchestrator internals. -->
...
<!-- detail: 260707_actions-teach-rebel-rules (plan docs/plans/260707_actions-teach-rebel-rules/PLAN.md; direction docs/project/ACTIONS_DIRECTION.md; staged M1 run). Greg's three asks + engine activation. (M1.1) Removed the experimental "Teach Rebel your rules" Settings toggle (actionsTeachingEnabled) and its gates across settings schema/store, resolutionRulesStore (getActiveRules/recordRuleRestore choke points), inboxHandlers, automationScheduler userInstructions, and 3 renderer surfaces — the resolve/protect/create engine is now always on for everyone. Byte-identical to the prior dormant state until a user teaches their first rule (empty store, no seed, no migration; verified via GLM-5.2 cross-family review). (M1.2–M1.5) A graduated deterministic rule-option ladder (just-dismiss floor → topic-by-tag → keyword → source → category), grounded so a rule can never be persisted whose matcher doesn't match the item it came from; an LLM reranker/relabeler (BTS) that only reranks/relabels the grounded axes — person/company/account axes structurally unrepresentable + banned + label-checked (M1.6-refinement F4 widened that label-honesty guard from keyword-only to every non-source axis); an eval STOP-gate (evals/actionRuleOptions.ts, 3×100% baseline, manual not CI); inbox:rule-options IPC + useRuleOptions hook. (M1.6) Unified DismissTeachDialog replaces TeachRuleDialog + ActionDeleteReasonDialog: reason textarea + dictation mic on top, compact radiogroup ladder below with the just-dismiss floor pre-selected (teaching offered, never forced); one body serves every entry point (Dismiss button, the caret "Teach Rebel about this…", and restore/reopen from Undo) via a target prop. Behavior change (GLM-5.2 F1): the caret entry now dismisses the action as well as opening the teach panel. Keep public copy non-technical — no engine/flag/grounding/axis/IPC/LLM-validator internals. -->
...
<!-- detail: 260707_actions-inbox-controls (plan docs/plans/260707_actions-inbox-controls/PLAN.md). Actions inbox controls (feature build, not a bug fix). (S1) Per-view ordering dropdown (RichSelect, ArrowUpDown) scoped to the Active and "Handled by Rebel"/auto-archived views only; other views keep the byte-identical 'smart' sort. Canonical sortInboxItems(items, mode, {groupByTemporal}) lifted to @rebel/shared with InboxSortMode = smart|priority|due|added|rebel-updated and a shared rebelResolvedAt accessor; non-smart Active modes prepend TEMPORAL_RANK so Today/This Week bands survive. Per-view choice persisted to localStorage rebel:actions:sort:<view> (written only on explicit change; validated against the per-view option set — glm-5.2 F5). (S2) All/Completed/Cleared pill tabs in the auto-archived view (completedBy==='rebel' vs dismissedBy==='rebel'). (S3) "Check now" button → new cloud-routable dual-write inbox:check-now IPC: runs the deterministic freshness janitor synchronously (honest tidied count) + fire-and-forget LLM source-capture sweep (never awaited, never fabricated into the count); explicit unavailable-handler guard, no silent success (glm-5.2 F1/F2). (S4) Recently-added dot (addedAt > last-visit baseline; empty on first visit). (S5) Attribution label fix — "Rebel marked this done" / "Rebel cleared this", mutually exclusive per card+modal (glm-5.2 F7); handled-count filter widened to completedBy|dismissedBy==='rebel'. Two autonomous copy calls ("Check now", "Cleared") flagged to Greg for veto. Keep public copy non-technical — no IPC/localStorage/sort-mode/TEMPORAL_RANK internals. -->
...
<!-- detail: 260707_fix-meeting-bot-spark-image (internal ticket; plan docs/plans/260707_fix-meeting-bot-spark-image/PLAN.md; bug_mode). The Meeting Bot's video tile (an animated Rebel "spark" avatar rendered by Recall.ai Output Media from our hosted avatar-webpage) was showing only the bot's name text with no image. Root cause established as a video-render failure with NO visual fallback and NO observability — if the per-state MP4 (served from R2) failed to paint, the tile fell back to a bare dark background, indistinguishable in the field from Recall never compositing our page at all. Fixes across two independently-deployed surfaces (Cloudflare Pages + wrangler; NOT bundled with the Electron app): (avatar-webpage) a branded spark still (assets/spark_poster.jpg, generated from spark_idle.mp4) is now the <video> poster AND a CSS background layer on .avatar-image, so a still image is ALWAYS painted even when live video fails or before first frame; bounded exponential-backoff video retry (3 attempts) on error/stall/autoplay-block with supersede-guards; a first-frame watchdog (8s); and a fire-and-forget diagnostic beacon (navigator.sendBeacon, text/plain to avoid CORS preflight) that reports video_error / first_frame_timeout / autoplay_blocked / first_paint / video_giveup keyed by botId — this DISAMBIGUATES our-page-blank (locus A, poster fixes it) from Recall-never-composited (locus B) in the field. (meeting-bot-worker) a new POST /api/avatar-diagnostic route logs the beacon with an [AVATAR_DIAGNOSTIC] prefix (size-capped, sanitized, no KV, no auth, always 202). Also folded in: transcript WebSocket no longer reconnects on a clean 1000/1001 close (F10), and pending video-retry timers are cleared on state change (F2). Deferred: variant/CPU-starvation locus-B hypothesis (F7, risky to ship blind), URL param-validation hardening (F8/F11). Keep public copy non-technical — no Recall/Output-Media/beacon/poster/locus/wrangler internals. -->
...
<!-- detail: 260706_atlas-2-redesign-phase0 + 260707_atlas-richer-browse-r2 (plan docs/plans/260706_atlas-2-redesign/PLAN.md; docs/plans/260707_atlas-richer-browse/; direction docs/project/ATLAS_DIRECTION.md). Phase 0 of the Atlas 2.0 redesign: removed the 3D force-directed graph visualization (~1,880 LOC of WebGL + react-force-graph-3d/-2d + transitive three) and rebuilt the Atlas surface as a faceted browse-first list (reusing the existing virtualised CardsView as the master list) with a new detail panel + Related rail in src/renderer/features/library/components/detail/. Neighbor hydration extracted into useFileNeighbors; AtlasNode type relocated to features/atlas/hooks/types.ts. Projection IPC channel + worker path + useAtlasProjection hook dropped in the same commit; neighbor/embedding/AI-insight channels (search:atlas-neighbors, -neighborhood, -embed-query, -summarize-file, -summarize-neighborhood, -ask-question) kept. Analytics: dead tracking.library.atlas.viewed/searchUsed/spaceIsolated events retired; fileOpened/conversationStarted preserved; new gist/ask events added. Old 3D architecture doc docs/project/ATLAS_VISUALIZATION.md marked deprecated in-place, pointing to ATLAS_DIRECTION.md. Phases 1-3 (hybrid RRF search, conversations/spaces as rows, cluster-label headers, optional 2D graph tab) remain roadmap. R2 follow-up (next day) fixed the sharpest rough edges reported against Phase 0: the search box losing focus while typing, switching to Atlas silently resetting the view back to Folders, and the detail rail being a fixed width — it's now collapsed until an item is selected and drag-resizable. Tooltip depth and result clustering remain open follow-ups. Keep public copy non-technical — no WebGL/IPC/embeddings/force-graph/lint-gate internals. -->
...
<!-- detail: 260706_composer-large-file-ref (forum request: "local file picker alongside the attach button"). Instead of adding a second attach button, the existing composer attach gesture now auto-degrades an oversized file that has a recoverable on-disk path into a byte-free "linked" reference (a binary attachment with originalPath set, no base64Data) that Rebel opens from disk at run time — no upload, no size ceiling. Single-source predicate isLinkedReference() (isBinaryFileAttachment && originalPath && !base64Data); no new persisted field, no store-version bump. The renderer size-gate + extraction-failure branches (PDF/office/text/binary in useFileAttachments.ts) degrade instead of reject WHEN a path exists; the main-process byte-size filter (attachmentValidation.ts) exempts linked references (they carry no bytes). Cross-surface: cloud/mobile have no window.fileApi → originalPath '' → degrade no-ops → embed-only ceiling stays; oversized clipboard/cloud files still rejected observably; oversized images still rejected (a path-read image loses vision). UI: an informational Link2 corner badge + plain-language tooltip mark linked chips (gated on isLinkedReference, aria-label carries the status). Deferred fast-follow: the sent-message file row (MessageItem.tsx) shows no icon/size/distinction. Keep public copy non-technical — no originalPath/base64Data/predicate/IPC internals. -->
...
<!-- detail: 260706_feedback-multi-screenshot. Feedback & Bugs previously accepted exactly one screenshot (a single screenshotBase64 field). Now up to 5, as a thumbnail gallery: multi-file picker that appends not replaces, "+" add-tile, per-item remove, "N of 5" count, pluralised privacy warning + Rebel-voice validation for wrong-type/oversize/over-count/over-aggregate. Carried as a bounded screenshots[] array on the bug-report:submit-bug IPC contract (max 5; per-item + 7,000,000-byte aggregate cap = the proven single-image worst case, so the Sentry envelope analysis is unchanged); legacy singular request fields retired. Each screenshot is a separate additive Sentry attachment. OSS egress sends only the first screenshot for now (backend reads singular). Keep public copy non-technical — no base64/superRefine/IPC-contract/outbox/OSS-egress internals. -->
...
<!-- detail: 260706_library-skill-source-badge. Skill provenance in the Library (plan docs/plans/260706_library-skill-source-badge/PLAN.md). Typed provenance (spaceType/stable sourceId/sourceLabel) now flows scanner → library:scan-skills IPC (additive-optional, cross-surface) → renderer index → card entries → both grid and detail render paths. (Stage 2) a source badge on every non-platform skill card (leftmost, icon+label, tooltip) naming where the skill comes from; the useSkillsForYou Chief-of-Staff detection swapped from path.includes('chief-of-staff') string hacks to the typed spaceType. (Stage 3) useFilterGrouping groups skills by real source — Chief-of-Staff first, named Spaces alphabetical, Workspace, then Built-in last (Built-in collapsed by default once you have any skills of your own); dead 'community' grouping machinery removed. Security: a trustChiefOfStaffType guard demotes a frontmatter-claimed Chief-of-Staff space to a plain space unless it's the canonical CoS folder, so a collaborator's README can't launder CoS identity into the badge/Share-gate/suggestions. The CoS badge/section label follows the app's existing resolver ("Private Space"), not a hard-coded word. Keep public copy non-technical — no spaceType/sourceId/IPC-channel/grouping internals. -->
...
<!-- detail: 260706a_actions_ui_refresh. The Actions surface refresh (plan docs/plans/260706a_actions_ui_refresh.md; direction in docs/project/ACTIONS_DIRECTION.md). Consolidates several stages: (S7) sidebar unification — the InboxFilterDropdown is replaced by a left <aside> with Active/Done/Dismissed/Auto-archived views; Search/From/Priority refinements now persist across a view switch (handleViewModeChange only clears selection), Priority filter is Active-only-gated (effectivePriorityFilter) so switching to Done can't silently narrow, and a Clear-filters recovery affordance is mandatory-visible. (temporal) Active list groups by Today/This Week/Upcoming/Someday via TemporalGroupView; Someday added as a no-deadline bucket. (editing) title/text/draft editable in place via inbox:update-fields (Zod {id, title?, text?, draft?}, ≥1 required, trusted like a user-request add). (S6) auto-mark-done renamed "Mark done when finished", demoted from the card to the detail modal + a global Settings default. (CTA) primary button collapsed to a constant "Send" (resolveInboxCtaLabel returns 'Send'); prompt still varies by tier. (S8) reference/provenance hover previews on cards. (S9) removed dead share-to-social plumbing + Eisenhower-quadrant/layout-mode props (the quadrant grid UI was retired in an earlier release; urgent/important flags now only feed the Priority filter). Keep public copy non-technical — no ViewMode/effectivePriorityFilter/IPC-channel/quadrant internals. -->
...
<!-- detail: 260705_actions-resolution-r2 (R2 of the Actions Resolution Engine; master plan docs/plans/260705_actions-resolution-engine.md § R2). The deterministic freshness rules that were too false-positive-prone to auto-archive (kept telemetry-only shadow rules since R1) now WRITE low-confidence `stale` resolutionSuggestions onto items instead of acting. Those items surface in a new "For you to confirm" queue pinned to the top of the Active view (default-expanded, ranked by confidence, confidence NEVER rendered — rank-only). Each row: outcome chip (Looks done / Looks stale) + plain-language reason + Confirm / Keep. Confirm files the item (completed→Done, stale→archived) with Rebel attribution + Undo; Keep rejects and records the evidence fingerprint so the SAME evidence never re-suggests. Bulk "Confirm all" loops the confirm path with a single batched Undo scoped to only the items that filed successfully. New additive-optional resolutionSuggestion + rejectedFingerprints fields (no store-version bump). Confirm/keep is one cloud-routable dual-written IPC channel (inbox:resolve-suggestion) — desktop/cloud/mobile parity. NO LLM judging, NO connector evidence-gathering (that's R4). Telemetry: counts + enums only. Keep public copy non-technical — no suggestion/fingerprint/confidence/channel internals. -->
...
<!-- detail: 260704_out-of-space-file-access. New "Locations" permission class lets Rebel's built-in file tools read files/folders OUTSIDE the workspace/spaces/~mcp-servers zones, with consent. Read-only in v1 (write/delete outside a zone stay hard-denied). Deterministic JSON grants (surface-local store, never cloud-synced) are the ALLOW authority — path-prefix match over canonicalized realpaths, not word-matching. Surfaces: (1) in-conversation deny-then-ask card with graduated scope (just this file / the folder) + lifetime (just this once / this chat / always) + "tell Rebel what to do instead"; (2) Settings → Safety → "Folders Rebel can read" with per-grant revoke + an "Add a folder…" picker; (3) powerbox — a file you attach/pick/drop becomes an automatic session-lifetime read grant so a follow-up read doesn't re-ask. Sensitive dirs (credentials, keychains, browser profiles, Rebel's own config) are a hard non-grantable carve-out even under a parent-folder grant. A Bash skip-gate demotes out-of-zone reads to full evaluation so Bash can't route around the ask. Desktop-first (cloud sessions stage the ask). Keep public copy non-technical — no grant-store/hook/canonicalization internals. -->
...
<!-- detail: 260705_rebel6jr-plaud-health (internal ticket). Plaud "connected" was derived from account-file existence only (isPlaudConnected / plaud:get-connection-state returned connected: account !== null), so a dead/rotated single-use refresh token left Settings showing "Connected" while every periodic sync tick silently failed ensureValidToken() and returned {synced:0}. Fix: persist authInvalidSince on PlaudSyncState, set in checkAuthWithThrottledNotification ONLY on a non-transient failure (isTransientError gate — network/5xx/429 don't flip it) and cleared on the next successful auth; expose needsReconnect + authInvalidSince via plaud:get-connection-state; VoiceRecordersSection shows a "Reconnect needed" state + warning + Reconnect button (themed via --color-warning, light-mode covered); manual sync (plaud:trigger-sync) now returns success:false with a reconnect message instead of a silent success when the token is dead (PlaudSyncResult.authInvalid). Also hardened performPlaudRefresh to validate the refresh response before saveTokens() so a malformed 200 can't persist refresh_token:undefined and self-brick. Trigger was operational token death (~June 9, pre-July-1 single-flight fix), not a code regression. Keep public copy non-technical — no token/refresh/schema internals. -->
...
<!-- detail: 260705_memory-sharing-hardening. New "Shared memory suggestions" control in Settings → Safety (off by default) gates a future shared-memory distribution engine. With it off, nothing changes — no proposals are generated, none can be published, the dormant distribution automation is blocked from running. Opting in to "Suggest things to share (you approve everything)" will, in a future release, let Rebel propose captured content for sharing into shared spaces; every proposal still goes through your explicit approval. Turning it off stops future proposals; it does not recall files already shared or hide what flag-on teammates share. Shipped alongside a memory-safety hardening: a shared README's `sharing` line can no longer loosen Rebel's safety gate — only your local settings can permit a wider audience; a README can only tighten. The library scan no longer copies a collaborator's README `sharing` into your local settings, and a new health check flags README/settings sharing drift with direction. Do NOT mention "system-source-distribution", "frontmatter", "blast radius", "safety-effective sharing", gated IDs, or the investigation filename in public copy. -->
...
<!-- detail: 260705_actions-autoarchive-distinct. Make the freshness auto-archive honest + visible (batch 4 of the Actions direction review; decision + verbatims in docs/project/ACTIONS_DIRECTION.md § 2026-07-05). New dismissedBy: 'user' | 'rebel' discriminator (mirrors completedBy) added to InboxItem + both Zod schemas + toIndexEntry projection. Backend: periodicFreshnessCheck + retroactiveInboxCleanup stamp dismissedBy 'rebel' + a per-item dismissedReason (built from the reason strings they already generated, prefix-stripped) and emit emitInboxAutoArchive({count, rules}); manual dismiss defaults dismissedBy 'user' in setInboxItemStatus. Renderer: new 'auto-archived' ViewMode/filter option (shows dismissedBy==='rebel', renders the reason, and — unlike other views — does not hide non-actionable items so the list matches the notice count); Dismissed view now filters to dismissedBy!=='rebel' (legacy/no-dismissedBy stays in Dismissed). Notice: inbox:auto-archived broadcast → App.tsx toast (every pass, no threshold) that deep-links to the Auto-archived view via a one-shot pendingViewMode prop; "Inbox Auto Archived" telemetry (count + rule-id breakdown, no titles). Auto-archive reasons excluded from getInboxFeedbackExamples (not user calibration signal). Rule aggressiveness deliberately unchanged this run. Keep public copy non-technical — no dismissedBy/schema/broadcast/rule-id internals. -->
...
<!-- detail: 260705_actions-archive-accountability (slice 1b, completion side). Companion to the staleness accountability entry above. When Rebel's janitor archives an inbox item it now sends a resolution:'completed'|'stale' discriminator on rebel_inbox_update; 'completed' stamps completedBy:'rebel' + a new additive persisted completedReason field (no store-version bump, transient resolution not persisted). Renderer (InboxItemCard + InboxItemDetailModal) shows a quiet "Completed by Rebel" attribution + a "Why Rebel marked this done" reason section, mirroring the staleness UI — but NO toast (evidence-of-done archives are expected; toast reserved for staleness to avoid fatigue). Absent resolution stays byte-for-byte legacy Completed with no attribution; a staleness archive leaking to Completed is made observable via omission telemetry, not silent. Mobile reason-display deferred (data syncs, views don't render the fields yet). Keep public copy non-technical — no resolution/completedBy/completedReason/schema internals. -->
...
<!-- detail: 260705_actions-snooze. "Quiet by distance" snooze model (renderer-only, no schema/IPC changes; dueBy remains the single scheduling field). (1) Home Today stream excludes non-urgent actions with dueBy beyond end-of-today, filtered at the top of the funnel in useTodayStream so no backfill path (stale or broader) re-admits them; urgent priority-pinned items exempt. (2) Actions default All view: Today + This Week render flat, Later (upcoming) collapses into a count-labelled section (partitionLaterItems helper, localStorage-persisted expansion, "Inbox Later Section Toggled" event); keyboard nav + select-all skip collapsed items; explicit filters and Done/Dismissed views unchanged. (3) Resurface accent: explicitly-dated cards whose day arrived get a one-day primary-tinted left-edge accent (.cardResurfacedToday). Decision + verbatims in docs/project/ACTIONS_DIRECTION.md § 2026-07-05. Keep public copy non-technical — no bucket/dueBy/funnel internals. -->
...
<!-- detail: 260705_actions-quickfix-batch1. Two user-facing Actions fixes (plus internal analytics wiring, not mentioned publicly). (1) Actions nav badge no longer sums pendingApprovalCount — approvals render in the notification drawer, not the Actions tab, so the badge promised items the tab couldn't show ("1 awaiting review" → empty tab). This is a re-fix: the original fix (187c4828d7, Apr 20) was silently lost to a same-day merge (8317c54998); now pinned by a source-level test (actionsTabBadge.pin.test.ts). (2) Delete→Dismiss rename across the Actions surface: card button, detail modal, batch bar, confirm dialog, "Deleted"→"Dismissed" filter view, empty state, toasts, shortcuts dialog, help docs — the action was always a recoverable soft dismiss (status 'dismissed', Undo restores); the label now says so. No store/IPC/schema changes. Keep public copy non-technical — no counter internals, commit hashes, status enums. -->
...
<!-- detail: 260704_remove-compact-toggle. Removed the Settings → Advanced → "Context compaction" toggle and the AppSettings.experimental.compactEnabled field. The toggle opted into Anthropic's experimental compact_20260112 server-side context compaction API; verification confirmed it was Anthropic-wire-scoped (only Anthropic-direct or proxy-to-Claude paths), still live, and recently hardened — but narrow provider coverage and experimental status made it a poor default surface for a non-technical audience. Underlying compact_20260112 machinery in anthropicClient.ts stays dormant (hardcoded enableCompact:false at the clientFactory construction sites); full dormant-code cleanup tracked as a follow-up. normalizeSettings explicitly evicts stale compactEnabled:true from persisted user stores. Context compaction itself (automatic BTS summarisation when conversations get long) still runs — only the opt-in server-side Anthropic path is gone. Keep public copy non-technical — no compact_20260112, enableCompact, Anthropic-wire, or "dormant" internals. -->
...
<!-- detail: 260704_openrouter-mcp-catalog. Added the OpenRouter MCP server as a direct HTTP + OAuth connector in resources/connector-catalog.json (endpoint https://mcp.openrouter.ai/mcp, DCR-based OAuth — no pre-registered client, same shape as Notion/Linear/Plaud/Monologue). category development, accountIdentity email, maturity beta, icon brain, 11 tool manifests (models-list, model-get, model-endpoints, benchmarks, rankings-daily, app-rankings, chat-send, generation-get, credits-get, providers-list, docs-search). connectorCatalog.test.ts 55/55 pass; live endpoint + OAuth flow empirically verified. Note: chat-send consumes OpenRouter credits; the scoped OAuth key expires after 7 days with a $10 cap (reconnect to renew). Keep public copy non-technical — no DCR/endpoint/tool-manifest internals. -->
...
<!-- detail: 260704_actions-manual-add-reschedule. Made Actions usable as a permanent to-do list. (1) Manual add: an "Add an action…" composer at the top of the active Actions view (title + optional due date), shown whenever the Active view is open (including the first-run/empty state). Writes via the existing inbox:add IPC with category 'user-request' (trusted — bypasses the quality/dedup filters that gate agent-created items). (2) Reschedule to a specific date: each card's schedule control gains a "Pick a date…" option (native date picker) alongside the existing Today/This Week/Later presets; when an item has an explicit due date in the Later bucket, the badge shows the concrete date instead of "Later". Both new write paths route through guarded useInbox helpers (handleAddItem / handleSetScheduleDate) so they respect the same read-only/update-required guard as the existing priority/schedule controls. Renderer-only; backends (inbox:add, inbox:set-dueBy) already existed. Keep public copy non-technical — no category names, IPC channels, bucket/temporal internals, or "user-request". -->
...
<!-- detail: 260704_actions-ui-easy-wins. Two additive Actions card UI wins (renderer-only, no store/IPC/schema changes). (1) Overdue visible: the per-card schedule badge now shows "Overdue" (red) for any action whose due date has passed — previously such items bucketed into "Today" so the badge read "Today" and the overdue state was invisible. (2) "Tomorrow" schedule preset: a new option in each action's schedule dropdown (placed after Today, guarded on the same date-write path as "Pick a date…") that sets the due date to end of tomorrow — alongside Today / This Week / Later / Pick a date; the badge also surfaces a "Tomorrow" label. Extracted pure resolveScheduleBadge/formatDueDateLabel helpers into utils/scheduleBadge.ts with 10 unit tests. Priority-pinned (urgent) items keep their static "Today" badge (unchanged, by design). Keep public copy non-technical — no bucket/dueBy/temporal internals. -->
...
<!-- detail: 260704_rebel-6je-managed-creds-rejected (internal ticket). Fixed auth-rejection error attribution: the "credentials rejected" toast + recovery action now key off the executed credentialSource (the connection that actually failed) rather than activeProvider/entitlement. A managed subscriber whose turn ran on a personal OpenRouter connection and got a 401 previously saw "Your Mindstone subscription credentials were rejected → check subscription status"; now sees "OpenRouter rejected your credentials → reconnect in Settings". Threaded credentialSource through all @main auth producers (runtime-result + structured-error + coach/recovery funnels); managed context (mindstone-managed-key / missing-mindstone) still shows the subscription copy. Does NOT address the separate managedProviderDeactivated opt-out latch (a paying subscriber can still be pinned to a personal connection — product decision pending). Keep public copy non-technical — no credentialSource/activeProvider/producer internals. -->
...
<!-- detail: 260703_approval-rule-hierarchy. Improved the graduated rule-suggestion hierarchy Rebel offers when a tool or memory action is blocked (the "Always / Similar / This only" tiers). Prompt-and-context fix, not a model swap: family-branched generation, trusted code-computed audience context threaded to the generator so the tiers reflect who can actually see a saved note (team-only vs company-wide, no longer conflated), the exact file/topic pinned for the narrowest tier, and a Recommended option pre-selected (pre-select only — never auto-confirmed). Do NOT mention prompt internals, tier/scope names, family branching, sharing-class enums, or model names in public copy. -->
...
<!-- detail: 260704_mobile-shake-feedback. Mobile app (all builds, incl. TestFlight/Play Alpha): shaking the phone opens a quick feedback sheet (type picker + one required message field + diagnostics toggle) with a screenshot captured at shake time. Privacy-by-design: the screenshot is always shown (thumbnail + tap-to-enlarge), removable and reversibly re-attachable, never auto-sent; capture failure degrades to a calm note and the text still sends. Reuses the existing feedback pipeline (durable offline queue → /api/feedback → Sentry), so reports survive offline/kill. Settings toggle "Shake to send feedback" (on by default, device-local, not synced). First-run explainer inside the sheet, shown once. Deploy ordering: cloud schema must be live before mobile builds ship (older cloud silently drops the screenshot). Keep public copy non-technical — no library names, g-thresholds, queue/schema internals. -->
...
<!-- detail: 260703_safety-flag-why-explainer. Display-only change to the Safety Approval card (the "Rebel wants to save this file" staged-write approval in the notification drawer). Replaces the opaque "your safety rules flagged this" one-liner with a shared-visibility framing that names the load-bearing reason, plus an inline "Why is Rebel asking?" expander (new shared Disclosure primitive) carrying the deeper nuance. Deterministic renderer-only copy keyed on blockedBy + sharing; shown only for safety_prompt / sensitivity_eval (structural_policy / eval_error keep a complete one-liner, no expander). No safety DECISION logic changed, no LLM eval prompt changed. Deliberately does NOT surface the classifier's confidence enum or its raw free-text reason (jargon + could echo withheld sensitive content). Covers both the staged-file and the non-staged memory-approval surfaces. Keep public copy non-technical — no "confidence", "Safety Prompt", blockedBy, or Disclosure internals. -->
...
<!-- detail: 260703_fox-3604-doubleword-provider. Added Doubleword.ai as a new BYOK pay-per-use provider (ModelProviderType-only preset, Together/Cerebras class) offering cheap open-source models (GLM 5.2, DeepSeek V4 Flash) via Doubleword's async/flex tier. Positioned for background/automation work, NOT live chat — spike measured ~1 min per step throughout (no cross-call speedup observed), so latency framing must stay honest ("about a minute per step", not "warm up then fast"). Every Doubleword request carries service_tier:'flex'; BTS/background transport timeout extended to ~120s and the settings Test-button validation timeout to ~90s to survive the observed ~60s cold start. Initially excluded from Council/Smart-picking for the MVP, now enabled (260704_doubleword-smart-picker) via a flex-tier first-byte/first-chunk proxy floor (120s/180s) in streamingLivenessPolicy; the membership toggle chips are now always enabled with a "responds slowly" hint, in both the add-model wizard and the settings model-team views. DeepSeek V4 Pro was initially deferred (catalog id collision with an existing Together entry) but is now offered too, once per-provider pricing overrides landed — so all three models ship. Do NOT mention "flex tier", "service_tier", ModelProviderType, cold-start/queue internals, Together/Cerebras internals, or the V4 Pro pricing-collision internals in public copy — keep it to "cheap open-source models, about a minute per step, best for background/automation, bring your own key." -->
...
<!-- detail: 260702_fable-reenable. Anthropic restored Claude Fable 5 API access (withdrawn ~1-2 weeks in 2026-06). Re-enabled in the app: catalog isMainModel/isAuxiliaryModel flipped back to true on both the direct and OpenRouter Fable rows, featured add-a-model preset restored, RUN_FABLE_LIVE_TESTS gate removed. Fable now powers the quality slider's Maximum tier (working + thinking); the slider stays 4 tiers (no separate Frontier). Switching to Maximum via the agent-facing set_quality_tier now stages for approval (catalog-driven premium cost-escalation), same as a model-role change to Fable. Global defaults unchanged (Opus). Keep public copy non-technical — no flag names, tier internals, or catalog details. -->
...
<!-- detail: 260630_openrouter-stt. Added OpenRouter as a first-class speech-to-text provider (voice.provider='openrouter') for dictation + in-person/mobile/live-coach transcription, using OpenRouter's /api/v1/audio/transcriptions endpoint (shipped May 2026). Works for BYO-OpenRouter users (personal OAuth token pays) AND Mindstone-managed users (managed OpenRouter key — Mindstone pays, billingSource:'subscription'). No separate voice API key needed. Onboarding defaults voice to OpenRouter when the chosen LLM provider is OpenRouter/Mindstone (else keeps local default), graceful fallback to local. Cross-surface (desktop/cloud/mobile) — also closes the gap where subscription/managed users couldn't transcribe on mobile without a BYO key. TTS is NOT wired for OpenRouter (degrades gracefully to no speech). Do NOT mention endpoints, billingSource internals, or the credential resolver in public copy. -->
...
<!-- detail: 260701_reusable-mic-surfaces (+114c36fb3b grey-out). (1) Wired the 4 lock-blind mic surfaces (Today card, inbox card, inbox detail modal, Home-hero input) to the single-active-recorder lock so their buttons grey out (tooltip "Finish the other recording first.") while another mic records; enforcement is hook-level so it can't be bypassed. (2) Extracted a reusable useMicButtonController + shared MicButton (src/renderer/features/voice/mic) with the lock enforced by construction; migrated 5 simple surfaces onto it (composer stays bespoke by design); deleted the old inbox-scoped VoiceMicButton. a11y: added keyboard focus-ring + processing aria-label; tokenized hardcoded colors. Home-hero + question-card mics now render as the shared tile (slightly different look). Keep public copy non-technical — no controller/component/lock internals. -->
...
<!-- detail: 260701_unify-voice-mic-appearance. Follow-up to 260701_reusable-mic-surfaces: converged the shared MicButton visual recipe onto the main-conversation composer mic via voice tokens (--gradient-voice-idle/-recording, --color-voice-icon-idle, --radius-voice-mic, --shadow-voice-mic/-hover repointed to the composer's exact palette; MicButton AND the composer both consume them = SSOT, composer stays pixel-identical). Added an lg (46px) MicButton size; Home-hero mic restored to lg (was md/32px after the migration). Home-hero double-tap-while-recording now sends draft+transcript via submitPrompt (was single-tap only). Contrast: dark idle icon on the pastel gradient holds in BOTH themes (removed stale body.light overrides). Compact inbox/Today/question mics stay sm/md. Keep public copy non-technical — no token/component internals. -->
...
<!-- detail: 260630_safety-rule-suggestions. Previously, Rebel silently rewrote your permanent safety settings whenever it thought you'd said "yes" to something durably — firing a vague "Rule saved (v194)" toast, sometimes many times in one task. Now nothing is added to your safety rules without you clicking Add rule on a clear card in the notification drawer that quotes exactly what Rebel would remember. You can choose how broadly it applies, adjust the wording, or skip — and if you skip, Rebel just keeps asking next time. There's also an off-switch in Settings → Safety. The detector was tightened to stop over-firing on ordinary confirmations, and duplicate suggestions are coalesced. Mobile consent card is deferred; desktop and cloud ship in this release. Do NOT mention version numbers, "Safety Prompt", "v194", "chat-intent", IPC, or detector internals. -->
...
<!-- detail: 260626_operators-reliability-ux. Operators panel UX: (1) a search/filter row below the tabs filters the active tab by name/display_name/description/consult_when (instant, query persists across tab switches, Escape clears); (2) a "Copy file path" action in each card's more-menu copies the OPERATOR.md path (with a visible failure message); (3) activated (Space) Operators show a quiet "Edited <relative> by <who>" line (provenance: created_at/by + modified_at/by frontmatter stamped on activate/duplicate/rename/role-toggle; full timestamp on hover; falls back to file mtime so agent edits still show). Shipped alongside under-the-hood reliability hardening (operator-file mutation now race-safe, hardened consult-response parsing, bounded diary growth) with no user-visible surface. Do NOT mention file paths, mutation locks, frontmatter, or internals beyond what a non-technical user sees. -->
...
<!-- detail: 260701_mcp-reconnect-prompt. App-side surfacing of super-mcp's dead-grant needsReconnect marker: describeMcpConfiguration second overlay source + count-only mcpConnectorReconnectHealth check (drives a toast) + a Reconnect button in Settings > Tools for direct connectors (mcpAuthenticate force:true). super-mcp 2.7.3 clears the marker on browser re-auth too. Desktop-first; cloud/mobile a follow-on. Keep public copy generic — no marker/health-check/SDK internals. -->
...
<!-- detail: 260701_microsoft-reconnect-prompt. Google-parity for the Microsoft calendar connector: app-side MS calendar OAuth refresh failures (invalid_grant streak) now latch needsReconnect in oauthRefreshFailureStore under the Microsoft365Calendar-<email> slug (built via generateInstanceId so it string-matches the MCP instance server.name), surfaced by the existing connector-list overlay. Recovery: clear-on-reauth (microsoftHandlers, keyed on auth success outside the best-effort MCP-registration block), generic removal, config-derived fail-closed orphan sweep, or an observed refresh success/peer-adoption. A dead latch can never be cleared by mere access-token validity (throws reauth regardless of expiry). Typed user-safe MicrosoftAuthFailureError replaces a raw-provider-body throw. All five Microsoft connectors share one grant, so reconnecting via Calendar heals Mail/Files/Teams/SharePoint too; their own in-process latch is a follow-up. Desktop-first. Keep public copy generic — no slug/store/latch internals. -->
...
<!-- detail: 260701_websearch-native-provider-fallback. OpenRouter/Mindstone-managed users now get built-in WebSearch routed through OpenRouter's native web_search server tool (provider-run, citation-backed) instead of DuckDuckGo HTML scraping, which has grown increasingly unreliable amid industry-wide anti-bot hardening. Direct-Anthropic/OpenAI-key and local-model users are unaffected by this tier (kept on existing DDG/BYOK behavior), but get a better fallback experience: WebSearch's CAPTCHA/rate-limit failures now surface a deterministic, clickable "set up Brave Search or Perplexity" card via the existing suggest_connector_setup card infrastructure, instead of relying solely on the model to narrate a plain-text suggestion well. Also corrected stale Brave Search API pricing copy (its free tier ended Feb 2026) in the connector catalog and failure messages. Keep public copy generic — no DuckDuckGo, CAPTCHA, API dialect, or OpenRouter request-shape internals; do not imply DDG search is fully removed (it remains a fallback for other paths). -->
...
<!-- detail: 260705_rebel-6jq-plan-not-showing (internal ticket). When a configured Planner (thinking) model was silently gated out (paused, missing model, flex-tier, undecodable), plan mode turned off with zero user-visible signal — the ticket symptom was "plan no longer shows at all". Ships a persistent, actionable session banner (alongside the existing session-error/oauth-deprecation notices) that names what happened and offers an "Open Planner settings" button, staying up until the user acts or the Planner recovers. Keep public copy non-technical — no "planModeNotice"/status-event/turn-terminal-telemetry internals; do not name the internal ticket. -->
...
<!-- detail: 260706_library-loading-honesty + 260706_spaces-lens-loading + 260707_library-reconnect-notice-precision. On first load the Library could paint a shallow preview, then falsely declare itself empty or "unavailable" while the deeper tree walk / cloud reconnect was still in flight — plus duplicate, unnamed reconnecting banners and a vague "Manage in Settings" button. Root cause: several Library consumers checked "what's visible right now" without checking "is the tree still loading?" Fix makes loading state explicit throughout (Folders/Cards views, the Spaces lens) and names the actual folder that's reconnecting — a same-day follow-up sharpened this further: the notice now shows the folder's full path rather than just its name, uses a workspace-relative path instead of leaking an OS-absolute one, and prefixes the notice with the folder's Space, rendered as a click-to-reveal link. Keep public copy non-technical — no tree-walk/reconciliation/consumer/IPC internals. -->
...
<!-- detail: 260706_mcp-oauth-fm6-graceful-drain (FM6). Notion (and other connectors using single-use rotating refresh tokens) could be force-killed mid-token-refresh on app shutdown, leaving a dead token on disk and forcing a reconnect every 1-2 days — a distinct root cause from the earlier "Notion (and Linear) stay connected" refresh-serialization fix above. Fix drains in-flight refreshes and refuses new ones during shutdown instead of hard-killing the connector process. Keep public copy non-technical — no SIGTERM/SIGKILL/token-rotation/router internals; do not name FM6. -->
...
<!-- detail: 260706_daily-spark-warmth. Daily Spark's generator prompt (v1.2->v1.3) could drift into either dwelling on a hard week or forcing an inward pep-talk. The two banned "gentle" formats now route to substitutions that name the week honestly and then turn outward. Keep public copy non-technical — no prompt-version/judge/eval internals. -->
...
<!-- detail: 260706_sidebar-starred-above-folders. A starred conversation with no folder rendered above every folder header in the sidebar — an unintended ordering artifact from the folders feature. Fixed at the root in the shared list-building transform; affects the Active, Done, and Starred tabs uniformly. Keep public copy non-technical — no buildFolderAwareList internals. -->
...
<!-- detail: 260706_inbox-mobile-autodone-parity. The per-item "Mark done when finished" toggle persisted on desktop but only changed local screen state on mobile — so toggling it on your phone didn't stick after reopening the item. Keep public copy non-technical — no cloud-client store/updateFields internals. -->
...
<!-- detail: 260705_testing-process-and-scheduling-fixes. automationScheduling mixed the process timezone with the user's target timezone in several branches, so the cloud scheduler (runs UTC, passed the user's IANA timezone) could fire a schedule on the wrong wall-clock day/time for non-UTC users — every-N-days, hourly, weekly, and monthly schedules, plus DST boundaries. Keep public copy non-technical — no timezone-math/wall-clock-computation internals. -->
...
<!-- detail: 260705_win-stt-badalloc (internal ticket). On memory-constrained Windows machines, on-device (Parakeet/sherpa-onnx) speech-to-text could throw a raw std::bad_alloc during recognizer init — surfacing as an opaque "bad allocation" error — partly because the ~670MB model was rebuilt on every transcription instead of reused. Fix caches the recognizer, and reclassifies the failure as a clear, non-retried, actionable error. Keep public copy non-technical — no sherpa-onnx/recognizer-cache/error-category internals; do not name the internal ticket. -->
...
<!-- detail: 260704_lost-conversation-plan (bug_mode, internal ticket). The plan/mission checklist is derived on-the-fly from the completed turn's task/mission tool detail; compactTurnEvents stripped that detail to path-only on turn completion (a sub-agent carve-out existed, but none for the 5 structured snapshot tools MissionSet/TaskList/TaskCreate/TaskUpdate/TodoWrite), so the plan vanished the moment a compacted copy was surfaced — a cache-hit reopen or a same-count/same-seq active-session re-ingest (the regression guard compares count/seq, not detail fidelity). Fix: eventCompaction preserves the (already ingest-bounded) detail for those tools, path-only fallback for over-budget/malformed. Cross-family GPT-5.5 independently confirmed the root cause. Keep public copy non-technical — no compaction/tool-detail/parser/seq internals. -->
...
<!-- detail: 260703_automation-missed-cooldown (internal ticket). Scheduled cloud automations (executeIn:'cloud') could silently skip a run when cloud connectivity degraded: the desktop's health gate read a signal that never flipped during a live degradation (success-only reporting on the hot path), and the cloud scheduler had no catch-up logic on restart, so a missed slot belonged to neither side. Fix: single-owner execution (the cloud owns every cloud automation; desktop never runs one locally on a scheduled path — enforced at one chokepoint so double-runs are impossible by construction); desktop's gate now also reads live breaker state and shows a calm, auto-clearing "on hold" banner plus one debounced notification per degradation window (never per missed slot); the cloud catches up a missed slot on restart; and desktop pulls recent cloud run history on reconnect so a run that happened while the desktop was closed becomes visible instead of showing "no runs today". Removes desktop's undocumented local-fallback execution during degradation (deliberate, design-cleared). Keep public copy non-technical — no "single-owner", "circuit breaker", "cloud instance", "chokepoint", or file/function internals. -->
...
<!-- detail: 260703_library-unreadable-cloud-flap (bug_mode). fileTreeService's three-plus-one catch sites collapsed the boundary's distinct transient WorkspaceFileSystemError('CloudReconnecting') into the permanent 'unavailable' truncation reason, so a briefly-unreachable cloud mount (Google Drive stream mount, Dropbox root) rendered the same harsh "moved or removed / won't fix itself" copy as a genuinely broken shortcut. Fix: thread a distinct 'cloud-reconnecting' reason (+ cloudReconnectingNodes count) end-to-end — producer type, Zod schema, classifyTreePartial, useTruncationSignal — to a new calm/self-healing notice variant. Permanent unreadable folders keep the existing actionable copy; a size cap or a co-present permanent failure still takes precedence. Keep public copy non-technical — no reason enum, catalog, or internal names. -->
...
<!-- detail: 260702_fix-update-permission-false-positive. On macOS a benign "an update is already downloading" concurrency signal from the updater was misclassified as a permanent permission / IT-managed-install failure, firing a false "Auto-updates unavailable — ask IT" toast even though the update downloaded and installed minutes later. Fix: classify that signal as benign/retryable (no longer surfaced as an error) and skip a redundant manual update check while a download is already in progress. Beta-window timing race. Keep public copy generic — no Squirrel/RACCommand/categorizer/error-code internals. -->
...
<!-- detail: 260701_fix-rogue-managed-lost-connection / StreamingLivenessPolicy (internal ticket). Long tool-heavy turns on Rebel's managed connection could abort with a spurious "lost connection" because the OpenRouter passthrough enforced a 90s inter-chunk idle deadline while direct paths had none. A provider-agnostic StreamingLivenessPolicy now raises managed and BYO OpenRouter passthrough to a 150s/200s first-byte/first-chunk + 300s inter-chunk envelope; a genuinely dead stream still fails fast. Keep public copy generic — no transports, provider names, or timeout seconds. -->
<!-- detail: 260701_managed-tier-model-not-allowed. Some managed plans include a specific set of models; app role defaults could still resolve to one the plan doesn't cover, hitting a hard "didn't have the right model" error. Fix: at request time, a disallowed model is swapped to a plan-included one; user sees once-per-conversation inline copy "Your plan runs on <FriendlyName>. Using it for this one." then the turn proceeds. An empty model set still blocks with a blameless error. Do NOT expose internal routing/proxy terminology or ticket IDs in public copy. -->
...
<!-- detail: 260701_per-subcall-attribution. Fixes billing error-message attribution only: when a backup/personal credential is the real culprit inside a smart-picked sub-call, billing copy now names that credential/provider instead of blaming the turn's primary Mindstone plan. Cost tracking is unchanged; per-subcall cost-row attribution is a separate follow-up. Keep public copy honest and non-technical. -->
...
<!-- detail: 260702_meeting-autojoin-hardening (internal ticket). Measure-first hardening of meeting-bot auto-join after a report of "auto-join on but no bots scheduled" whose single root cause wasn't determinable from one field report (originating ticket kept OPEN pending telemetry). Shipped: (1) record botScheduled/botScheduledAt/botId at schedule time (update-only, never creates a skeletal entry) + REMOVE the sticky botScheduled term from the 'missed' reconciliation guard so a past scheduled-but-no-transcript meeting is still correctly 'missed' (was a no-op for prior behaviour); (2) PII-free per-cycle auto-schedule telemetry (counts/enum-reason-buckets only) + two Sentry known-conditions (auto_schedule_zero_success on genuine anomaly, backend_config_missing at the unchanged fail-closed throw) so a silent no-schedule is diagnosable in one shot; (3) getTodaysMeetings timezone/DST day-boundary fix (Luxon, masked latent bug); (4) widened + tightened meeting-URL extraction (zoomgov, Teams /meet short-link, MS Graph onlineMeetingUrl/body.content fallbacks; Google Meet limited to join-code shape so Meet landing/settings pages aren't mistaken for join links). Keep public copy generic — no Sentry/known-condition/telemetry internals, no ticket IDs. -->
...
<!-- detail: 260701_fix-operator-consult-token-starvation (bug_mode, internal ticket). Operator consults (rebel_operator__consult) intermittently failed with consult_failed (malformed_response), surfacing a non-blocking error to the user instead of the operator's perspective. Two causes: (A) the model returned fenced/prose-wrapped JSON that a bare JSON.parse rejected (the consult was the outlier vs watchdog/heroChoice — no outputFormat, no fence tolerance); (C) a reasoning model spent its whole small output budget on thinking, returning zero text blocks (stop_reason max_tokens). Fix, all local to operatorConsultRunner: add outputFormat json_schema + parse via extractJsonFromStructuredResponse (fence-tolerant, mirrors watchdogJudge); retry once with a larger token cap on the specific empty+max_tokens signal; enrich the malformed-branch log (stop_reason, output_tokens, content-block counts) so recurrences are diagnosable. Keep public copy generic — no parser/token/model internals. -->
...
<!-- detail: 260630_mcp-oauth-refresh-race (bug_mode). super-mcp OAuth refresh-token rotation race: Notion (~06-04) + Linear (~06-22) switched to single-use rotating refresh tokens; concurrent super-mcp processes sharing one non-atomic, unlocked token file raced → invalid_grant/"Grant not found", then refresh-only mode never cleared the dead grant so the connector wedged until manual reconnect. Fix: transactional refresh chokepoint under a per-package cross-process lock + atomic writes + disk-as-source-of-truth + fail-closed + dead-grant detection/marker. General across all rotating-refresh connectors. Keep public copy generic — no token/lock/SDK internals. -->
...
<!-- detail: 260630_fix-blocked-tool-renotify-loop. "Always block" on a tool-safety approval now writes a durable blocked action, enforced silently across conversations/surfaces and removable in Settings > Safety > What Rebel will never do. Keep public copy non-technical: do not mention settings.blockedTools, packageId/toolId, staging, broadcasts, or safety hook internals. -->
...
<!-- detail: 260629_fix-cloud-sync-feedback-loop (bug_mode). A session whose stored updatedAt was inflated above its content-derived value made cloud sync non-convergent: cloudRouter.syncSessionFromCloud always took the skip-upsert branch (comparing the stored value) and never self-healed (self-heal only ran on the accept-cloud branch), while the cloud could only ever recompute updatedAt to the content-derived value — so the pull skipped-but-still-broadcast cloud:sessions-synced ~1/s, flooding the renderer with summary reloads until V8 aborted (exitCode 5); the render-process-gone handler only logged, so the window stayed blank, and the condition persisted on disk across restarts. Fix: skip-path self-heal of the poisoned updatedAt (idle sessions compare on the content-derived value; active turns keep raw stored, no clobber) + broadcast only on a genuine renderer-visible change; close a self-echo-suppression gap on the outbox-drain push; renderer-side coalescing of sync broadcasts + a bounded auto-reload of a crashed window. Keep public copy generic — no timestamps/sync/renderer internals. -->
...
<!-- detail: cloud /api/voice/transcribe hard-rejected mobile uploads with a bare 400 whenever the synced voice.provider was an on-device provider (local-parakeet — the macOS/Windows default), which the mobile queue mapped to a permanent, silent, no-retry failure ("This recording couldn't be processed"). Fix: cloud now falls back to the user's own OpenAI/ElevenLabs key when present; otherwise it shows an honest, actionable message instead of silently dropping the recording. Also extended to meeting recordings. Keep public copy generic — no provider/route internals. -->
...
<!-- detail: Stages 2-3 of the same run — broad mobile error-message overhaul (~20 dead-end strings) to match the connectivity-banner voice (name the cause, offer a recovery, don't blame). Generic; no internals. -->
...
<!-- detail: 260624_hubspot-scope-gap-surfacing (internal ticket). Connector Stage 1: rewrote HubSpot 403 capability-denied copy (error-parser.ts buildHubSpotCapabilityDeniedError, wired into shared parser SCOPE_MISSING + crm-handlers PERMISSION_DENIED + file-handlers). Old copy told users to "reconnect and grant scopes" which can't work (scope needs HubSpot-app admin registration or account plan). New copy says reconnecting won't fix it + names the two real causes. Generic/public-safe. errorCodes preserved. The actual unblock is a dev-portal scope registration (docs-private runbook). Do NOT mention scope strings, dev-portal, or client_id in public copy. -->
...
<!-- detail: 260624_oss-byo-oauth-creds-ui. OSS-only: the 4 bundled OAuth connectors (Google, Microsoft, Slack, HubSpot) now collect the user's own OAuth client ID/secret via an in-app credential form (ExpandedConnectionCard + SetupFieldsForm), in BOTH Settings and onboarding, replacing the old "edit .env.local and restart from a terminal" path. Creds save to settings.json and resolve at connect/auth time (env → OSS settings → provider precedence); no app restart. Commercial build unaffected (managed creds, form hidden). Microsoft is client-ID-only (PKCE); one ID covers all 5 Microsoft connectors. Do NOT mention .env, resolvers, settingsKey, or internals in public copy. -->
...
<!-- detail: 260623_diagnose-anthropic-key-rejected Stage 1 (Track A). The auth-error copy in classifyErrorUx.ts and humanizeAgentError.ts now names the specific provider (e.g. "Anthropic rejected your API key." rather than "Your AI provider rejected the credentials."), explains that the *saved* key was turned down (so re-typing the same key won't help), and surfaces one clear action ("Update key" deep-linked to the provider's key field). The four credential states (rejected key / no key / disconnected / billing) each produce distinct, provider-named copy. Existing invariants preserved: Claude Max OAuth branch, Mindstone subscription branch, enforceHumanizerOwnedOverrideContract. -->
...
<!-- detail: 260623_diagnose-anthropic-key-rejected Stages 2-4 (Track B). In-memory per-credential rejection tracker (credentialRejectionTracker.ts, keyed on credentialSource + key fingerprint) trips after ~2 consecutive scheduled/catch-up auth failures for the same credential; feeds an optional rejected-credentials signal into evaluateProviderReadinessRule in automationRules.ts (kept separate from the existing missing/disconnected states); wires into both automationScheduler.ts (desktop) and cloudAutomationScheduler.ts (cloud) at the scheduler listener. On trip, the existing shared gate blocks scheduled/catch-up spawns for the affected credential and persists a provider_not_ready run. Interactive turns and "Run now" always bypass. Counter resets on any successful turn for that credential. The existing provider-readiness banner in AutomationsPanel.tsx gains three new *_auth_rejected cases with "Automations paused: Anthropic rejected your key." framing, the no-replay line, and the Update key CTA. -->
...
<!-- detail: 260623_model-routing-notes-guard. In Settings → Models, the per-profile button that runs background AI research to fill a model's routing notes was relabeled "Research this model" (was "Enrich this profile's description with AI research?") and gated to only appear when the model genuinely lacks routing notes — no per-profile notes, no legacy strengths/weaknesses, AND no built-in catalog default. So built-in catalog models (which ship curated routing notes) no longer show it; only custom/local/self-hosted models Rebel doesn't recognise do. Backed by a new CI guard ensuring every selectable built-in model ships routing notes. Do NOT mention internal terms (MODEL_CAPABILITY_DEFAULTS / planner routing / the guard) in public copy. -->
...
<!-- detail: 260623_fix-glm5-2-subagent-route-coercion. A planner-assigned route-table sub-agent for a SLASH-named model (z-ai/glm-5.2 and any OpenRouter/DeepSeek/Kimi pool model) tripped the non-passthrough-Anthropic foreign-body guard before generation: the assigned branch in agentTool.ts collapsed to the concrete profile, keeping the slash slug in the Anthropic-dialect body. Fix mirrors the unassigned path (slash-free alias body, concrete in x-routed-model). Follow-up run added: fail-closed-codex regression test + gated live-API slash route-table test + postmortem. Do NOT mention route tables / x-routed-model / dialects in public copy. -->
...
<!-- detail: 260623_provider-status-probe. When a turn fails with an AI-service/provider server error (e.g. an Anthropic 529 overload storm), the error notice now offers a "Check <Provider> status" link (Anthropic → status.claude.com, OpenAI → status.openai.com, OpenRouter → status.openrouter.ai) so the user can quickly check whether the provider is reporting wider problems. The link is a pure registry lookup (no live fetch) and is a starting point only — status pages can lag, so it may show "operational" during a real incident; it NEVER changes the verdict or claims "no outage" / "not Rebel". Separately, the diagnostics provider-reachability panel now surfaces public status-page info (incl. a live status check, diagnostics-only, surfaced only on major/critical incidents). Do NOT mention Statuspage / JSON / internal mechanics in public copy. -->
...
<!-- detail: 260623_fix-supermcp-quit-orphans. Rebel runs a background helper process to talk to your tools and integrations. If Rebel was force-quit, crashed, or restarted during an update, that helper could stay running after the app was gone — quietly consuming memory, and accumulating across restarts. Fix: (1) the helper now shuts itself down when Rebel goes away; (2) Rebel sweeps up any lingering helpers from prior sessions when it starts fresh. Implementation: super-mcp process now registers a parent-exit watchdog via SIGTERM/SIGKILL path; Rebel on startup scans and terminates orphaned helpers by PID file. Do NOT mention "super-mcp", "watchdog", "orphan", PIDs, or process internals in public copy. -->
...
<!-- detail: 260624_dead-space-surface-remove. A Space whose backing folder (or its Google Drive / Dropbox / iCloud folder) had been permanently deleted was silently dropped from your Spaces lists — invisible, so there was no way to tidy it up — yet it lingered behind the scenes and kept emitting repeating background error noise on every scan. Now such a Space is surfaced with a clear "can't find this folder" state and a Remove action; removing it unlinks the Rebel shortcut (your cloud files are untouched) and the next scan prunes the leftover entry. Distinct from the transient "Reconnecting" state, which is for folders that are briefly offline. Do NOT mention symlinks/ENOENT or other internals in public copy. -->
...
<!-- detail: 260622_fix-message-render-drop (internal ticket). A completed turn's final answer could vanish from the transcript — showing only an initial snippet — after a background cloud sync. Root cause: a destructive cloud→desktop pull overwrote the already-correct on-disk conversation with a timestamp-newer-but-content-poorer cloud snapshot of the same turn, and the renderer then re-ingested that regressed copy and wholesale-replaced the live transcript. Two layered fixes: (1) the cloud→desktop pull now routes through an additive merge when the local copy has content the cloud snapshot lacks (uses per-turn max event sequence, catching the count-stable in-place result-promotion case); (2) the renderer guard refuses any external ingest that would shrink a shared turn's content, with an observable breadcrumb on refusal. Together they make "an external ingest regresses the active transcript" unrepresentable. Do NOT mention internal ticket IDs in the public copy. -->
...
<!-- detail: 260623_render-drop-followups, mobile half. Same render-drop family as the desktop fix above, on the mobile/web cloud client. On reconnect the client runs a catch-up that enriches the live transcript, but a stale per-conversation cache or a strict-shrink snapshot could wholesale-replace it — compounded by a transient network blip — dropping a just-completed answer. Fixed by applying the monotonic-content invariant to fetchSession at BOTH the cache-hydrate site and the REST replace site (decideSessionContentRegression in cloud-client/src/stores/sessionContentRegressionGuard.ts): a replacement that would lower a shared turn's non-user message count or max event seq is refused, keeping the richer live transcript, with an observable refusal signal. Do NOT mention internal ticket IDs / class names in the public copy. -->
...
<!-- detail: 260622 Stages 3-4. When the Chief-of-Staff README (the file holding the user's personal instructions, load-bearing for every turn) can't be read at turn admission, Rebel no longer silently runs the turn on a generic template. On a user-initiated desktop turn it now pauses and shows a calm per-turn recovery notice (reuses classifyErrorUx → SessionErrorNotice) with one of three causes: reconnecting (dead/slow drive — auto-retries, then offers "Run without my instructions"), unreadable ("Try again" + "Open the file"), or missing-after-setup ("Recreate from template" + "Run without my instructions"). "Run without my instructions" is the explicit, logged escape (never a silent degrade). Cloud/mobile/background turns are unaffected (they log and continue — no drive for the user to reconnect). -->
...
<!-- detail: 260622_mobile-feedback-outbox. The same persist-before-confirm + retry durability now covers the MOBILE feedback path: help.tsx enqueues through the existing on-device offline queue (Expo FileSystem) before the receipt, drains immediately when online, and retries on reconnect/foreground otherwise; a permanent failure surfaces the honest delivery-unavailable card + Copy-report. Plus the cloud relay now dedups retried reports (client-minted Sentry event_id) and gives each distinct report its own issue (per-report fingerprint) — the desktop parity gaps, closed for mobile. Do NOT mention Sentry internals. -->
...
<!-- detail: Apple Shortcuts joins the OSS connector catalog (published at @mindstone-engineering/mcp-server-apple-shortcuts@0.1.0). Monologue lands as an official hosted OAuth MCP at api.monologue.to, so you can search and read your voice notes directly from Rebel. Bonus: kebab-case connector display names are now title-cased everywhere (so "apple-shortcuts" renders as "Apple Shortcuts"). -->

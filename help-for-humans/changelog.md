...
- **"Still setting up" is no longer mistaken for "no"** — in the first moments after an update, while Rebel moves your old safety settings into your Safety Rules, an action that slipped past the "initializing" notice could be held as if your rules had turned it down. It's now handled like any safety check that couldn't finish: the card says so instead of blaming your rules for something they never saw, and actions still waiting on it get checked again automatically.

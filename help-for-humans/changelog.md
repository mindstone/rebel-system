...
- **Wise set up from chat now works on the web and your phone too** — a Wise token you gave Rebel in chat on your computer stayed on your computer, so Wise came up empty in the web app and on your phone. It now goes where Settings puts it, so it works everywhere, and a token saved that way before moves across by itself. To disconnect Wise, remove it in Settings → Connectors. Setting Wise up from chat in the web app or on your phone isn't possible yet: do it once on your computer, and if you set it up from your phone in the last day or two, do that once more. One key, one place.
- **Save or Skip, not both** — answer the same "may I save this?" note from your phone and your computer, and only the first answer counts: the second no longer claims it happened too, a Save that lost is no longer reported as saved, and if the other answer went the opposite way Rebel tells you in one line. A note you saved on your phone also stays saved, instead of turning into "declined" once your computer catches up. And if you've recently used a newer version of Rebel, this one now says it can't save or skip notes until you update, rather than half-doing it.
- **Your cloud checks your computer's handwriting** — when you edit, reset or restore your Safety Rules on your computer, your cloud now checks the change really came from that computer before taking it, instead of trusting anything that happens to hold its access key. Rules you approve on your phone work exactly as before. If you've connected a second computer to the same cloud, Safety Rules you save there stay on that computer, and Settings → Cloud says so rather than leaving you to find out.
- **When your company's AI account has a billing issue, Rebel says so and stops asking** — if the AI account your organisation provides refuses Rebel's safety check over billing or a rejected key, Rebel now tells you exactly that and that your administrator can fix it, instead of blaming its own "AI plan" or offering a retry that can't work. It also stops asking that account for a couple of minutes rather than on every step, and an automation that had to hold actions back now says so instead of announcing it's complete. Nothing unchecked runs in the meantime.
- **Safety checks keep to their time limit** — when the model checking an action had to try twice inside one check (asked again without a setting it refused, or rerouted after a garbled answer), each try could get the full time allowance, so a check could hold your action for twice as long as it should. A check now stops at its limit however many tries happen underneath, and Rebel carries on as it does for any check that could not finish in time. Punctual, if not always decisive.
- **"Still setting up" is no longer mistaken for "no"** — in the first moments after an update, while Rebel moves your old safety settings into your Safety Rules, an action that slipped past the "initializing" notice could be held as if your rules had turned it down. It's now handled like any safety check that couldn't finish, and the card says so instead of blaming your rules for something they never saw.
- **Your connector settings file is yours alone** — if you pointed Rebel at your own connector settings file (Settings → Connectors → Change config file, say the one Cursor uses), the sign-in tokens Rebel saved there could be read by any other account on the same computer. Rebel now keeps that file, and its backups, readable only by you whenever it saves to it, and tells you once when it tightens a file that others could open before. Apps you run under your own account read it as before; if you deliberately shared that file with another account, you'll need to share it again. Some things are better kept to yourself.
- **Your Safety Rules stay in your words** — saving your rules in Settings no longer has Rebel quietly tidy them afterwards, so a rule you approved word for word keeps its words. If Rebel saves a rule from an approval card while you're editing, your save keeps it too and shows you the combined result, and when your phone is connected Settings now says plainly that saving here replaces the rules everywhere, including any you saved from your phone. Your rules, as written.
- **Approval cards say what actually happened** — if an approval disappears before your answer lands, the card now says Rebel couldn't confirm the result and what to check, instead of "out of date", "already resolved" or nothing; once one rule is saved for an approval, the card won't offer to save its opposite as well; stopping a permission on your phone over a patchy connection now says it couldn't confirm and that it's safe to try again, instead of claiming it failed; and a rule that would change too much at once gets its own explanation rather than being called too broad. Fewer mysteries.
...
- **Retired ChatGPT models: the honest fallback now covers every retired model** — 0.4.70 promised that a retired saved model would be swapped for the nearest one your plan offers, with a note in Settings; for gpt-5.2, 5.1, 5, 4.1 and their mini and nano variants that swap silently missed and the old quiet promotion happened instead. It now covers them all.
...
- **A sleeping cloud folder no longer takes the whole app down with it** — if one of your spaces lived in a Google Drive folder that quietly stopped being available offline, Rebel kept asking for files the drive could no longer fetch, and the window locked up for the best part of a minute at a time while it waited. Those reads now give up after a couple of seconds and Rebel carries on without that space until the folder is reachable again. The underlying fix is still yours to make — switch "Available offline" back on for the folder, or unlink the space — but you can now get to it without fighting a frozen app.
- **Windows: "not this one again"** — on a Windows command card you couldn't tell Rebel to stop asking about a command for the rest of the conversation, though you could everywhere else. Now you can. Worth being exact about what that does: it remembers a **refusal**, not an approval. Rebel stops asking because it stops offering to run it.

  Three gaps we'd rather name than let you find. A command carrying a credential — an API key, an access token — is still asked about every single time on Windows, however often you approve it. The commands Rebel can read well enough to verify on Windows are a short list of built-ins: ordinary things like `grep`, `ls`, or a script of your own still get a card every time, whatever your Safety Rules say. If being asked about everyday commands is the thing wearing you down, this release does not fix that. It's the next thing we're looking at.

  And the one we like least: on Windows, approving a command Rebel has held back for review can end with the same card coming back instead of the command running. Approving is supposed to run it once. When this happens, the command may not have run at all, and you're asked again. If it's something you wouldn't want happening twice, check before you approve again. We know, and the fix is under way, but what's in this release is groundwork rather than the fix itself, so it can still happen to you. The fix ships in a later release.
...
<!-- feature: cloud-replay-proof-carried | detail: the sync response already carried the complete event set and cloudOutbox validated it, but the proof was not carried through to the session write, so a restamp that reproduced the answer was refused permanently. Shipped sites: cloudOutbox.ts (evidenceEvents), sessionCatchUpReplayEvidence.ts. -->
- **A conversation that stopped saving can get itself back** - When Rebel re-checked a save and got the same answer it had before, it treated that agreement as another failure and refused the save from then on. It now recognises its own answer, so a conversation that got stuck this way starts saving again by itself rather than staying stuck until you restart.
<!-- feature: watchdog-anchored-at-egress | detail: the 5-minute awaiting_api ceiling counted from the last loop message, so local request preparation (history assembly, hydration, lease commit) was charged as provider silence; the budget now anchors at actual provider egress and planner streams stay visible to the watchdog. -->
...
<!-- feature: safety-check-fail-closed | detail: When Rebel's quick safety check cannot complete, routine actions proceed on the tool's own risk band and anything risky waits for the person; a check that runs out of time holds the action and says so; three failed quick checks in a row raise a Health check named "Quick safety checks aren't finishing" that points at Report a problem; the outage is counted and traced so the fleet number exists from this build. -->
- **When a safety check cannot finish, Rebel says so instead of guessing** - If the quick safety check runs out of time, the action is held and the card says the check ran out of time and nothing has run, on our side and not because of your request. Routine actions still go ahead on their own risk level and anything risky waits for you; if the quick check fails three times in a row, Health shows "Quick safety checks aren't finishing" with what to do.
<!-- feature: cloud-volume-cleanup-safety | detail: When a cloud machine setup fails partway and is cleaned up, a storage volume that this setup did not create is never deleted, and the failure result says so through the existing cleanup field: 'The machine was removed. Your existing storage volume was kept, because this setup did not create it — nothing on it was deleted.' Same on DigitalOcean and Hetzner. -->
- **A failed cloud setup never deletes a volume it did not create** - If setting up your cloud machine fails partway and Rebel cleans up after itself, a storage volume that was already yours is left exactly as it was, and the message says so: the machine was removed, your existing volume was kept because this setup did not create it, and nothing on it was deleted.
<!-- feature: cloud-permissions-notice-truth | detail: The cloud permissions warning says it is about the connector copies the cloud uses (phone, web, Slack, automations) and does not mean this computer is unhealthy; it withholds the repair-attempt promise when the cloud's repair marker is already set; the connector card is scoped without guaranteeing this computer is healthy; the shared eligibility check is a type predicate again so the narrowing an extraction dropped is restored. -->
- **The cloud permissions warning now says what it is about** - The warning about connector permissions in your cloud says plainly that it concerns the copies your cloud uses away from this computer (on your phone, the web, Slack, or in an automation), not this computer, and it no longer promises a repair it cannot make.
<!-- feature: cloud-recovery-detail-visible | detail: The cloud continuity diagnostic is readable in Settings > Cloud > Advanced troubleshooting instead of only in the drawer, without reaching the warning surface; the contract test no longer asserts the drawer is the only place. -->
- **The cloud recovery message is readable in Advanced troubleshooting** - When your cloud is recovering, the message explaining it can be read under Advanced troubleshooting in the Cloud settings, not only in the drawer where it used to appear.
<!-- feature: whats-new-onboarding-guard | detail: The post-update What's New surface no longer opens over onboarding on a fresh install or after a same-version relaunch; it defers until onboarding is done and the version actually changed. -->
- **What's New waits its turn** - On a fresh install, or when you reopen the same version, the What's New window no longer pops up over the first screens; it appears only after an actual update, once you are past setup.
<!-- feature: blocked-save-notices | detail: When a conversation save is declined, one warning is shown per problem and re-raised only when the problem key changes; the composer stays editable, selectable and copyable while declined; sending by typing, voice or dropped file is blocked when the save is known to fail; the same on desktop, mobile and web companion. -->
- **One warning when a conversation stops saving, and you keep your words** - If a conversation stopped saving, Rebel used to warn you over and over, once for every attempt, and lock the message box while doing it, so you could not even copy your own words out. Now you get one warning per problem ("This conversation isn't saving", with the advice to copy the conversation before you leave), it comes back only if the problem itself changes, and the message box stays usable: you can type, select and copy as normal. What Rebel will not do is send a message it knows it cannot save, by typing, by voice or by dropping in a file, so nothing you send quietly disappears later. Same on desktop, mobile and the web companion.
<!-- feature: meeting-autojoin-detection | detail: Which meeting to join is decided from the calendar anchored on time, with the Desktop SDK's meeting window treated as a hint; a refused join link is retried with the invite's link; late joins are allowed up to scheduled end plus grace; when two overlapping meetings cannot be told apart Rebel deliberately stays out (fail-closed conflict policy). -->
- **Rebel now joins the meeting you're actually in** - It used to decide from the meeting window, which sometimes named a room from an unrelated booking days away, and it joined that one silently. It now decides from your calendar and the time, retries with your invite's link if the first is refused, and comes even if you join late. When it genuinely cannot tell which of two overlapping meetings you're in, it stays out rather than guess.
<!-- feature: connector-tool-toggle-visible-refusal | detail: When a connector tool cannot be switched on, Settings says which of five things is true beside that tool instead of silently snapping the switch back: disabled by the organization's administrator; Rebel still checking with the organization (the switch is the retry); set up through a config file; not in Rebel's catalogue; or the organization could not confirm it. The class comes from the policy producer; an unrecognised class falls to a neutral sentence that promises no retry. -->
- **A tool switch that would not stay on now tells you why** - Before, switching a connector tool on in Settings could silently snap back off with no explanation. Now the switch stays off and Settings says why, beside that tool: your organization's administrator disabled it, Rebel is still checking with your organization (flip it again in a moment), it was set up through a config file, it is not in Rebel's catalogue, or your organization could not confirm it. Nothing tells you to try again unless trying again can help.
<!-- feature: mcp-connecting-discovery | detail: list_tools, get_tool_details and search_tools on a package whose catalog is still connecting no longer throw -33004 to the model as a failed tool call; they wait once (2 s, abortable), retry once, and a second connecting answer becomes a non-error result that says the package is still connecting. Automations stop counting it as a failed step. -->
- **Asking a connector that is still starting up is no longer a failure** - In the first moments after Rebel connects a package, asking it what it can do could fail outright and, inside an automation, count as a failed step. Rebel now waits a moment and asks again, and if the package is still starting up it says so instead of reporting an error.
...
- **Folders you reach through a shortcut now reach your cloud copy and your phone** — Conversations and files kept in a folder you reach through a shortcut (Google Drive, Dropbox, OneDrive) were silently skipped by the cloud copy; they now sync, every kind of session syncs rather than only active conversations, and Rebel's own internal scratch sessions stay out. With Cloud Continuity off, everything stays on this device. With it on, your conversations and files are copied to your cloud, your own machine if you run one, Mindstone's if you are on a Mindstone-run plan; Rebel tells you which and does not guess. The phone shows what the cloud holds.
- **The first sync after this update shows as running** — It can be large and slow by nature, and it no longer looks finished while still going. The sync indicator now names which conversations did not reach the cloud and why, instead of an opaque count.
...
- **Questions Rebel asks you now survive a restart** — When Rebel stops to ask you something, the question is saved with the conversation instead of held in memory. A crash, a restart or a long lunch no longer makes it disappear, and Rebel no longer carries on as if you had answered. A helper working inside a bigger job now pauses the whole job until you reply, rather than finishing on a guess. One honest caveat: an older version of Rebel does not understand saved questions and drops them the next time the conversation is saved, so if you go back a version while a question is waiting, that question is lost and Rebel may resume as though it were answered. Answer or dismiss any open questions before you downgrade.
...
<!-- detail: After you answer a question, Rebel now reviews older saved actions it was already holding and withdraws only those it can confirm are no longer needed. Needed or uncertain actions stay pending, prepared connector and file work stays untouched, and withdrawn actions are never run and receive one grouped receipt wherever they appear. -->
- **Approval cards Rebel no longer needs stop hanging around** — Sometimes Rebel asks before an action, finds a better route while finishing the work, and leaves the old question waiting as though it still matters. It now tidies up only the requests it explicitly decided not to use, and tells you: **“Rebel decided not to use this. It was never run.”** Anything it still needs stays put for your decision; prepared actions and files stay put too. Fewer ghost chores, no guessing with your permission.
...
<!-- detail: After a Quick Capture recording reaches transcription, its recoverable WAV follows the configured shared Voice provider route. The route supports bounded parallel transcription with visible progress; failure retains the WAV and offers Show file, and an on-device failure never silently switches to a managed cloud key. -->
- **Quick Capture's transcription path now follows your Voice setting — and keeps the recording when transcription fails** — After a recording reaches transcription, Quick Capture uses the Voice provider configured in Rebel. Long recordings can be handled several pieces at a time with visible progress. If transcription fails, Rebel keeps the WAV and offers **Show file** instead of disposing of the evidence; if you chose on-device transcription, it reports that failure rather than quietly sending the audio through a managed cloud key.
...
<!-- detail: Rebel used to refuse certain places inside a folder you had already allowed — the kind of places that usually hold keys and passwords — every single time, with no way to say yes. Now the check happens once, while you are deciding: if there is anything of that kind in the folder, Rebel names it there and then, and allowing the folder means allowing it from then on. Folders you set up before this change keep working exactly as they were. -->
...
- **“Any time” now follows the safety level you can see** — Tools that spend money always ask the first time; choose **Any time** and Rebel stops asking, while still checking every later request for obvious mistakes. Tools that move money directly still ask each time. One visible system deciding, at last.
...
- **Calendar and scheduling tools now use the same four safety levels as everything else** — At either of the first two levels, a change can run without a separate approval. **Rebel can act for you** asks once and keeps checking every request; **Rebel always asks about this one** asks every time, unless you allow it for the current conversation. Review or change a tool's level in **Settings → Safety**.

<!-- detail: Older saved permissions that can still be matched safely to the current connector and rating are updated automatically. Permissions that cannot be matched are retired and offered again only when you next use that tool, so dormant tools stay quiet. -->
...
<!-- detail: Fireworks connections supplied by your organization now appear in Settings as company-managed and cannot be edited locally. The connection works on desktop, cloud and mobile without exposing its access key in synced settings. -->
...
<!-- detail: Usage through an organization-provided model no longer appears as a false $0 or as money you personally spent. Rebel labels it as organization-paid and estimates the amount from the provider's current public list price; when no reliable price is available, it says Cost unknown. -->
- **Company-provided model costs are now honest, not invisible** — Costs for models your organization provides (like a company-configured Fireworks connection) used to quietly show up as free — which was never actually true, Rebel just wasn't saying so. It now shows those costs, clearly labeled as your organization's rather than yours, estimated at the provider's public list prices. And when Rebel genuinely doesn't know the price yet, it now says **Cost unknown** instead of guessing zero. No more free lunches that were never free.

<!-- detail: Pages that assemble their visible words after loading could look empty to Rebel even though the text was present in the page's own data. Rebel now recovers that text where possible and clearly refuses to cite the page when there is genuinely nothing readable. -->
- **Rebel can now read pages that build themselves in the browser** — A lot of the web ships an empty page and assembles the words once it loads: shared chat transcripts, app-like pages, single-page sites. Rebel used to fetch one of those, find nothing but the page title, and cheerfully report back as though it had read the thing. Now it looks in the page's own data for the text you would see on screen, and usually finds it — labelled as recovered, since the order can differ from what's on screen. When it genuinely can't read a page, it says so and points at something that would work (open it in your browser, or use a page-reading connector) instead of quietly handing you an empty page it never read. Works the same for Japanese, Chinese and Thai, where words don't come with spaces between them. Fewer confident reports about pages nobody read.

<!-- detail: Retry countdowns now stay tied to the moment the service first asked Rebel to wait, rather than restarting whenever you reopen a conversation. The main warning shows the current remaining wait, while the original message in the conversation stays as a historical record. -->
- **Reset times that stay honest when you come back later** — When a provider makes Rebel wait ("try again in about 5 minutes"), reopening that conversation an hour later used to still say "5 minutes" — the clock had frozen at the moment things went wrong. Now Rebel remembers *when* the wait started and counts from there, so a reopened conversation shows the real remaining wait — or, once the window's passed, drops the stale countdown for an honest "try again later" instead of a number that's no longer true. Older conversations get the same treatment. A wait you can actually trust.

<!-- detail: Messages for usage limits, billing problems, lost conversation context and blocked requests now lead with what happened and the next useful step, without claiming more than Rebel knows. When a service provides a reset time for a longer wait, Rebel shows the clock time; shorter or unknown waits stay honestly approximate. -->
...
<!-- detail: A background conversation could briefly look idle while it was still working, causing a waiting message to knock repeatedly and leave noisy errors. The thinking indicator now stays accurate and the message waits for a real opening before trying once. -->
- **Waiting messages have stopped knocking every second** — While Rebel was working in a background conversation, its **thinking** indicator could flicker off and a waiting follow-up would repeatedly try the door, get turned away, and leave a noisy error behind. The indicator now holds steady instead of flickering, and the message waits until the conversation is actually ready before going through once. Less knocking; same message.

- **Scheduled automations keep their schedule when your laptop does not** — Once Cloud continuity takes over, your scheduled automations run on your cloud instance by default, so they fire at 3am whether or not your laptop is open. The **Runs on** control disappears when that happens, because there is no longer a choice to make — the Automations page tells you it has. Automations triggered by something on your computer still run there, since that is where the thing that triggered them happened.
- **Finished automations stop asking to be saved again** — An automation could complete, then set off **“This conversation isn't saving”** and make a conversation you had marked **Done** reappear. Rebel now keeps the finished result intact instead of disputing its own paperwork. Done means done; the paperwork agrees.
- **A slow start no longer leaves a conversation stuck “not saving”** — If Rebel took too long to get started, its timeout could finish the turn but leave the conversation's paperwork disagreeing with itself. It now keeps the original finish intact, so saving carries on normally. Slow start, clean recovery.
- **A background pause no longer looks like a stalled turn** — If your computer paused Rebel while it was in the background, starting a turn could fail as though Rebel itself had stalled. Rebel now counts only the time it was actually able to work, while keeping a firm upper limit; if too many other conversations are already running, it says it is busy rather than waiting forever. Naps are no longer billed as labour.
- **Model changes now end in a useful next step, not an “impossible state”** — If a conversation still points at a model from a connection you no longer have, Rebel now drops that stale choice and uses your current default. If the chosen model genuinely cannot run through the current provider, Rebel says so and offers a way out instead. Less existential drama from a settings mismatch.
- **Updates now finish the journey** — Every update prompt keeps **Not now** available; on Windows, a later quit installs only after you accepted the update, and waits if a recording or live meeting is still underway. Rebel remembers a downloaded update across restarts, reminds you no more than every 12 hours, and keeps a banner up after three days. The several-versions-behind warning works again, checks continue when another window has focus, and two failed **Update now** attempts in a day—or a read-only installation—now stay visible with a download or IT route. A successful check clears the warning — updates, now with follow-through.
...
- **Conversations that vanished from one screen come back properly** — If Rebel's cloud copy was cleaned up while your desktop still had the conversation, it could keep trying the same failed delivery forever. It now rebuilds the cloud copy and carries on; deleted conversations stay deleted, naturally. Housekeeping with an exit strategy.
- **When Rebel can't verify an old saved answer, you get to choose** — A rare old bug could leave a conversation paused because Rebel couldn't prove which saved answer to trust. Rebel can now rebuild the answer from that conversation's history and show it alongside the saved version, one at a time, so you choose what to keep. Saving stays paused until you've reviewed each one. No surprise editorial decisions.
...
<!-- detail: An Action now keeps your “mark done when finished” instruction through approvals, questions, retries and recovery. It shows In progress while Rebel owns the work, returns to Needs you when your input is required, and moves to Done only after the work has actually finished. -->
...
<!-- detail: Rebel already hid the invisible source markers attached to web references, but markers attached to your own files could still appear as stray symbols. File and document references now receive the same cleanup without removing ordinary text. -->
...
<!-- detail: Reconnect links in Automations and several notices could open Settings and immediately close it again, leaving the button looking dead. Those links now keep Settings open, and the Automations links take you directly to the connection that needs attention. -->
- **The "Reconnect" button in Automations does something again** — When Rebel warned that your ChatGPT connection had dropped and your automations were waiting on it, the **Reconnect** button did… nothing. You clicked; you stared; it stared back. It now takes you straight to the right spot in Settings to reconnect — and the same quiet dead-button gremlin is fixed for a handful of other places (a few notifications and notices) that had caught the same cold. A button worth pressing.

<!-- detail: ChatGPT Pro usage-limit replies were sometimes mistaken for ordinary rate limits, producing the wrong warning on every turn and omitting the reset timing. Rebel now recognises those replies consistently, calls a plan limit a plan limit, and shows the timing when the service provides it. -->
- **ChatGPT Pro stops crying "rate limit" when it isn't one** — If you connect your own ChatGPT Pro (Codex) subscription, Rebel could get stuck showing **Rate limit reached.** on every message — even when your subscription was working fine everywhere else. The real trouble was on Rebel's side: it wasn't recognising OpenAI's "you've hit your plan's usage limit" replies for what they are, so it slapped the wrong label on them and gave you no idea when things would reset. Now Rebel reads those replies correctly, tells you honestly when it's a plan limit (and roughly when it lifts), and stops mislabelling a passing hiccup as a hard wall. The right diagnosis, for once.

<!-- detail: People on a managed plan now get the same honest wording when a service does not say when a limit will clear. Rebel also retries brief request timeouts and conflicts again, so those momentary network hiccups do not end the turn. -->
- **The honest rate-limit copy now reaches everyone — and a blip no longer ends a turn** — A quick follow-up to the recent rate-limit honesty fix. The truthful "we don't know when this clears" wording now shows for everyone, including people on a managed plan who could still see the old "try again in a bit" line. And if the AI service has a brief hiccup mid-task, Rebel quietly tries again instead of stopping — a momentary stumble shouldn't cost you the whole answer. Loose ends, tied.

## v0.4.62 — Aug 17-19, 2026

### Improvements

<!-- detail: The Workshop is now a regular appearance option rather than an experimental setting; Conversation remains the default, and each conversation can follow or override that choice. Its stations, helper layout, detail panel and compact arrangements now make busy work easier to follow, with light, dark, reduced-motion and accessible presentation covered. -->
- **The Workshop has come out of hiding** — It now lives in **Account & Preferences → Appearance**, where you can choose the default view while Rebel works. **Conversation** is still the default, and each conversation can follow it or choose its own view. Inside the room, Rebel now names far more of what it is doing instead of funnelling assorted work through one anonymous portal; labels stop trying to become short novels; helpers can be clicked for a proper panel that stays inside the Workshop; and a busy team arranges itself without forming a small pile. Same work, considerably fewer workplace incidents.

<!-- detail: For a queued connector action, approving now runs the exact action you reviewed and denying removes it rather than leaving it waiting. If Rebel cannot prove what happened after an action may have started, it reports the outcome as unknown and tells you what to check; broader command approvals are not included in this change. -->
- **Approvals have stopped treating “yes” as a creative-writing prompt** — For a queued connector action, approving now runs the action you actually reviewed — same details, no “approved, please try that again” round trip — while denying throws the waiting action away instead of leaving it parked for a comeback. If Rebel can't prove what happened after something may have started, it says the outcome is unknown and tells you what to check, rather than awarding itself a success on vibes. There is also a short new page, [The Approval Contract](rebel://library/rebel-system%2Fhelp-for-humans%2Fapproval-contract.md), explaining what **Allow once**, remembered choices and the occasional extra “Are you sure?” really mean. Five paragraphs. Meetings have had longer agendas.
...
<!-- detail: Repeated attempts to save the same note outside your Spaces now share one approval card, with the latest version replacing the older request. The card names the destination clearly; Rebel still asks again on a future save, and any cards already piled up before this update can be dismissed normally. -->
- **Rebel stops asking the same question over and over** — If Rebel kept trying to save the same note somewhere outside your Spaces — a note it files again and again, like a running set of figures headed for a folder on your drive — you could end up with a stack of near-identical "can I save this?" cards for one file. Now they collapse into a single, current request: ask once, and if the note has changed since, the card just updates to the latest. The card is clearer too — it shows the actual folder it's headed for instead of a bare "Outside workspace / Unknown", and says plainly that because the folder isn't one of your Spaces, Rebel will check with you each time before saving there. One note, one question.

<!-- detail: Messages for usage limits, billing problems, lost conversation context and blocked requests now lead with what happened and the next useful step, without claiming more than Rebel knows. When a service provides a reset time for a longer wait, Rebel shows the clock time; shorter or unknown waits stay honestly approximate. -->
- **Error messages that tell you what happened, whether your work's safe, and what to do** — When something goes wrong mid-task — a rate limit, a billing snag, a dropped connection — Rebel's messages used to be charming but cryptic ("the provider asked for a breather"). They now lead with the plain facts: what happened, that your message is still here, and the next step — naming the provider where we can, and telling you *when* a rate limit resets ("try again after 8:20pm") when the service says so. The wit still shows up, just after the useful part. A breather you can actually time.

<!-- detail: A save approval can now cover one file or its folder, either once, for this conversation, or always. Always is available only for a private folder you confirm is yours alone; sensitive-content checks still run on every save, deletion is never included, and cloud or mobile saves continue to use one-time approval. -->
- **Rebel can now save to a folder outside your Spaces — with your standing permission** — Say Rebel keeps a running set of figures headed for a folder on your own drive, outside your Spaces. Until now it asked every single time. Now, when it asks, you can answer once, *for this conversation*, or *always* — and choose whether that covers just the one file or the whole folder. "Always" is offered only for a private folder that's yours alone, and Rebel asks you to confirm that in as many words before it takes you up on it. Two things stay true no matter what you pick: Rebel still checks every save for sensitive content before it writes, and it can create or change files but never delete them. You can see and remove any of these permissions any time under **Settings → Safety → Locations**, where reading and saving are now shown as separate, separately-revocable permissions on one tidy list. Standing permission, on your terms, revocable on a whim.

<!-- detail: The Zendesk connector is available again as a desktop Beta, covering ticket management, replies, notes, macros, views and Help Center search. Setup requires an API token created by a Zendesk admin; cloud and mobile support are not included yet. -->
- **Zendesk is back — in Beta** — You can connect Zendesk again and put your support queue in Rebel's hands: search and manage tickets, add public replies or internal notes, apply macros, run views, and search your Help Center. Two honest caveats. It's **Beta**, so if something doesn't work, please tell us via **Feedback** — a quick report genuinely helps. And setting it up needs a **Zendesk admin**: only admins can create the API token it uses, so if you're not one, you'll want to grab a colleague who is. Desktop-only for now. Support queue, meet Rebel.

<!-- detail: Ordinary background syncing could make Rebel think a finished answer was being overwritten, so healthy conversations showed a false protection warning. The notice now appears only for a genuine conflicting change, uses calmer wording, and clears old false alarms when those conversations are loaded. -->
- **The "protected answer" warning stops crying wolf** — A little alarm — **This answer is protected** — had started popping up constantly, with no hint of what it meant or what you were meant to do about it. It was a false alarm: Rebel was mistaking a bit of ordinary background syncing for someone trying to rewrite your finished answers. Rebel now tells the difference, so the notice only appears on the rare occasion something genuinely tried to change a saved answer — and when it does, it's a calm note (**Rebel kept your saved answer. Nothing for you to do.**) rather than a warning triangle. Conversations that were already showing it sort themselves out on their own. The wolf, uninvited.

<!-- detail: Interrupted interactive turns now keep a bounded same-process checkpoint at the last complete provider-history boundary. A short typed connection loss gets one same-route retry; otherwise desktop/cloud/mobile offer or display continuation from saved work. Completed and unresolved possibly-side-effecting actions are fenced from repetition. Short ChatGPT Pro usage-limit resets show a waiting state and use the same admission path when the wait ends. Process restart, account/route change, or expired state fails closed to Start again. -->
- **Connection drops no longer make Rebel forget the work it just did** — If a turn loses its connection partway through, Rebel now picks up from its saved work instead of starting the whole task over. Actions it already sent stay sent rather than getting an encore. Short usage-limit resets can simply wait and continue on their own; if the saved continuation is no longer safe to use, Rebel says so and offers **Start again**. Recovery, with less theatre.
...
<!-- detail: When the current model cannot see an image, Rebel no longer follows advice to reopen the same file repeatedly. It reads the image once, asks an image-capable model when possible, and otherwise explains the limitation instead of looping. -->
- **Rebel stops going in circles over an image it can't see** — When you're on a model that can't view images, Rebel used to occasionally get stuck re-opening the same picture over and over, narrating its own loop. Now it looks once, quietly asks a model that *can* see to describe it, and if that isn't possible it just says so — instead of pacing the same three steps forever. Fewer circles, more answers.

<!-- detail: When a connected view needs permission, the message now names the connection and the action it wants to take, such as Slack sending a message. It points to the inline Allow for this conversation button instead of sending you to a Settings page that cannot grant it. -->
- **Permission prompts that tell you who wants what — and where to say yes** — When a connected view needed your OK to do something (send a Slack message, say), Rebel used to flash a confusing note that sent you to a Settings page with no button to press. It now says plainly what wants to happen — "Slack wants permission to send messages in this conversation" — and puts an **Allow for this conversation** button right where you're looking. Consent you can actually give, in the place you're actually standing.

<!-- detail: While Rebel is working, Send passes your message and attachments into the current task, while Stop ends it; the old queue tray, Remove action, Stop & send option and special send shortcut are gone. Unsent text returns after a restart, but attached files must be added again, and a message already waiting cannot be taken back once Rebel picks it up. -->
- **Sending while Rebel is working got a lot less like filling in a form** — There used to be three ways to send a message mid-task — queue it, nudge it, or stop-and-send it — plus a modifier key, a confirmation, and a small toast explaining the difference. Now there are two: **Send** reaches the task Rebel is already running, files and all, and **Stop** stops it. That's the whole grammar. If a message can't reach the run in time it becomes your next message instead, and says so; if Rebel picked it up but couldn't confirm in time, it says that too rather than guessing. Messages waiting on a conversation you're not looking at show a small clock on it in the sidebar, so nothing is parked out of sight. Two things genuinely went away: the tray above the composer, along with its per-message **Remove** — a message that's already waiting can no longer be taken back — and **Stop & send**, which is now just Stop, then Send. Spoken interrupts got more decisive as well: when Rebel is confident you meant "stop and do this instead", it does that, without asking you to confirm first. One honest caveat: files attached to a message that hasn't gone out yet are held only while the app is running — quit and reopen and Rebel restores your words but asks you to reattach the files, rather than pretending it still has them. Two buttons, no homework.

<!-- detail: When Rebel gives up on a meeting bot that went quiet, it records the outcome once in meeting history and, where system notifications are available, tells you which meeting was affected. The message says whether captions were kept, notes were already saved or awaiting review, saving failed, or nothing was captured; wider recording-service outages keep their own warning. -->
- **Rebel now tells you when it had to give up on a meeting bot** — A meeting bot that goes quiet used to disappear without a word: the meeting would simply never get notes, and the first you'd hear of it was never. Rebel now says so, once, at the moment it gives up — what meeting it was, and what happened to your notes: whether it kept the live captions it had (click the notification and it takes you to the file), whether the notes were already safe, or whether nothing was captured this time. The quiet end, announced.

<!-- detail: Computers with less than 12 GiB of memory now receive a dismissible Home offer for Efficiency Mode, and Rebel never turns it on without your agreement. Search remains available while it is enabled, though new files can take longer to appear. -->
- **Rebel now offers a lighter mode on computers with less memory** — **Efficiency Mode** can trim memory use and ease off background work, but Rebel asks first. Search keeps working; newly added files may take a little longer to show up while it is on. Say “Not now,” accept the offer, or open **Settings → Account & Preferences**. Your computer, your call.

### Fixes

<!-- detail: The Efficiency Mode search result and Home offer used to open Account & Preferences without showing the promised setting. Efficiency Mode now appears there, and the Smart model picking search result now opens its actual home in Advanced settings. -->
- **The Efficiency Mode search result now leads somewhere** — Searching Settings for **Efficiency Mode** (or pressing the Home offer's Settings button) used to deliver you to a page where the toggle simply wasn't. It now lives where the search takes you — **Settings → Account & Preferences** — and the **Smart model picking** toggle's search result has been walked to its actual home too. A destination worth arriving at.

<!-- detail: Reconnect links in Automations and several notices could open Settings and immediately close it again, leaving the button looking dead. Those links now keep Settings open, and the Automations links take you directly to the connection that needs attention. -->
- **The "Reconnect" button in Automations does something again** — When Rebel warned that your ChatGPT connection had dropped and your automations were waiting on it, the **Reconnect** button did… nothing. You clicked; you stared; it stared back. It now takes you straight to the right spot in Settings to reconnect — and the same quiet dead-button gremlin is fixed for a handful of other places (a few notifications and notices) that had caught the same cold. A button worth pressing.
- **Conversation titles now show exactly as typed** — Underscores, asterisks and all. Rebel was quietly “tidying” them away; it has stopped.
<!-- detail: Rebel was losing useful information about some failures before it displayed or recorded them, leaving a vague error with no reliable provider name. More failures now keep their original category and provider, and ChatGPT Pro plan limits are described as plan limits rather than generic rate limits. -->
- **Fewer baffling "something went wrong" errors** — When something failed, Rebel too often fell back to a vague, unlabelled error with no sense of which provider or what kind of problem — by a wide margin the most common error people saw. That turned out to be Rebel throwing away its own diagnosis of the problem partway through, then showing you the shrug. Rebel now holds on to what it actually knew: more errors arrive correctly labelled (with the right provider), and a ChatGPT Pro plan limit reads as a plan limit rather than a generic "rate limit." Less shrugging, more telling you what happened.

<!-- detail: Rebel now uses OpenRouter's real reset time when it is available and stops promising that a sustained usage limit will clear within minutes. When no timing is provided, the message says so, keeps the option to switch providers, and does not blame a connection you never set up. -->
- **Rate-limit messages that tell you the truth** — When your AI provider hits a usage limit, Rebel used to shrug and say it "usually resets within a few minutes — try again shortly." For heavy use that was often just wrong: the limit could take a good while to clear, so "try again shortly" sent you in circles. Rebel now uses the real reset time when the provider shares one — and when it doesn't, it says so honestly instead of promising a quick fix, and points you to switching providers if you have another connected. The little "short pause" note no longer invents a countdown it can't keep, and it won't blame a service you never connected. Fewer white lies, more useful answers.
...
<!-- detail: The Active conversation filter could stop responding after you chose another filter, leaving you stuck there even after a restart. Active now works again with a mouse or keyboard while keeping its conversation-count hint. -->
...
<!-- detail: Planner could fail before a ChatGPT Pro conversation began, then show the name of a model you had not selected and keep returning after you switched. The Planner now starts correctly; genuine compatibility errors name the model involved, and Switch model repairs the choice for the conversation you are in. -->
- **Planner works on ChatGPT Pro again — and the error that wouldn't go away is gone** — If you power Rebel with **ChatGPT Pro** and had a **Planner** model set in [Settings → Agent & Voice](rebel://settings/agents), some conversations dead-ended on *"ChatGPT Pro doesn't run GPT-5.5 Pro"* — naming a model you may never have picked, and shrugging off every fix: change the model, restart, change it again, same message. Planning now runs properly on ChatGPT Pro. When a model genuinely can't run there, Rebel names the one that actually failed instead of always blaming the same innocent bystander, and **Switch model** now unsticks the conversation you're in rather than quietly repairing a setting that conversation wasn't using. Picking a Claude model while on ChatGPT Pro gets the same clear, fixable message. Wrong name, no exit — both fixed.

<!-- detail: If a meeting bot goes quiet for long enough that Rebel must give up, the live captions already captured are now saved and shown in meeting history. The file is labelled as captions rather than a polished transcript, because the complete version will not arrive once the bot is gone. -->
- **Rebel no longer throws away your meeting notes when a meeting bot goes quiet** — If the meeting bot lost touch with a meeting long enough that Rebel had to give up on it, the notes it had already captured — live captions, taken in the moment — used to vanish with it. They existed; nobody could see them. Rebel now saves them as the transcript for that meeting before closing the books, and says plainly in the file that they're live captions rather than the full polished version (that one was never going to arrive). Half the notes, kept.

<!-- detail: Saving warnings now stay with the affected conversation and distinguish a temporary delay, an internal block and a genuine storage problem. You can dismiss the larger warning and copy the text still on screen, while a small Not saving badge remains until that conversation saves again. -->
- **“Conversation not saved” now tells you what actually happened** — A saving problem used to blame storage whether storage was involved or not, then keep coming back with no useful way out. Rebel now distinguishes a slow save from an internal problem and a genuinely full or restricted disk, keeps the warning attached to the conversation it belongs to, and lets you copy the text that's still on screen. A small **Not saving** badge stays until saving recovers; the alarming toast does not have to.

## v0.4.61 — Aug 11-17, 2026

### Improvements

<!-- detail: Cloud-backed Spaces now show when Rebel last checked and offer Check for new files in the Library and Settings. A manual check refreshes the Library, but the normal background schedule is unchanged; “checked” means Rebel made the attempt, not that every cloud folder was reachable or completely up to date. -->
- **Rebel now tells you when it last looked, and looks again when you ask** — Nothing tells Rebel when someone else changes a file in a cloud folder — Google Drive, Dropbox, OneDrive, iCloud, Box — so it has to go and look, and a file added elsewhere can take a while to turn up. Your Library and **Settings → Spaces** now show when Rebel last looked, with a **Check for new files** button that makes it look right now: handy mid-session when a colleague drops in a new skill or note and you need Rebel to notice it. It reports what actually happened — how many new or changed files it found, or that there's nothing new yet and your cloud folder may still be downloading, or plainly that it couldn't check. What it will never tell you is that you're up to date, because it can't honestly know: if a folder wasn't answering, it says that instead. Workspaces with no cloud folders don't get the line — nothing to be stale about. Looked, and willing to say so.
<!-- detail: When Rebel cannot automatically join a meeting, the meeting now shows a warning and points to Meeting settings so you can check the setup. The warning returns while joining still fails and clears as soon as Rebel joins successfully; meetings that were not eligible for automatic joining stay quiet. -->
...
<!-- detail: While Rebel works, the progress card can show a time range drawn from similar tasks earlier in the same conversation — "Similar tasks: 2–5 min" — or a rougher hint like "Usually quick" when there's less to go on. It only appears when there are enough comparable tasks to be worth saying, and it's a guide from your own history, not a promise: if the task runs past the range, Rebel says it's taking longer than similar tasks and keeps working. -->
- **Rebel now shows how long similar tasks have taken** — While it works, the progress card can offer a range drawn from comparable tasks in the conversation — "Similar tasks: 2–5 min" — or just "Usually quick" when the evidence is thinner. It's a rough guide from your own history, not a promise; when a task runs long, Rebel says so plainly and keeps working. Past performance, honestly disclosed.

<!-- detail: Usage analytics and automatic crash reports now have separate switches in Settings → Privacy & Safety → Privacy & Data. Each switch takes effect immediately on the current device and follows other devices through Cloud Continuity when they next connect. Turning crash reports off also removes reports waiting to send; manual bug reports still work, and an undelivered conversation rating is labelled "Rating saved, not sent." The new data-sharing guide explains which services these switches do and do not cover. -->
- **You can now turn Rebel's telemetry off — and read exactly what that does and doesn't cover** — [Settings → Privacy & Safety](rebel://settings/safety) has two new switches: **Share usage analytics** (which features you use, plus the occasional in-app survey) and **Share crash & error reports** (the automatic reports when something breaks). Both start on, including if you've been using Rebel for a while — nothing changed under you. Flip either and this device stops immediately; if you use Cloud Continuity, your other devices follow when they next connect — it's what carries the choice between them, so without it each device keeps its own answer. Turning crash reports off doesn't stop you filing a bug yourself, and if a conversation rating can't reach us Rebel now says "Rating saved, not sent" rather than pretending. Being straight about the rest: these switches don't cover app updates, signing in, the AI provider you chose, connectors you've enabled, meeting transcription, or cloud sync — so there's a new page, [What data can leave Rebel](rebel://library/rebel-system%2Fhelp-for-humans%2Fwhat-data-can-leave-rebel.md), listing every one of them, including the fine print we'd rather you heard from us. Consent, with the receipts.
<!-- detail: Rebel can now repair and file a captured note automatically only when the corrected note would already qualify for saving without another approval. Private Mode, cautious Spaces, changed content, conflicts, timeouts or any uncertainty still produce the review card, and if Rebel finishes while you act the message says the note was already filed. -->
- **Rebel now fixes small mistakes in captured notes instead of handing them back** — When a captured note came out slightly wrong — a full timestamp where a plain date belonged — Rebel used to set it aside and ask you what to do with it. Now, when it can correct the note itself *and* that note would have been saved without asking anyway, it corrects it and files it where it was headed. Notes it can't fix, or isn't certain it's allowed to file, still come to you on a card with the note safely kept. One less question nobody wanted.
...
<!-- detail: Rebel still checks empty, too-large and otherwise unsupported files on each pass so it can notice a change, but it no longer repeats the same indexing attempt until the file's size or modification time changes. Temporary drive and service problems are never remembered as permanent; healthy unchanged cloud folders are scanned periodically instead of being fully reread every few minutes. -->
- **Rebel stops retrying files it still cannot index** — Rebel looks over your files in the background to keep search current. It still checks an empty, too-large or otherwise unsupported file on every pass so it can notice a change, but it no longer repeats the same failed indexing attempt until the file's size or modification time changes. Cloud folders still get a reachability check just as often and are reread immediately when one reconnects or you add a folder; a healthy unchanged folder is scanned periodically instead of being fully reread every five minutes. A file edited on another device can therefore take up to half an hour to appear in search. Quieter laptop, slightly more patient search.
<!-- detail: The live work card now combines Rebel's actions and delegated work into one chronological stream, with clear ownership marks and job names. Blocked work appears on the plan, periodic updates describe something Rebel actually learned, and the full technical record remains available when you want it. -->
- **Rebel's work card now says who did what, and what it's learning along the way** - Rebel and its assistants used to report progress from separate corners of the same card, which made a busy job surprisingly hard to follow. Their steps now share one timeline, each with a small, stable mark showing who handled it, and an assistant introduces itself by the job it was given rather than the model it runs on. Work that gets stuck now shows up on the plan, where it belongs, instead of shouting from the sidelines. And every so often during a long job, Rebel posts a short note about something it has actually learned so far, grounded in what it really did, not filler. The full record is still there when you want the details. Less lane-watching, more actual progress.
...
<!-- detail: Rebel's safety check now reads shell commands according to the shell's syntax instead of relying on text patterns that could misunderstand escaped options and paths. If the command still cannot be understood with confidence, Rebel stops and asks rather than guessing which files it would change. -->
...
<!-- detail: Cloud sync used to download and inspect the contents of files that were already too large for Rebel to upload. It now recognises those files by size and skips reading them, while still keeping track of their presence so they are not mistaken for deleted files. -->
- **Rebel stops re-reading the files it was never going to sync** — Rebel's workspace sync walks every file in a cloud-synced folder (Google Drive, Dropbox, and friends) each cycle to see what changed. It used to open and hash every file — including the ones far too big to ever upload — so on a streamed mount it was downloading files it would never send, just to ask "did this change?". Rebel now recognises the files it can't upload and skips the read. Kinder to your cloud folders, and to the network they live on. The big files, left alone.

<!-- detail: If an image appears during a task and the current model cannot see it, Rebel can ask an image-reading model the specific question it needs answered and continue. Images you attach still offer a model switch instead; on Mindstone plans the lookup is included, elsewhere Rebel asks before a per-use charge, and text inside an image is never treated as an instruction. -->
- **Rebel can borrow a second pair of eyes** — When the model running your conversation can't read images, a chart, screenshot, or photo that turns up mid-task no longer stops it in its tracks: Rebel asks a model that *can* see, gets an answer to the exact question it needs, and carries on. Images you attach yourself still get the one-tap switch instead. Eyes, borrowed.

### Fixes

<!-- detail: A bookkeeping summary could disagree with an otherwise sound finished answer and make Rebel refuse to save the conversation. Rebel now trusts the saved finished answer in those proven cases, while genuinely conflicting answer text remains protected; the warning clears only after the conversation's content saves successfully. -->
- **Rebel stopped being suspicious of its own finished answers** — Rebel got suspicious of a few of its own finished answers and stopped saving those conversations out of caution. The answers were fine. The suspicion is gone, most **Preserved** notes went with it, and saving is back to normal.
...
<!-- detail: Live now answers from what Rebel already knows and can start a fresh check or action without ending the conversation. It confirms or corrects its earlier answer aloud when the result arrives; two known rough edges remain around announcing a check and sometimes waiting too long to give the useful part. -->
- **Live keeps answering while it works** — Ask something Rebel already knows and it answers straight away; if it needs fresh information or needs to do something, it starts that work while the conversation carries on, then confirms or corrects itself out loud. It can still occasionally announce the check or make the useful answer wait for it — more colleague than switchboard, with two corners left to sand.
<!-- detail: Company-provided plans now appear as managed by your organization in both Settings and first-run setup, without purchase, upgrade, cancellation or empty billing-portal controls. Personal subscriptions keep the controls they had before, and plan-change notices for company plans point you to your admin. -->
- **If your company provides your Rebel plan, Rebel now says so instead of trying to sell you a second one** — [Settings → Agents](rebel://settings/agents) used to show people on a company-provided plan the same controls as people who pay for themselves: a billing portal with nothing in it to manage, upgrade and cancellation buttons that could only confuse, and — during first-run setup — subscription offers for the very plan they already had. Now it shows the truth plainly: your company provides your plan, your admin handles changes to it, and there's nothing to buy here. First-run setup shows the same. If you pay for your own plan, nothing has changed for you.

<!-- detail: Source-capture recovery cards now use one plain explanation and the same action labels in the drawer, strip, and review dialog. Save appears only when filing remains possible; Keep note appears only when it is a genuinely different destination from Save (main projects that fact; absent means Keep is shown); Keep note says where the note went and offers View when the saved path is available; Review or change opens on the instruction field; Discard is visually subordinate until its confirmation. Within this flow the built-in space is named once, by its display name. Resolution telemetry is content-free. -->
- **Captured notes that need help now ask clearly** — When Rebel couldn't file a captured note automatically, the rescue card used to explain the same mishap twice and offer a muddle of **Save**, **Save privately**, and **Preview**. It now gives you one explanation and only the choices that lead somewhere different: **Save to** the intended Space when that can work, **Keep note** when that keeps it somewhere else, **Review or change**, or **Discard** behind a confirmation. Keeping a note tells you where it went and offers to open it. Fewer riddles; same safely kept note.
...
- **The shared-memory switch is taking a short holiday** — The **Shared memory suggestions** option in Settings → Safety is hidden for now: it was a switch for a feature that hasn't arrived yet, which made it a promise rather than a setting. It'll be back when there's something behind it — and if you'd already turned it on, it's still there for you, so you can turn it off. A switch that does nothing is worse than no switch.

<!-- detail: Rebel now recognises a recording even when a service gives the same meeting a new identifier, stopping the transcript from being filed repeatedly through the night. If saving was interrupted, Rebel tries again rather than dropping the transcript and tells you if it still cannot finish. Cloud meeting-bot transcripts are shared once the polished version arrives, sometimes a few hours after the meeting. -->
- **Meeting transcripts stop getting filed on a loop** — If a recording service quietly renamed a recording between syncs, Rebel could treat it as brand new and file the same meeting all over again — every few minutes, all night, with nothing to show for it. That loop is closed: Rebel now recognises a recording it has already handled. It will still take another run at a transcript it got interrupted on, rather than quietly dropping it — so the promise is no more all-night loops, not never twice. Meetings captured by the cloud meeting bot are also shared once their polished transcript arrives, which can be a few hours after the meeting. Once, as a rule, and never until dawn.

<!-- detail: One old set-aside note that Rebel cannot read no longer prevents you from discarding or declining another note. Rebel verifies the note you chose, completes that action when it can, and tells you if it could not confirm whether other waiting notes still need attention. -->
- **One unreadable note no longer holds all the others hostage** — When Rebel sets notes aside for your review, a single old note it couldn't read used to block you from discarding or declining any of the rest — with an error advising you to refresh and try again, when no amount of refreshing was ever going to help. Rebel now checks the one note you're acting on instead of demanding a perfect roll call of the entire waiting room, so the discard just works. And in the rare case where Rebel completes what you asked but can't double-check everything around it, the message now says exactly that, plainly, instead of stopping you or claiming more certainty than it has. The hostage situation, resolved.

<!-- detail: Cloud-sync file work now has firm time limits, so a drive that stops answering ends that sync attempt instead of freezing the rest of Rebel. Rebel checks the drive again later and continues when it recovers; any system work already stuck on the drive may wait there, but it can no longer block the app. -->
- **A cloud drive that stops responding mid-sync no longer freezes Rebel** — If your cloud drive (Google Drive, Dropbox, OneDrive) went unresponsive in the middle of a sync, Rebel could freeze solid and stay that way until you restarted it. Now Rebel notices the drive isn't answering, skips that sync round, and picks up exactly where it left off once the drive wakes up — no restart, no half-written files, no drama. The drive gets a time-out, not you.

<!-- detail: If Rebel temporarily cannot read your Chief-of-Staff instructions, it can use a previously verified copy and discloses when that copy was last refreshed once it is more than a day old. It also says when unavailable folders meant operators could not join the turn, records the use in Recent activity, and reports read-only folders accurately. -->
...
<!-- detail: Rebel now verifies the downloaded files used for on-device speech, search and supported local models before using them, including a one-time check of older installs. Damaged managed downloads are set aside and offered again, while unverified older locations are left untouched; some mobile and specialist model components are not covered yet. -->
- **Local models now check their paperwork before getting to work** — The first time Rebel uses or checks an existing on-device model after this update, it may spend a moment verifying the files. If a managed download is damaged or has been tampered with, Rebel sets it aside and offers a clean download instead of failing mysteriously. Trust, then verify.

<!-- detail: Starting a conversation from an action (tap an action → a new conversation opens) could lose both the conversation and the action if the network hiccuped at exactly the wrong moment: the only copy of the result travelled in a reply that never arrived, and by the time Rebel retried, the action had already been used up. Rebel now keeps that result until you have actually received it, so a retry picks up exactly where it left off instead of leaving you with nothing. -->
- **Starting a conversation from an action no longer vanishes mid-hiccup** — If the network hiccuped at exactly the wrong moment when you tapped an action to start a conversation, Rebel used to lose both: the action was spent, the conversation never opened, and neither came back. Rebel now recognises a retried start and recovers the original conversation instead — so long as the retry reaches Rebel before the record it needs has aged out, a flaky connection at the wrong second no longer costs you the action or the turn.
<!-- detail: A background automation that captures your meeting and email notes into memory could, when it found nothing new, file its own run log as if it were a captured note. The safety check correctly refused it, but the refused copy kept looking to Rebel like a task that was already done, so the card for it came back every few minutes no matter what you clicked. Rebel now stops the run log from being filed as a note in the first place, and a refused card is no longer treated as finished. The same update also brings back a handful of older notes that had been quietly stuck out of sight because their header was unreadable, so you can save or discard them. -->
- **Source-capture cards that wouldn't go away now go away.** When a background capture run found nothing new, it used to file its own run log as a note; Rebel's safety check refused it, but the refused copy kept looking like a finished task, so its card came back every few minutes no matter what you clicked. Rebel now stops the run log from being filed as a note in the first place, and older notes that were quietly stuck out of sight are shown again so you can save or discard them. No more return engagements.
...
<!-- detail: If your cloud drive stopped responding, checking its folders could block the parts of Rebel that everything else depends on — so every question spun forever with no answer and no error, and changing models didn't help. Rebel now notices when a drive stops answering, fails fast with an honest message about what's wrong and what to do, and keeps the rest of the app working around the unreachable folder. -->
- **A dead cloud drive no longer hangs Rebel — it says so, quickly** — If your cloud storage (Google Drive, Dropbox, and friends) stopped responding, Rebel used to spin forever: no answer, no error, nothing. Now Rebel notices the drive isn't answering and tells you promptly — what's wrong, and what to do about it. While a space is out of reach you'll see a slim banner up top, and everything that doesn't need that folder just keeps working. Diagnostic reports also got sturdier, so one sent from a struggling machine still arrives useful — which is, after all, exactly when you're likely to send one. The eternal spinner, retired.

<!-- detail: When saving a bug report times out, Rebel now says only what it knows: the save took too long, the report is still available, and you can try again. A save that completes after the timeout is delivered promptly, while failed attempts no longer leave temporary files behind. -->
- **The bug-report error stops accusing your innocent storage** — If saving a bug report took too long, Rebel used to guess at a culprit: "your device storage may be busy." One colleague checked their storage — plenty free — and reasonably concluded Rebel was making things up. It was. The message now says only what Rebel actually knows: the save took too long, your report is still here, try again in a moment. The save itself also got sturdier — a report that finishes saving late is now delivered rather than waiting for your next restart, and abandoned attempts no longer leave stray files behind. The accusation, withdrawn.

<!-- detail: Diagnostic reports briefly inspect your Chief-of-Staff instructions, but that check could wait forever when the instructions lived on an unresponsive cloud drive. It now has a firm time limit and omits that section when necessary, allowing the rest of the report to finish. -->
- **Diagnostic reports stop waiting on a drive that stopped answering** — Part of a diagnostic report is a quick look at your Chief-of-Staff instructions. If those lived on a cloud drive that had gone quiet, that quick look could wait indefinitely — inside the very report you'd send to complain about it. That step now gets a firm time limit: if the drive isn't answering, the report ships without it rather than not at all. The diagnosis no longer waits on the patient.

<!-- detail: Related to the bookkeeping fix below. The remaining stuck conversations held an old answer whose own saved work record no longer backed it up, and Rebel's guard refused every save rather than risk losing it. Rebel now keeps such an answer exactly as it was, marks it with a quiet "Preserved" note whose plain-language explanation is in the note's tooltip and accessibility label, and lets the rest of the conversation save normally again. Preserved answers are never quietly rewritten: if a later update tries to change one, Rebel keeps the original and shows a dismissible notice saying so. The note means "kept, not re-checked" — Rebel does not pretend it verified these answers, and healthy answers get no note at all. -->
- **Conversations stuck on "Conversation not saved" now save again, honestly** — The fix below handled the cases where the difference was pure bookkeeping. This one handles the deeper ones: when Rebel can't fully vouch for an old answer, it now keeps that answer exactly as it was, marks it with a quiet "Preserved" note, and gets on with saving the rest of your conversation. Preserved answers are never quietly rewritten; if anything later tries, Rebel keeps the original and says so. Honesty over tidiness.

<!-- detail: A harmless bookkeeping difference could make Rebel refuse every later save even when the stored answer and conversation history were identical to the letter. Rebel now repairs those proven cases on desktop and cloud, while continuing to block saves when answer text genuinely conflicts or more than one result could be correct. -->
- **Some stuck "Conversation not saved" errors now fix themselves** — Rebel guards your conversation history jealously: if saving might lose one of its answers, it refuses to save at all. Sensible — except one harmless bookkeeping wrinkle could trip that guard permanently, leaving a conversation that refused every save, forever, with a "Conversation not saved" message that never went away. Rebel now spots when the difference is pure bookkeeping — the answer itself identical to the letter — and quietly tidies the paperwork instead of refusing. Where there's genuine doubt about which answer is right, Rebel still refuses, on purpose: a nagging toast is annoying, but losing your answer would be worse. Fewer false alarms; the real alarm still works.

<!-- detail: On phones, answered and skipped question cards now move into the conversation so only unanswered questions stay pinned above where you type. You can hide an answered summary with Undo; the choice stays on that device and never changes or deletes the answer, while desktop continues to let answered cards scroll away naturally. -->
- **Questions you've already answered finally get out of the way on your phone** — When Rebel asked you something on your phone — a permission, a quick clarification — and you answered it, the card used to stay parked at full size above where you type, with nothing you could do about it. Answered questions now take their place in the conversation, scrolling away like any other message; only the questions still waiting for your answer stay pinned. And if you'd rather not keep looking at an answer you've already given, there's a quiet **Hide** on it (with an **Undo** if you change your mind) — hiding the summary never changes or deletes your answer. The card that wouldn't leave, finally excused.

<!-- detail: On phones and the web companion, a temporary “Couldn't load conversation permissions” notice can now be hidden separately from Try again. Hiding it changes no permissions, and a genuinely new failure brings the notice back so an old dismissal never conceals a later problem. -->
- **The "Couldn't load conversation permissions" notice can now be hidden** — If Rebel briefly couldn't load a conversation's permissions, a small notice would sit there with only a **Try again** and no way to move it on — sometimes right beside a card showing you'd already granted them. It now has a **Hide** beside **Try again**, and hiding it doesn't change any permissions; if permissions genuinely fail to load again later, the notice comes back. The notice that wouldn't budge, given a way out.
...
<!-- detail: Deciding whether a question needs a detailed plan used to require setting up a second model, so for almost everyone the feature never ran at all. It now works with whatever model you already have, and in a spoken conversation Rebel leans further towards answering directly. -->
...
- **Rebel stops crying wolf about which AI answered** — A behind-the-scenes watchdog that's meant to catch “you asked for one model, a different one actually answered” was comparing names letter-for-letter, so punctuation and naming differences for the same model set it off constantly. It now recognises equivalent spellings and still speaks up when a genuinely different model or billing provider served. The watchdog that cried wolf, recalibrated.
...
<!-- detail: On Macs running Tahoe, Rebel's icon now uses one layered source to produce Light, Dark, Tinted and Clear treatments with the Liquid Glass look. Older Macs, Windows, Linux and the installer icon are unchanged; the idea came from a Rebel user. -->
- **Rebel's Dock icon now dresses for the occasion on newer Macs** — On a Mac running Tahoe, Rebel's icon now picks up the Liquid Glass look and adapts to Light, Dark, Tinted, and Clear modes all on its own — same purple gradient, same `//m`, just better-mannered across the system's icon styles. Older Macs and other platforms keep the icon you already know. Suggested by a Rebel user.
...
- **Claude Opus 5 takes the lead on complex work** — Rebel's default Planner is now smarter at the same per-token price as Opus 4.8. Anthropic starts its thinking by default, so token use may look a little different; if its cyber-safety checks decline a request, Rebel says so rather than quietly swapping models — and Opus 4.8 keeps its seat in the picker.
...
<!-- detail: The four-step quality slider quietly disabled the Planner on three of its four settings, while a tight output limit could cut larger plans off halfway. The slider is now three clear steps — Quick, Balanced, Maximum — with Balanced as the plan-first default, a plain-language indicator showing what Rebel will do, and enough room for the Planner to finish. -->
...
<!-- detail: Choosing "Always block" on a tool approval now records a durable block, enforced quietly across every conversation and surface, and removable in Settings, Safety, under What Rebel can do → Never allowed. -->
- **"Always block" now takes the hint** — When you block an action for good, Rebel stops doing it and stops asking across conversations. Manage the list in Settings → Safety under "What Rebel can do" → "Never allowed." A firm boundary, finally acting like one.
...
<!-- detail: In the installed Mac app, Rebel's file watcher was quietly falling back to constant polling, which kept a CPU core busy and the fan spinning even when Rebel was idle, and it weakened a safety guard on quit. File watching now uses the efficient native path again. -->
...
<!-- detail: On Mac, an update could occasionally download but never install, leaving the app stuck until it was force-quit and reopened. The updater now sees that through and completes the install on its own. -->
...
<!-- detail: Open build only. Settings, Meetings gains a “Set up recorder” button that installs the meeting recorder for you, with progress, a plain-language error and retry, an honest note that it's available only on Mac and Windows, and the old copy-a-command route kept as a fallback. -->
...
<!-- detail: On Mac, quitting Rebel — including during Install & Relaunch — could occasionally end with a “quit unexpectedly” dialog even though everything had already been saved. Rebel now closes its file watchers in the right order; nothing was lost in the old failure, but it looked alarming. -->
- **Quitting Rebel no longer ends with a surprise crash dialog (Mac)** — Quitting could occasionally trigger a “quit unexpectedly” message after everything had already been saved safely. Rebel now closes its file watchers in the right order, so quitting stays quiet. Exit stage left, no drama.
...
<!-- detail: Rebel now runs on Claude Opus 4.8 (released May 28, 2026) as the default Planner and deep-thinking model for pay-as-you-go (Anthropic API key) and OpenRouter setups. Opus 4.7 and 4.6 stay selectable in Settings → Models if you prefer to pin a previous version. Pricing and context window match Opus 4.7. Mindstone flat-fee subscriptions keep their own curated model line-up. -->
- **Now on Claude Opus 4.8** — Rebel's default Planner just got an upgrade. Opus 4.8 is the new default for Anthropic API key and OpenRouter setups; 4.7 and 4.6 are still there if you'd rather pin one. Smarter by default.
...
<!-- detail: The plan view now groups contiguous parallel tasks into an "At the same time" swimlane with shared motion and screen-reader deduplication. This makes concurrent work visible without making each parallel task look like a separate crisis. -->
...
<!-- detail: On Mac, the preview/edit shortcut was catching Ctrl+E before the built-in move-to-end-of-line command could use it. The toggle now lives in the document header, leaving Ctrl+E to the editor. -->
- **Ctrl+E works in the Markdown editor again (Mac)** — The old preview/edit shortcut was quietly catching Ctrl+E, colliding with the Mac's built-in “jump to end of line” command. The toggle now lives as a button in the document header where it belongs. Ctrl+E is yours again.
...
<!-- detail: Diagnostic reports now summarize sign-in changes, cloud pauses, connection lifecycles, tool errors, interrupted turns and approval timeouts. When something goes wrong, the bundle provides a useful sequence of events without requiring someone to inspect raw logs. -->
...
<!-- detail: Rebel now repairs stale ChatGPT Pro connection details automatically and guards against the same routing problem returning. Safety checks, quick questions and personality touches also work reliably instead of stopping a request before it begins. -->
- **ChatGPT Pro routing is finally staying fixed** — Rebel now recognises your ChatGPT Pro connection reliably, including if an earlier version left it stuck. Safety checks and other quiet background work no longer block every request. The saga, ended.
...
<!-- detail: Existing Salesforce connections now move cleanly to the open-source connector instead of being left with stale setup. Delegated tasks using a lightweight model now start reliably, and cloud conversations keep their in-progress events through a recovery retry. -->
...
<!-- detail: When you ask Rebel to add a community connector, it now follows the full setup procedure instead of merely reading its instructions. That means it first checks whether a suitable connector already exists before proposing to build another. -->
...
<!-- detail: Tool-call summaries must now repeat the exact file, email or message identifiers involved in the proposed action. Approval cards therefore show what Rebel is about to act on instead of replacing the useful detail with a generic description. -->
...
<!-- detail: The Rebel Browser Extension for Chrome, Arc and Brave is now available from Settings → Connectors and pairs with a six-digit code. It can read, fill and click in your active tab while Rebel is running, keeps the connection on your machine, and still asks separately before filling sensitive fields or making destructive clicks. -->
...
<!-- detail: Comments on AI replies and markdown files are now protected from text that tries to disguise itself as instructions. A document comment clears only after Rebel accepts the message, so it stays put if sending fails or you change your mind. -->
...
<!-- detail: Safety checks, conversation titles, guiding principles and personality touches were failing for ChatGPT Pro subscribers because they were not using the signed-in connection. They now use that connection correctly, so approvals no longer loop when this background work fails. -->
- **ChatGPT Pro Background Tasks** — Safety checks, conversation titles and personality touches were silently failing for ChatGPT Pro subscribers because they missed the signed-in connection. They now use it correctly. Authentication, actually applied.
...
<!-- detail: The safety evaluation completed all 155 checks successfully. Memory-update wording was also compared across three measured approaches. -->
- Evals: Safety evaluation suite passed 155/155 checks; memory-update wording compared across three measured approaches
...
<!-- detail: DeepSeek, MiniMax and Z.ai/GLM models now use only providers with infrastructure in the US or EU when routed through OpenRouter. Rebel's default OpenRouter choices are now GPT-5.5, Claude Opus 4.6 and MiniMax M2.7. -->
- **Model Routing Safety** — Chinese-origin AI models now route exclusively through US/EU infrastructure. OpenRouter now defaults to GPT-5.5, Claude Opus 4.6 and MiniMax M2.7. Routing, intentional.
...
<!-- detail: Replit SSH setup now verifies the connection before finishing and reports setup failures more clearly. This makes a broken or incomplete connection visible while you can still fix it. -->
...
- **Multi-Provider Routing** — Background plan-first tasks no longer fail when routed to non-Claude AI providers. Compatibility, broadened.
...
<!-- detail: When Rebel is 95%+ confident it can answer your question from context it already has, it skips the plan-first and tool-calling phases entirely and responds directly. Simple factual questions, quick clarifications, and follow-ups get answered faster because Rebel doesn't go through unnecessary setup steps. -->
- **Faster Simple Answers** — Rebel now answers straightforward questions directly instead of going through its full plan-first phase. Quick questions, quick answers. Latency, cut.
...
<!-- detail: Delegated work now records its cost, enforces a timeout for each task and can use an explicitly chosen model when needed. This prevents a stalled task from waiting indefinitely and makes its cost visible. -->
- **Delegated work now has cost tracking and timeouts** — Behind-the-scenes tasks record their cost, each has a timeout, and a specific model can be chosen when needed. The meter and the clock agree.
...
- Early groundwork for a clearer strategic-work view exists behind experimental flags; it is not switched on yet.
...
<!-- detail: The local speech recognition model (Parakeet) now downloads automatically in the background the first time you launch Rebel on Mac or Windows. An inline progress indicator shows download status in the voice setup step. You can type normally while it downloads — voice activates as soon as the model is ready. -->
...
- CI: dev-push checks workflow added; end-to-end and performance tests re-enabled on Mac.
...
<!-- detail: Source Capture now uses a less expensive model for its behind-the-scenes work. The capture workflow itself is unchanged. -->
...
- Background conversations now use the same self-contained runner as the main app.
<!-- detail: Quality checks now share one set of scenarios across conversation channels, followed by another full end-to-end run. This makes the same behaviour easier to verify wherever a conversation runs. -->
- Evals: shared fixture corpus across conversation channels, plus another full end-to-end run
...
- Rebel can now build and track a plan for complex work without losing it between steps.
...
<!-- detail: Connect your Claude Max subscription directly — no API key needed. Rebel walks you through a two-step sign-in during onboarding or from Settings. If you hit rate limits without a fallback API key, a clear banner explains your options. Works on Mac, Windows, and Linux. -->
...
- **Watchdog** — Delegated work no longer triggers false stall warnings. Rebel now judges activity from what the work is actually doing instead of a message type that could never appear. Patience, calibrated.
...
- Supporting model tasks now receive more room when deeper reasoning is enabled.
...
<!-- detail: Skill quality now has a scoring system, visible indicators, better repair guidance and proactive reminders. Together they make it easier to spot a skill that needs attention and improve it. -->
...
- Stall detection now adjusts upstream timeouts from 30 to 240 seconds according to the selected reasoning effort.
...
<!-- detail: Rebel now separates AI models into "Thinking" (complex reasoning) and "Working" (execution, writing) roles. You can assign different models to each role — including non-Claude models like GPT, Gemini, and others — via tier-based proxy routing. Pick the right tool for each part of the job. -->
- **Thinking & Working Models** — Choose different AI models for reasoning versus execution. Mix and match Claude with other models for different tasks. The right brain for the right job.
...
<!-- detail: Errors that previously landed in one broad fallback group are now separated again by their specific failure details. Exact repeats stay together, while distinct underlying problems no longer hide in the same pile. -->
- **Broadly grouped app errors are now separated by their specific cause** — Exact repeats stay together, while failures with different details get their own groups. The useful signal is easier to find.
...
- **Updates recover when relaunching goes wrong (Mac)** — Rebel now notices when an update fails to reopen the app and helps the process recover. Updates, resilient.
...
- **Updates install cleanly when Rebel quits (Mac)** — A conflicting shutdown step no longer prevents an update from installing. Updates, smooth.
...
<!-- detail: Two especially noisy groups of error reports now use their source and specific failure details to separate distinct problems while deduplicating exact repeats. The result is less noise without merging unrelated causes. -->
- **Repeated error reports are grouped without mixing different causes** — Exact duplicates no longer crowd out the useful clues, while distinct failures remain separate. Less noise, better diagnosis.
- Help docs updated: file attachments, AI models, Microsoft 365 corrections and the Granola connector
...
- **Update handoff fixed (Mac)** — Rebel no longer trips over the timing between closing the app and installing an update. Updates, reliable.
...
<!-- detail: Rebel now has the foundations for connectors to show richer interactive views inside a conversation. The supporting message format, display surface and safety boundaries are in place behind the scenes. -->
...
<!-- detail: Progress tracking now includes fluency tiers, evidence-based advancement and 27 new badges across a wider range of useful habits. It gives you more ways to see what you are learning and using consistently. -->
...
<!-- detail: Restarting onboarding now clears the previous welcome-flow state before beginning again. Old progress no longer leaks into a fresh start. -->
...
<!-- detail: Enable auto-archive mode with Cmd/Ctrl+Enter and Rebel will automatically archive conversations when they naturally conclude. Before archiving, a lightweight AI check looks for unfinished business or follow-up items; if it finds any, Rebel leaves the conversation active so nothing important gets buried. -->
- **Auto-Archive Mode** — Toggle “fire and forget” mode to auto-archive conversations when complete. A lightweight check looks for unfinished business before filing anything away. Cleanup, automated.
...
<!-- detail: New users now get a voice-guided introduction to Rebel, its key features and the initial setup. The experience includes badges and a 14-day journey, followed by a brief tour of the main parts of the app. -->
...
- **Smart goal healing** — Rebel automatically repairs malformed goals into the format it can use. Your intentions, preserved.
...
- Release signing on Mac now retries transient failures, and interface checks wait more patiently for the app to respond.
...
- **Smarter space descriptions** — Auto-generated space descriptions now sample about 100 files and produce richer, more nuanced summaries. Three to five sentences that actually capture what's in there.
...
- **Spaces activity dashboard** — See what's happening across your spaces with AI-synthesized summaries. "Your Home space saw 3 new memories this week, mostly about Q1 priorities."
...
- **Smarter plans** — Complex plans can now be handed to a specialist assistant. Divide and conquer, as they say.

...
<!-- detail: Rebel's built-in web search leans on a free service that occasionally returns a "prove you're human" checkpoint under load. A previous change turned that into a prominent card urging you to add a paid search service — which also fired on Rebel's own per-turn search limit, so it read as "my internet is broken and Rebel is selling me something". The hard per-turn limit is replaced with gentle pacing, and the card now appears only for a genuine checkpoint from the search provider, with blame-free wording naming the real cause and the options to set up your own search or wait it out. The Brave Search help page was also corrected, since its free tier has ended. -->
...
<!-- detail: Open-source build only. Setting up the open build now offers an optional "About you" step — first name and email — stored locally to personalise Rebel. If you fill in the email it is also sent to Mindstone as a consented, best-effort contact, disclosed at the point you enter it and never blocking setup. The privacy policy and the open-source-build help page were updated in the same change. -->
...
<!-- detail: The Usage page's headline spend figure counted the equivalent API value of every turn, including those covered by your ChatGPT or Mindstone subscription — so it could read many times higher than the amount that actually left your wallet. The headline now leads with an honest "You paid", showing true out-of-pocket spend only, presents what your subscription covered as reassuring context rather than a bill, and puts older spend from before cost tracking on its own line instead of silently inflating the total. Two attribution fixes sit underneath: a managed Mindstone plan is now correctly counted as covered, and background context compaction is attributed to the subscription that actually ran it. -->
...
<!-- detail: Rebel now has a policy for who can message it from Slack, with three modes: just you, an allowlist, or the previous permissive behaviour. New installs default to just you, so strangers are silently ignored, while existing Slack users keep the previous behaviour until they review it once, so nobody gets a surprise lockout. Blocked attempts are visible under Continuity & Messaging, where you can allow or block by Slack ID. The policy isn't Slack-specific — the same logic works for any inbound channel that identifies who is writing — and it includes per-sender rate limiting and layered self-loop suppression, so an accidental self-trigger is quietly absorbed rather than starting a back-and-forth. Cloud instances record blocked attempts authoritatively so the list is consistent everywhere. There's a user-facing guide in the help docs. -->
...
<!-- detail: Plan steps marked as independent now fan out to run concurrently, up to a cap of four sub-agents at once, settling as each finishes and reporting through a single combined progress banner. -->
...
<!-- detail: A unified provider information architecture replaces the old single-provider model namespace. One curated catalogue is the source of truth, de-duplicating by billing route so a ChatGPT Pro subscription and a direct API-key profile stay distinct. Settings model dropdowns show every model from every provider in one clean list, route events are tagged with what executed them, and learned context-window limits are now remembered per model profile across sessions. -->
...
<!-- detail: Behind a feature flag, a planner picks the best model and reasoning effort for each step of a turn from the models eligible on your account, instead of running the whole turn on one expensive model. Models are presented to the planner as peers with strengths and weaknesses — curated defaults for a couple of dozen known models, customisable per model in Settings. The thinking panel shows which model executed each step. It's ready for per-step model switching when that lands. -->
...
<!-- detail: Adding your own sign-in-based connector, whether through Rebel or from a bare URL, used to leave you with an unauthenticated server and a misleading "service unavailable" message, because a connector with no catalogue entry was never probed or marked as needing sign-in. Two rounds of fixes: recovery now triggers sign-in for connectors outside the catalogue, and probing a URL auto-detects that it needs sign-in. Separately, the sign-in callback now listens on both local network stacks, closing a case where a sibling development server on the other stack silently received the callback. -->
...
<!-- detail: Several connectors weren't wired up correctly for their setup fields, so API keys you entered in Settings were silently dropped: Fathom, PandaDoc, Mixmax, Humaans, Kling, Runway, QuickBooks, ServiceNow and TalentLMS. The IMAP email connectors — iCloud, Yahoo and Custom Email — were dropping both the address and the password. -->

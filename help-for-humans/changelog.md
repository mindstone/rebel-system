...
<!-- detail: 260831_approval-wait-and-withdraw. After an accepted answer, Rebel conservatively reviews only saved legacy tool deferrals. Needed actions remain pending; explicit no-longer-needed actions settle durably as withdrawn, never run, and receive one grouped cross-surface receipt. Staged connector/file/memory work is excluded. Invalid or uncertain review keeps everything. Keep public copy non-technical. -->
...
<!-- detail: 260829_fireworks-cost-tracking, Stages 1-4. Fireworks shipped as a dispatchable provider with no cost-tracking strategy — every turn silently recorded a wrong $0. Fixed end to end: an 'unpriced' state now survives resolution through every ledger/analytics reader (never a silent $0 for non-local, non-free usage), a new 'organization' payment bucket keeps admin-delivered spend out of both "you paid" and subscription-covered totals, and Fireworks prices are now live-fetched from its own control plane with a persisted last-good snapshot (no hand-maintained price table to go stale). D2: costs are labeled "Estimated (list price)" everywhere, never presented as an invoice-exact organization bill. Keep implementation internals (snapshot state machine, catalog-purity pin, credential-source naming) out of public copy. -->
...
<!-- detail: WebFetch embedded-content recovery (direct run, no planning doc). src/core/rebelCore/tools/webFetchEmbeddedContent.ts + the delivery gate in webFetchTool.ts. A page that ships an empty DOM and its text in an inline hydration payload used to return as a successful read carrying only the page title; recovery now mines json-ld / framework state payloads / inline JSON, and a page with genuinely nothing readable fails as the new empty_content class with the do-not-cite advisory. Keep public copy non-technical — no tier names, thresholds, or class names. -->
...
<!-- detail: 260820_reset-timing (internal planning doc; CE2 run, residue R1 of 260819_error-copy-clarity). Dispatch-time anchoring for rate-limit/quota reset timing: the humanizer now anchors a duration-only retry window ('retry after N') to the error event's own timestamp, so reopened transcripts recompute the wait from a real moment instead of re-showing a duration frozen at the original failure (which read as fresh forever). Applies to ALL windows incl. <15min, and repairs already-saved transcripts (they already carry the event timestamp). The primary desktop banner, which rendered a body frozen at classify time, is refreshed at render from the anchor. Design chosen (Design B) threads event.timestamp, no new persisted field, no schema/rollback surface. Also consolidated the two near-duplicate reset formatters behind one @rebel/shared home + shared internals (removes a hand-sync hazard, no output change). Known residual: the in-transcript message card / diagnostics still show the original text (canonical-text persistence) — the diagnostics label now reads '(original retry-after)' so it's clearly historical. Keep public copy non-technical — no anchor/formatter/dispatcher internals. -->
...
<!-- detail: 260819_error-copy-clarity (internal planning doc; CE2 run, tranche 1 of the worry-surface copy audit 260819_error-copy-audit). Rewrote the highest-traffic worry-surface copy (rate-limit, BYOK billing, session-not-found, moderation) to lead with what happened -> whether the user's work is preserved -> what to do next, keeping Rebel's dry voice but only after the facts. Honesty rules enforced: no unverifiable "your work is safe" / "this turn didn't run" (tools may have run) -> "This request couldn't complete."; provider named only from an attested canonical map (never "Openai"/route-label "Codex"); session-not-found reframed as server-side resume-context loss (not deletion/other-device), Try again retained; moderation drops the pointless identical-retry; billing says "needs more credit" not "run out of credits", no daily-reset claim. New reset-timing (Option A): show an ABSOLUTE clock-time (e.g. "try again after 8:20pm", "tomorrow at 8:20pm PST" for longer waits) when the provider gives a real reset timestamp and the wait is >=15 min; shorter/duration-only waits stay relative; nothing new persisted (dispatch-time anchoring deferred). Classifier + humanizer changed in lockstep so cloud/mobile get the same copy. Keep public copy non-technical — no classifier/humanizer/marker/anchor internals. -->
...
<!-- detail: 260829_queue-drain-busy-retry-loop Stages 1-6. A background conversation's event-derived busy state was repeatedly erased by receipt/draft/list projection writes, while main-admitted null-window continuations were invisible to the renderer. Waiting messages then retried into main's correct reject-policy guard on summary churn (incident excerpt: 84 drains / 84 typed refusals across two turns). The summary now folds its background event buffer and main admission occupancy; deferral is transition-keyed with one queue-owned reconsideration deadline instead of the 15-second refusal timer. Keep public copy non-technical — this is quieter errors and a steadier thinking indicator, not a feature launch. -->
...
<!-- detail: 260820_fox3783-reconnect-cta (internal planning doc; CE2 bug_mode; (internal ticket)). The Automations "Reconnect" CTA (and 5 sibling deep-link CTAs across toasts/notices) opened the Settings dialog without switching the active flow surface, so a surface-sync effect closed it instantly and the Settings view never mounted — the button did nothing from any non-Settings surface. Root cause was a per-caller footgun (6 of 27 settings-open callers omitted the surface switch); fixed by construction at the shared entry point so every opener activates the Settings surface, then the Automations reconnect CTAs were deep-linked to the exact provider card (ChatGPT Pro / OpenRouter). Deferred follow-ups: return-to-origin after closing Settings, and a one-click reconnect on the "ChatGPT rejected your connection" card (tokens still present, so it shows Disconnect). Keep public copy non-technical — no surface/hook/section internals. -->
...
<!-- detail: 260819_fix-chatgpt-pro-ratelimit-resurface (internal planning doc; CE2 bug_mode; internal ticket). ChatGPT Pro (Codex) users saw "Rate limit reached." on every turn while a managed plan worked. Production telemetry (ai_error_shown, codex-subscription: billing=0) refuted the prior "genuine weekly cap" verdict — Rebel wasn't recognising Codex quota/limit errors as quota (divergent token lists across transports + a new usage_limit_exceeded wire variant handled nowhere + streaming reset-timing dropped + text over-match), so they surfaced as unknown/server_error/rate_limit with the misleading rate-limit banner and no reset time. Fix classifies token-bearing Codex quota correctly across all transports with reset timing, honest plan-cap copy, and single-sources the vocabulary; token-less bodies remain deferred but are now measurable. Keep public copy non-technical — no token/transport/classifier internals. -->
...
<!-- detail: 260820_openrouter-retry-408-honesty (internal planning doc; CE2 bug_mode; internal ticket). Fast-follow to the entry above. Two user-facing corrections: (1) the honest no-timing copy now also reaches users on a Mindstone-managed plan (a gap where they still got "try again in a bit / not on a timer"); (2) Rebel again retries two transient network hiccups (request-timeout / conflict) it had briefly stopped retrying after the watchdog fix above, so a momentary blip no longer dead-ends a turn. Keep public copy non-technical — no 408/409/maxRetries/classifier internals. -->
...
<!-- detail: 260819_workshop-depth. The Workshop is no longer a Labs/experimental surface. The default lives in Account & Preferences → Appearance and remains Conversation unless changed. Each conversation can follow that default or explicitly use Conversation/Workshop. The room now has 15 semantic station types, fixed placement, represented helper clustering, shorter at-rest copy, a contained live inspector, responsive side/bottom/compact layouts, and accessibility/theme/reduced-motion coverage. Keep public copy non-technical; do not quote fixture/corpus metrics, internal key names, provenance layers, or implementation terms. -->
...
<!-- detail: 260814_unified-approval-authority Stage 38. Public truth boundary: queued MCP/connector calls already use service-owned staged execution, so approving dispatches the exact stored call and denial terminalizes it. Outcome-unknown is explicit after a possible effect. The new exact-held-call lifecycle for Bash/built-ins is implemented but STATIC DEFAULT OFF pending an owned production cohort/calibration/rollback/user go; do not describe it as generally live. Rating-derived bands/capabilities/modes are inert: no production artifact packaging/loading/ingestion, empty verification-key ceremony, catalog backfill 0/2,633. Remembered deny is incomplete: always deny exists, conversation deny is hidden behind its cross-surface readiness fence, automation deny is absent. Keep public copy non-technical and do not claim all lifetimes/surfaces ship. -->
...
<!-- detail: 260818_repeated-memory-approvals (internal planning doc; CE2 bug_mode run). Fixes an outside-workspace memory-save approval pile-up: addPendingMemoryApproval deduped only by toolUseId and the destination-wide checkExistingPendingPreflight guard consulted only staged pending FILES (which never exist for outside-workspace dests, since validateDestination refuses staging), so a recurring save to one outside-Space file minted a fresh approval card on every attempt (observed: 8 cards for one file from one conversation's background turns over ~5.7h). Fix: broadcastApprovalRequest now coalesces non-staged memory_write approvals by normalized destination — exact-digest-equal suppress, else last-wins supersede — plus a store-level exact-dup guard for the cloud relay; the digest-bound single-use token model is untouched and stays fail-closed. Stage 3 makes the card honest (real destination tail, "Outside your Spaces" badge replacing "Unknown", one recurrence line); F8 stops re-deriving the file location on every fetch (kills 500-650x/log noise). Deliberately NOT done: durable "always allow this folder" grant (outside-workspace saves still ask each time — by design), one-time migration of existing piled-up cards (dismiss by hand). Keep public copy non-technical — no toolUseId/digest/coalesce/preflight internals. -->
...
<!-- detail: 260819_outside-workspace-write-grants (internal planning doc; CE2 run). Lets a user grant Rebel permission to SAVE to a destination outside their Spaces at three lifetimes (once / this conversation / always) and two scopes (just this file / the enclosing folder), inline on the memory-save approval card, harmonised with the existing file-access "Locations" (read) grant machinery. This deliberately lifts the durable-write deferral the prior 260818 changelog entry noted ("outside-workspace saves still ask each time — by design") for the PRIVATE/single-owner case only. Architecture: unify at the location-authority seam (reuse FileAccessGrant / durable store / canonical matching / one Settings surface); the memory lane keeps content-scan / staging / digest tokens / publication. Model: a save proceeds only when THREE independent authorities pass — location grant AND content decision (the credential/sensitivity scan still runs on every save — a location grant satisfies only the "where") AND a turn-local, single-use execution capability minted after both gates (so a durable grant never becomes an ambient writable zone). Durable "always" is offered ONLY for private/single-owner outside destinations: Rebel never infers outside = private; a known-shared destination is refused; an unknown outside destination requires an explicit recorded private/single-owner attestation via a dedicated stronger confirmation, and a later authoritative shared classification makes the grant inert. Delete is never grantable; durable grants are desktop-local (cloud/mobile honest-degrade to one-use). Store bumped v1→v2 with a rollback-safe ledger entry (an old build parses but never honors a write row). Keep public copy non-technical — no capability/attestation/digest/store-version internals. -->
...
<!-- detail: 260820_zendesk-reland-beta (internal planning doc; CE2 run). Relands the Zendesk connector (cut June 2026) as an admin-assisted desktop Beta on @mindstone/mcp-server-zendesk@0.4.1 (26 tools). API-token auth only (admin-created token); setup persists accounts.json via the host and supersedes any stale legacy OAuth token. Desktop-only for Beta; connector degrades observably on cloud/mobile. Keep public copy non-technical — no accounts.json/bridge/config-dir/handler internals. Do not overstate: it needs a Zendesk ADMIN to create the token, and it's Beta. -->
...
<!-- detail: 260819_protected-answer-notice-investigation (internal planning doc; CE2 run, (internal ticket) follow-up). The "This answer is protected" live-fence notice fired constantly on healthy turns because the answer-integrity kernel used the event-trace digest (a hash of the whole, legitimately-mutable event array) as a quarantine + fence trigger; benign cloud-sync/compaction churn flipped verified answers to sticky conflict-quarantined. Fix grounds conflict decisions in answer content (text/displayText + answer-bearing primary MCP App view payloads), demotes event-trace drift to telemetry, and lazily re-verifies the falsely-quarantined turns at load. The notice now fires only on a genuine attempt to overwrite a finished answer, and was retoned from an amber warning to calm info with claim-safe copy. Deferred to follow-up: the verified-answer write-path guard (unsafe on multi-attempt turns as specified), cloud fence-receipt propagation, mobile/web notice + displayText presentation. No store-version bump. Keep public copy non-technical — no digest/quarantine/fence/envelope internals. -->
...
<!-- detail: 260817_vision-look-robustness. When the working model can't see images, a non-vision worker that Read an image mid-turn used to loop — re-copying and re-reading the same file — because the placeholder copy literally told it to "re-read it from disk". Three model-facing changes: (1) placeholder copy no longer invites a re-read (honest: "reading the file again will not change this"); (2) a deterministic loop-breaker suppresses the 2nd+ read of the same image by construction (no new handle to chase) and restates the first handle so the Look tool can still describe it; (3) the planner is now told deterministically whether the worker can read images, so it plans the Look lookup instead of hedging. No user-visible UI/copy surface; purely reliability. Keep public copy non-technical — no placeholder/materialization/handle/tracker internals. -->
...
<!-- detail: 260817_mcp-tool-grant-copy (internal planning doc; CE2 run). When an interactive view (e.g. the Slack message composer) calls a tool it has no per-conversation grant for, the trust-boundary rejection copy was untrue and vague: "View tried to use a tool that isn't allowed. Grant access in Settings." — but Settings → Connected app permissions is revoke-only (no grant control), and the message never named the source or capability. Copy on both surfaces now names the source (resolveSourceDisplayName) + a host-owned capability phrase (describeMcpAppToolCapability: post_slack_message→"send messages", send_workspace_email→"send emails"; fail-closed to generic copy, Object.hasOwn guard so prototype-key tool names can't leak inherited values), frames the USER as decider, and points at the real inline "Allow for this conversation" button (relabelled from "Grant for this conversation"). The iframe-facing safeMessage stands alone with "…Allow it in Rebel, then retry". Same false-Settings copy fixed on the sibling ui/sendMessage + ui/updateModelContext paths. No change to the permission model, auto-grant allowlist, or replay behaviour. Keep public copy non-technical — no tool-id/nonce/allowlist internals. -->
...
<!-- detail: 260816_nudge-files-queue-simplify (internal planning doc; CE2 run). While a turn is running the composer now offers exactly two options: Send (which reaches the running task, carrying attachments) and Stop. Named retirements, must be stated so nobody hunts for them: the queued-messages tray is gone, and with it the per-row Remove — there is NO replacement for un-sending a message that is already waiting (a pending nudge in the current conversation still has Cancel nudge before it is picked up). Stop & send is gone: its one-click redirect is now Stop, then Send, with no confirmation. Alt/Option+Enter and the one-time nudge explainer toast are gone. Voice changed too: a spoken interrupt Rebel is confident about now stops and sends directly, with no confirmation click. Durability boundary is a hard honesty line — attachments on an unsent message are held only for the running app session; a restart restores the text and asks for a reattach, and copy must NEVER imply files survive a restart. Also never imply Send waits until the run finishes, never imply Send cancels the run, never claim a missed nudge was acted on, and never claim a "couldn't confirm" nudge was NOT acted on. Keep public copy non-technical — no directive/inbox/injection/digest/token/payload-store internals, and no byte limits. -->
...
<!-- detail: 260817_residue-chunk8-meeting-mobile Stage 4 (internal planning doc; CE2 run, completing the zombie-watchdog #R2 recommendation). When the meeting bot's liveness watchdog declared a bot dead, the only user-visible signal was the absence of notes. There is now a one-shot acknowledgment at that moment: an OS notification (title "Your meeting bot went quiet") carrying the meeting title and the honest notes outcome — live captions kept (click reveals the saved file), notes already saved, notes awaiting review, captions that couldn't be saved, or nothing captured — plus a one-shot durable marker on the meeting-history entry recording the same outcome, so the event stays legible after the notification is gone. Fires once per dead bot (idempotent marker + per-process guard), never for bots that failed for other reasons, and never during a recording-service outage — the outage warning owns that signal (it arms ~30 minutes into an outage, hours before the watchdog can attribute the silence to a single bot). Deliberately not routed through the meeting-health warning envelope: that model is for ongoing degraded states that resolve, not a per-meeting one-shot terminal event. Keep public copy non-technical — no watchdog/terminal/marker/envelope internals, and do not promise a notification in every case (OS notification support and test mode can suppress it). -->
...
<!-- detail: 260818_low-ram-mode (internal planning doc; CE2 run). Computers under 12 GiB now receive an explicit, dismissible Home offer; Rebel never changes the setting without acceptance. Search remains available, with slower indexing while enabled. -->
...
<!-- detail: 260820_fix-efficiency-mode-settings-unreachable (internal planning doc; CE2 bug_mode). The Efficiency Mode section rendered only inside an orphaned settings tab no destination mounts, so the Settings-search result and the Home low-RAM offer's Settings button navigated to Account & Preferences and scrolled to nothing. Same class: the "Smart model picking toggle" search result routed to Workspace while its section renders in Advanced. Fixed by mounting the section in Account & Preferences (with an on-page anchor), rerouting the Smart-picking toggle result to Advanced, and adding a render-tree regression test. Keep public copy non-technical — no anchor/destination/contract internals. -->
...
<!-- detail: 260820_di4-error-kind-unknown-bucket (internal planning doc; CE2 bug_mode; internal ticket). The product's single largest error category was a vague catch-all "unknown" bucket (fleet-wide, all providers) caused by Rebel destroying its own error classification/attribution at two internal transport seams before the error could be shown or recorded: the in-band string channel flattened a fully-classified error to plain text one hop before classification (~59%), and hand-written error-copy terminals carried no kind (~36%); 66% also lost provider/credential attribution. Fix recovers the real kind across the string seam (as a fallback consulted only after the load-bearing billing-before-rate-limit ladder, so nothing else reclassifies) and recovers provider/credential attribution at three dispatch seams, fail-closed (refuses to guess rather than mislabel a provider). Also flips codex/ChatGPT plan-cap errors from "rate limit" to honest plan-limit copy on the newly-attributed population. Evidence-first re-diagnosis overturned the spun-out framing (a near-no-op) and a tautological telemetry signal. Keep public copy non-technical, and don't overstate for the user: this mostly improves error labelling/diagnostics and honesty of copy, not a feature they'll see directly. -->
...
<!-- detail: 260819_openrouter-ratelimit-copy (internal planning doc; CE2 bug_mode; internal ticket). A heavy user on an OpenRouter-served model kept hitting a real, sustained rate limit and saw vague/misleading copy: the terminal "try again shortly / resets within a few minutes" and the "Rebel needs a short pause" toast, neither true for a throughput ceiling that won't clear soon. Root cause: the local model proxy stripped the provider's timing headers on error responses (so Rebel never knew the real reset time), the timing parser didn't understand OpenRouter's reset header, and the copy branch only recognised OpenAI-named providers. Fixed: forward the timing headers (curated allowlist), parse OpenRouter's reset timestamp, and make the copy honest + route-aware (never promise a short wait when we don't know), keeping the switch-provider option. Also fixed a review-caught regression where forwarding the wait time could make Rebel hang until it aborted, and a misattribution where managed-pool users were told "your OpenRouter connection" though they never connected it. Keep public copy non-technical — no proxy/header/parser/maxRetries/attribution internals. -->
...
<!-- detail: 260818_codex-gpt55pro-stuck (internal planning doc + internal ticket; CE2 bug_mode). ChatGPT Pro users with a Planner model set could hit a pre-dispatch routing failure: plan mode's internal model placeholder reached the ChatGPT Pro model check unresolved, so the turn ended before it started. Two UX defects made it unrecoverable: the error title was hardcoded to "GPT-5.5 Pro" regardless of what actually failed, and the switch-model recovery repaired global settings only, so a per-conversation model choice looped back into the same error forever. Also fixed: a Claude model in the Planner or Main work seat while on ChatGPT Pro now gets the clear, recoverable "Claude isn't available on ChatGPT Pro" message. Keep public copy non-technical — no placeholder/route/terminal internals, and don't imply the user misconfigured anything. -->
...
<!-- detail: 260817_residue-chunk8-meeting-mobile Stage 2 (internal planning doc; CE2 run, residue from the 260811 zombie-watchdog plan). When the meeting bot's liveness watchdog declared a bot dead (status unconfirmed past its window), any live captions already captured for that meeting were orphaned forever — the notes existed on disk but were never finalised or shown in meeting history. The watchdog's teardown now, before killing the bot's runtime state, awaits one final caption flush, finalises the on-disk live transcript as a captions-only file (clearing the live flag, pinning captions quality, stamping a provenance note — no full transcript will ever follow, the bot is gone), and marks it saved so it appears in meeting history with the same treatment as any captions-quality transcript (standard analysis scheduling, deliberately no async-upgrade polling). Salvage failures are logged and never block the rest of the teardown. Keep public copy non-technical — no watchdog/terminal/frontmatter/analysis-queue internals, and do not promise the full polished transcript: captions only. -->
...
<!-- detail: 260818_fix-conversation-not-saved Stage 3. Persistence warnings are now scoped to the affected conversation and distinguish a temporary delay, an internal save block, and a genuine storage problem. Permanent warnings can be dismissed without immediately returning, offer Copy text from the live on-screen conversation, and leave a Not saving badge until that conversation saves again. Keep public copy calm and non-technical. -->
...
<!-- detail: 260816_space-refresh-staleness-ux (internal planning doc; CE2 run, successor to 260813_cloud-rewalk-target-convergence and the closure of its R11 residue). Adds a workspace-scoped freshness surface + one rate-bounded manual re-walk on cloud-backed workspaces: a "Checked Xm ago" line and a "Check for new files" action in the Library toolbar and Settings → Spaces, plus per-Space freshness on the storage badge tooltip. Two desktop-local library IPC channels; every rate bound lives in main (no cloud targets ⇒ no walk; one shared request N clicks join; a 5s completed-result floor; at most one trailing pass mid-patrol; a 15s handler bound past which it answers still-checking and keeps working). Honest bounds, do NOT overstate in public copy: the 10→30-minute background patrol ceiling is UNCHANGED — this makes the staleness visible and overridable, it does not make Rebel notice faster on its own; "checked" means a pass RAN, never "you are up to date" (the bounded walker turns an unreachable cloud folder into a truncated pass), so never write "up to date"; the found-files count can under-report but never over-reports; freshness is per-app-run and starts cold on launch, so after a restart it honestly says it hasn't checked yet; and a BACKGROUND pass still does not repaint the Library tree (the manual check does). Keep public copy non-technical — no re-walk/index/verdict/probe/poll/channel/patrol internals, and no "re-index". -->
...
<!-- detail: Two telemetry consent toggles in Settings → Privacy & Safety → Privacy & Data for the managed app: Share usage analytics (behavioural analytics, and the in-app surveys that ride the same channel) and Share crash & error reports (automatic error reporting). Both default ON with existing users grandfathered; nothing was silently switched for anyone. Both take effect on that device immediately — new events refused, held events discarded, no flush on shutdown or re-enable, identity cleared, and for crash reports the crash files already waiting on disk are deleted — and the choice syncs to other devices through Cloud Continuity, applying when each next connects or is brought to the front (wake-driven, not pushed). Cloud Continuity is the ONLY transport for the consent flags, so two devices without it never converge — each keeps what was set on it. Turning crash reports off asks for confirmation. User-submitted bug reports keep working (deliberate act each time); a conversation rating that cannot be delivered now says "Rating saved, not sent" rather than faking success. Honest limits, all stated in the new help doc: a dispatched request cannot be recalled; reverting to a pre-control version resumes collection. The analytics library's own on-disk delivery queue is emptied on opt-out and blocked from being refilled — verified end-to-end, so the earlier "may survive a restart" hedge is retired (a session that STARTS opted out still loads no analytics client at all, which remains the strongest case). Not covered: app updates, sign-in/org checks, AI providers, connectors, meeting transcription, Cloud Continuity sync, and Mindstone's monitoring of its own cloud service. New bundled doc what-data-can-leave-rebel.md inventories every egress point; privacy policy bumped to v3.3. Keep public copy non-technical — no gate/argv/queue/store internals, and do NOT claim it applies instantly everywhere or that nothing more is ever sent. -->
...
<!-- detail: 260814_source-capture-recovery-ux Stage 3B (checkpoint-gated). At turn end the host may now repair and file a captured note itself, bounded to notes whose valid twin would already have been auto-approved at the same seam — proven atomically under the staging lock (destination provably unoccupied) and re-proven under the destination lock at consumption (same row, bytes, conflict anchor, destination, sharing, turn, and a fresh policy re-resolution). Filing takes the complete guarded user publish path with no actor: live refusal, deterministic repair, fail-closed re-validation, conflict detection and verified terminal removal all run. Anything unprovable, Private Mode, a cautious/balanced space, changed bytes, a competing review, a conflict or a timeout all decline and reveal the card exactly as before. A user action that loses the race reports "Rebel filed this note" rather than a discard or keep. Keep public copy non-technical — no capability/mint/occupancy/lock internals. -->
...
<!-- detail: 260813_cloud-rewalk-target-convergence (internal planning doc; CE2 bug_mode). Two non-convergent background loops. (1) cloudPeriodicRewalkService.runTick used the standing level fact "≥1 cloud target is healthy" as a freshness trigger, so a stable healthy cloud Space forced a whole-workspace discovery pass every 5-minute tick forever. Now: health-EDGE admission (first-healthy / degraded→healthy / explicit trigger all walk immediately) plus an unconditional adaptive quiescent patrol at 10→20→30 min. The 5-minute PROBE cadence is unchanged — probing, not walking, keeps the admission verdict alive. Watcher silence is never used as proof of cloud freshness (cloud subtrees stay excluded from the live watcher), so the patrol is bounded rather than event-gated. Only a walk that actually ran may buy backoff, and an enumeration blip is not a departed Space. (2) The indexer recorded only successful writes, so deterministically un-indexable files (empty, larger than the size limit, non-regular) were indistinguishable from unseen ones and re-entered the indexing path on every pass; the lookup state now carries a stat-fingerprinted terminal decline alongside the committed mtime, and any change to the file clears it. Transient problems (a cloud folder that isn't answering, timeouts, embedding failures) are NEVER remembered as terminal. Honest bounds, do not overstate in public copy: this eliminates repeated indexing ATTEMPTS, not filesystem checks — a declined file still gets looked at each pass, which is exactly what makes it recover automatically; and NO worker-process/kill reduction is claimed (no post-landing data). Keep public copy non-technical — no verdict/memo/fingerprint/patrol/watcher internals, and no numbers for background work. -->
- **Rebel stops re-checking the files it could never open anyway** — Rebel looks over your files in the background to keep search current. It kept re-examining the same handful it can't index — an empty file, a file too large — asking the same question every time and getting the same answer. It now remembers the answer until the file itself changes, and picks the file straight back up when it does. Cloud folders got the same treatment: Rebel still checks just as often that your drive is reachable, and re-reads it the moment one reconnects or you point it at a new folder — but a folder sitting there perfectly content gets looked in on periodically instead of re-read every five minutes. Nothing tells Rebel when a cloud folder changes, so it has to go and look: the trade is that a file you edited on another device can take up to half an hour to turn up in search. Quieter laptop, slightly more patient search. Asked once, answered once.
<!-- detail: 260814_attributed-progress-card (internal planning doc; whole run, replacing the earlier Stage 10-only note). The live work card now merges main-thread and assistant actions into one time-ordered attributed stream (four visible rows plus the earlier-actions disclosure), every row carrying a typed action icon and a stable ownership mark: brand-tinted Bot for Rebel, job-initial monogram for assistants, neutral ? for malformed attribution. Assistant identity cards lead with the job name from the Agent tool's new optional description, keep model labels secondary, and say in plain language when the model that ran differs from the one requested. Blocked tasks moved off the rail onto the plan, with an amber count beside the planning progress. The quip footer is permanently occupied while Rebel works (13-second cadence) and all three quip corpora were recalibrated for permanent display. Periodic narration, journal-digest-backed one-sentence updates about the most useful thing learned so far, now ships enabled on every desktop turn with a live renderer window: the enableTurnNarration flag is deleted and pacing (10s entry / 45s interval / max 6 per turn) is enforced by a tested pure gate. One prioritized card-level announcer carries terminal/needs/error, then commentary, then the current attributed action. Technical details, earlier actions, assistant completion trace, hover detail and the completion receipt remain reachable. Keep public copy non-technical — no event-fixture, attempt, or admission internals. -->
...
<!-- detail: 260814_cloudwalk-load-reduction (internal planning doc; CE2 run). buildLocalManifest (src/main/services/cloud/cloudWorkspaceSync.ts) now records a size-derived marker (oversized:v1:<sizeBytes>, OVERSIZED_MARKER_PREFIX) instead of content-hashing files in the (UPLOAD_SIZE_LIMIT 7MB, MAX_FILE_SIZE 50MB] band — files too big to upload but small enough to keep in the manifest. The marker is never persisted: recordLastPushedEntry rejects marker-shaped entries and load() filters them on hydration, so lastPushedManifest (the on-disk map) stays marker-free by construction. Oversized paths stay present in the local manifest (omitting them would trip getDeletedFiles → a cloud delete), read as permanently-changed until shouldSuppressOversizedPush filters them out of the push set, and the TOCTOU fingerprint stays scoped to the post-suppression set. Files >50MB retain the existing omission behaviour. Warn memoization is size-keyed (stable across mtime-only touches, changes on size change) and resets on restart. A Darwin-only zero-block (stat.blocks === 0 && stat.size > 0, likely-placeholder) counter ships in the manifest log payload as measurement only — no behaviour change. Rejected alternative: recording oversized entries in lastPushedManifest would trip getCloudMissingFiles every cycle (spurious re-push warns) — do not do this. Keep public copy non-technical — no marker/sentinel/TOCTOU/fingerprint/memoization internals; say "files too big to upload" not "oversized band". -->
...
<!-- detail: 260814_vision-lookup-tool (internal planning doc; CE2 run). Some of the models Rebel runs on — including the fast, inexpensive ones used on Mindstone plans — cannot read images at all. Until now, an image that turned up in the middle of a task (a picture file Rebel opened, a screenshot it took, an image returned by a connector) came back as a note saying it could not be seen, and the only way forward was to switch the whole conversation to a different model. Rebel now asks an image-reading model one focused question about that image — "what is the Q3 figure in this chart?" rather than "describe this picture" — and carries on with the answer. On Mindstone plans this is included, and you are never interrupted. If the answer would have to be paid for per use, Rebel asks first, and you can allow it once for that conversation or always. Images you paste or attach yourself work differently and are unchanged: Rebel still tells you plainly that it cannot see them, and offers the one-tap switch to a model that can. Rebel treats anything read out of an image as information, never as instructions — text inside a picture cannot tell Rebel what to do. Keep public copy non-technical — no tool name, handle, fencing, or model-role internals. -->
...
<!-- detail: 260819_terminal-outcome-false-quarantine (internal planning doc; CE2 bug_mode). A bookkeeping summary of the whole work record was being treated as authority for a finished answer, so some sound answers triggered the protective save refusal and were later labelled Preserved. Answer authority now comes from the saved terminal record itself; narrowly proved raw records heal before routine normalization, while genuinely conflicting answer content stays protected. The refusal notice clears only after a later successful content-bearing save on desktop or mobile, never after metadata or checkpoint traffic. Keep public copy non-technical and do not include conversation content, titles, names, or identifiers. -->
...
<!-- detail: 260815_company-managed-subscription-ui (internal planning doc; CE2 run). Employees whose Mindstone plan was assigned by a company admin on the web platform saw Stripe-centric subscription UI in Settings — a billing portal with nothing in it to manage, upgrade and cancellation buttons that could only confuse — and, during first-run setup, live "Subscribe" cards for the very plan they already had, because the desktop discarded the source discriminator the platform already sends. The source now flows end to end: the app serves the company plan from its own cached company envelope (the Stripe-linked status endpoint never knows about company plans), checkout/portal actions fail closed while the app cannot yet tell who manages the plan and refuse outright when the company does, and both Settings and onboarding render a truthful "provided by your company, managed by your admin" card with no purchase or portal controls; lifecycle notices point at the admin. Personal (self-purchased) subscribers see exactly what they saw before. Keep public copy non-technical — no endpoint/precedence/guard internals. -->
...
<!-- detail: When a recording service handed Rebel the same meeting under a new identifier, or Rebel lost track of what it had already handled, the same transcript could be filed again and again — quietly repeating background work through the night. That loop is stopped: Rebel recognises a recording by what it actually is rather than by the label the service gave it, and remembers what it has already filed. Deliberately an outcome claim, not a counting one — in rare recovery cases (an interrupted app, a file it briefly could not read) Rebel will take another run at a transcript rather than give up on it, and it gives up loudly when it runs out of tries. Separately, meetings captured by the cloud meeting bot are now shared once the polished transcript arrives rather than at the moment of saving — sometimes a few hours later. Keep public copy non-technical — no ledger/journal/retry-budget internals. -->
...
<!-- detail: 260814_pending-recovery-rearch (internal planning doc; CE2 bug_mode). Root incident 2026-08-14: one malformed legacy pending file made the whole-directory strict inventory fail, which refused every identity-precise deny/discard with "Rebel could not verify every staged file on disk. Refresh and try again." — unactionable by construction, since quarantine is explicit-only and nothing on the refusal path ever invokes it. The run: read-failure classification in the strict scan (ENOENT=absent, transient=bounded retry, hard=terminal); coded refusals + a counts-only census so error copy is chosen by code, never string-matching; the coalesce-resurrection mechanic severed across all five gesture families; a host-local pendingFileBinding minted at approval time (store v3→v4, additive/safe); bound per-file verify-then-act for deny/discard with fall-back-to-strict-on-any-miss; and the whole-directory scan re-contracted from precondition to post-action residual check whose degradation marks the outcome honestly-incomplete (residualUnverified) instead of vetoing the action. Keep public copy non-technical — no inventory/census/binding/scanner/coalesce internals; say "set-aside note" and "the one note you're acting on". -->
...
<!-- detail: 260813_sync-write-path-async (internal planning doc; CE2 follow-up to the 260812 dead-cloud-mount turn-hang run). The cloud workspace sync write path — pull writes, the atomic write, manifest stat, the hash stream, push reads, deferral/migration probes, conflict gates — was converted from synchronous filesystem calls to bounded async operations raced against an abort timeout, with a fired bound treated as a transient cloud problem: the sync round stops early, Rebel re-checks the drive, and the next healthy round converges (integration-tested, including a verdict flipping mid-round). A static guard now rejects any future unguarded file read in the sync files. Known bounded residual, deliberately deferred: an operating-system thread already wedged on the dead drive stays parked until the drive recovers — it cannot block the app. Keep public copy non-technical — no fs/promises/AbortSignal/threadpool internals. -->
...
<!-- detail: 260805_local-model-integrity Stage 8. Desktop STT now pins immutable Hugging Face commits plus every consumed file and gives historical installs a one-time full verification with manifest-keyed receipts; attributable corrupt installs are quarantined, while unattributed legacy paths are left untouched and reported unavailable for explicit re-download. Desktop/cloud embeddings provision immutable, complete manifests into a Rebel-owned versioned root with local-only loading; Ollama verifies archive, executable, installed executable, and macOS Team ID. Build-time Node, ripgrep, Moonshine native inputs, and Windows MSVC runtime inputs are also pinned. Mobile model files, Recall native payloads, and gpu-worker ORT WASM remain explicit follow-ups. Keep public copy non-technical and do not claim every model is covered. -->
...
<!-- detail: 260814_convo-drop-crash-hang Stages 2+3 (internal planning doc; CE2 bug_mode; follow-up to the 260812 dead-cloud-drive fix, prompted by a colleague's incident on the same pre-fix build). The bug-report durable-write timeout toast no longer claims "your device storage may be busy" — an unearned cause claim (nothing measured storage; the incident's real cause was a wedged cloud mount starving the local write of a worker thread); the ratified cause-neutral copy says only that the save took too long, the report is still here, try again. Outbox hygiene: a failed save attempt now cleans up its temporary file on every pre-commit failure, stale temporary files are swept on later delivery passes, and a save that finishes after the timeout is logged and delivered promptly instead of sitting silently until the next restart. Keep public copy non-technical — no outbox/tmp/drain/libuv/durable-write internals. -->
...
<!-- detail: 260814_convo-drop-crash-hang Stage 1 (same run). The diagnostic-bundle Chief-of-Staff README collector moved off a raw unbounded fs.readFile onto the bounded killable lane, sharing one deadline fed from the collector's real remaining budget and consulting the cloud-mount circuit breaker, so generating a bundle while a cloud drive is unresponsive can no longer park worker threads or stall the collector; link-form and unclassifiable Chief-of-Staff roots now also route through the cloud lane, and space-scan failures are logged without raw paths. Keep public copy non-technical — no collector/lane/libuv/circuit internals. -->
...
<!-- detail: integrity-fail-closed (internal planning doc; CE2 bug_mode; internal Sentry ticket). The session-persistence integrity guard (which refuses any save that would lose an answer) had no recovery path: a stored answer byte-identical to the canonical replay but with drifted attempt/endedWith/createdAt bookkeeping stamps could never be repaired (repair was insertion-only) and blocked every subsequent write — permanent "Conversation not saved" toast, disk file frozen, cloud outbox retrying forever. Such results are now reconciled in place on load and on write (only the three stamp fields canonicalized, everything else byte-preserved), gated on byte-exact nonblank text plus prior-proven provenance or the hydration/prior-absent policy; the same shared code heals desktop and cloud. Genuinely ambiguous shapes (answer text differing from the replay, multiple results, deletion-ledger-protected) intentionally STAY refused-and-visible — the Stage 4 live witness proved the originally-reported session is in that residual class, so public copy must NOT promise every stuck conversation heals. Telemetry now reports parked-vs-retrying honestly. Keep public copy non-technical — no guard/replay/stamp/analyzer internals; say "bookkeeping", "identical to the letter", "protective refusal". -->
...
<!-- detail: 260809_mobile-answered-card-ux Stages A + B (internal planning doc; CE2 bug_mode; user-reported via a mobile screenshot of a permission-clarification card that stayed pinned after being answered). On mobile, every question card Rebel asked — pending or already answered — rendered pinned above the message box at full size, with no way to move, shrink, or hide it; desktop renders only pending cards there and lets answered/skipped cards take their place inline in the conversation where they scroll away naturally. (A) Answered and skipped question cards now render inline in the mobile conversation at the point Rebel asked them, so only questions still waiting for an answer stay pinned above the keyboard; the shared anchoring algorithm (the backward walk that ties each card to its turn) was extracted into a pure cloud-client utility, userQuestionPresentation.ts, so mobile and desktop place the cards identically. (B) An answered question summary can now be hidden via a Hide action in its header, with an "Answered summary hidden" toast and an Undo; hiding is remembered per conversation on that device and never changes or deletes the answer, which lives in the conversation's event history. Mobile only — desktop already scrolled answered cards into the transcript and stays scroll-away-only (no Hide there). Keep public copy non-technical — no composer/accessory/anchoring/event-history internals; say "above where you type" and "into the conversation". -->
...
<!-- detail: 260809_mobile-answered-card-ux Stage D (internal planning doc; same run). When Rebel couldn't load a conversation's permissions, a single "Couldn't load conversation permissions" row appeared with only Try again and stayed put until the next successful load — one transient failure became a permanent line above the keyboard, often beside a card proving those permissions had already been granted. The notice now has a separate Hide beside Try again (mobile and the web companion); hiding it shows a brief "Permissions notice hidden — this didn't change any permissions." toast with no Undo, because the notice deliberately re-appears on any later genuine failure (no suppression state — a hidden error never masks a future one). Every failed load is logged centrally; the shared approval store gained clearConversationGrantError, and load and revoke errors are now rendered as separate inputs. Keep public copy non-technical — no grant/store/suppression/error-kind internals. -->

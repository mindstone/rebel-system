...
<!-- detail: 260820_fox3783-reconnect-cta (internal planning doc; CE2 bug_mode; FOX-3783 / REBEL-7TB). The Automations "Reconnect" CTA (and 5 sibling deep-link CTAs across toasts/notices) opened the Settings dialog without switching the active flow surface, so a surface-sync effect closed it instantly and the Settings view never mounted — the button did nothing from any non-Settings surface. Root cause was a per-caller footgun (6 of 27 settings-open callers omitted the surface switch); fixed by construction at the shared entry point so every opener activates the Settings surface, then the Automations reconnect CTAs were deep-linked to the exact provider card (ChatGPT Pro / OpenRouter). Deferred follow-ups: return-to-origin after closing Settings, and a one-click reconnect on the "ChatGPT rejected your connection" card (tokens still present, so it shows Disconnect). Keep public copy non-technical — no surface/hook/section internals. -->

...
<!-- detail: Rebel guards your history so strictly that it refuses to save anything that might lose an answer — but one harmless bookkeeping wrinkle could trip that guard and then nothing would save at all: a permanent "Conversation not saved" error, a frozen file, and a lot of quiet retrying. Now, when the saved answer is identical to the letter and only the bookkeeping drifted, Rebel repairs the bookkeeping in place and life goes on. Cases that are genuinely ambiguous stay safely refused and visible — so a small number of stuck conversations will still need a human, and that's deliberate. -->

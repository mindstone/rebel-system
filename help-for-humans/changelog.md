...
<!-- detail: 260703_fox-3604-doubleword-provider. Added Doubleword.ai as a new BYOK pay-per-use provider (ModelProviderType-only preset, Together/Cerebras class) offering cheap open-source models (GLM 5.2, DeepSeek V4 Flash) via Doubleword's async/flex tier. Positioned for background/automation work, NOT live chat — spike measured ~1 min per step throughout (no cross-call speedup observed), so latency framing must stay honest ("about a minute per step", not "warm up then fast"). Every Doubleword request carries service_tier:'flex'; BTS/background transport timeout extended to ~120s and the settings Test-button validation timeout to ~90s to survive the observed ~60s cold start. Hard-excluded from Council/Smart-picking for MVP (shared eligibility predicate; the local-proxy timeout those surfaces use is shorter than Doubleword's cold start) — the membership toggle chips now show a disabled state + tooltip explaining why, in both the add-model wizard and the settings model-team views. DeepSeek V4 Pro intentionally NOT offered yet (catalog id collision with an existing Together entry; needs per-provider pricing to re-enter safely). Do NOT mention "flex tier", "service_tier", ModelProviderType, cold-start/queue internals, Together/Cerebras internals, or the deferred V4 Pro reason in public copy — keep it to "cheap open-source models, about a minute per step, best for background/automation, bring your own key." -->
- **New provider: Doubleword** — Bring your own API key and get access to cheap, open-source models (GLM 5.2 and DeepSeek V4 Flash) built for background and automation work. Each step takes about a minute, so it's not the model to reach for in a live back-and-forth — but for jobs that can run in the background, it's a budget-friendly option. Add it from Settings → Your Models.

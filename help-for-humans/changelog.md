...
<!-- detail: 260705_win-stt-badalloc (REBEL-6FM). On memory-constrained Windows machines, on-device (Parakeet/sherpa-onnx) speech-to-text could throw a raw std::bad_alloc during recognizer init — surfacing as an opaque "bad allocation" error — partly because the ~670MB model was rebuilt on every transcription instead of reused. Fix caches the recognizer, and reclassifies the failure as a clear, non-retried, actionable error. Keep public copy non-technical — no sherpa-onnx/recognizer-cache/error-category internals; do not name REBEL-6FM. -->

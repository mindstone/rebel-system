...
<!-- detail: The connector improvement wave landed entirely in the public mindstone/mcp-servers repo (github.com/mindstone/mcp-servers): new tools across microsoft-teams (search_messages, reply_to_message, get/set presence), microsoft-mail (get/set_automatic_replies, BCC on send/draft/compose), microsoft-calendar (find_meeting_times, per-attendee RSVP status), microsoft-sharepoint (site page authoring, resumable/binary uploads), microsoft-files (read_document for docx/pptx, binary upload), google-workspace (contacts create/update, Gmail vacation responder + send-as), and google-analytics (audience exports, async report tasks) — plus a google-workspace auth-reconnect signal fix and replit-ssh/pandadoc hardening. -->
- **Your Microsoft and Google connectors learned new tricks — in the open** — Teams: search messages, reply in a thread, set your presence. Outlook: out-of-office replies and BCC. Calendar: find meeting times that work for everyone, and see who's accepted. SharePoint and Files: upload large files, read Word and PowerPoint content. Google: manage contacts, set a Gmail vacation responder. Every line of it is open source — anyone can read, fork, or contribute. New tricks, no closed doors.

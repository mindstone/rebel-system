...
- **Source files**: Store `.mermaid` files in `help/diagrams/` (or your preferred documentation directory)
...
**Always generate both SVG and PNG**:
```bash
...
1. Generate filename prefix: `npx tsx src/ts/cli/sequential-datetime-prefix.ts help/diagrams/` if available, otherwise `date +%y%m%d` (add letter suffix if needed)
2. Create/edit `.mermaid` file with the generated prefix in `help/diagrams/`
3. Generate SVG and PNG:
   ```bash
   npx mmdc -i help/diagrams/FILENAME.mermaid -o help/diagrams/FILENAME.svg -w 1400 -H 1600 -s 2 -b transparent
   npx mmdc -i help/diagrams/FILENAME.mermaid -o help/diagrams/FILENAME.png -w 1400 -H 1600 -s 2 -b transparent
   ```
4. Open the SVG in default app: `open help/diagrams/FILENAME.svg` (or `xdg-open` on Linux)
5. Output the generated filenames

---
name: sd-string-displacement-find-replace
description: "Reference guide for the sd (string displacement) CLI tool - a modern, safe find-replace tool with dry-run mode and literal string handling ideal for codebase refactoring."
...
sd is a modern, intuitive find-replace CLI tool designed for safe codebase refactoring, with excellent dry-run capabilities and literal string handling perfect for LLM automation. It is **optional** and mainly useful when you need to rename or replace strings across many files at once.
...
- `ripgrep` search tool: See [`ripgrep-search.md`](ripgrep-search/SKILL.md) for fast, large-folder searching that pairs well with `sd` when you need to first find and then replace strings across big workspaces.
...
You only need `sd` if you're doing **large, multi-file rename/replace operations** and want a dedicated tool to speed that up.
...
- **Windows download**: [sd releases](https://github.com/chmln/sd/releases) — download the latest `sd.exe` and add its directory to your `PATH`.
- **Windows recommendation**: On Windows, especially in corporate/locked-down environments, treat `sd` as **optional**. Prefer using it only when a human (or IT) has already installed it and you’re doing large, multi-file refactors; don’t expect agents to bootstrap or manage the Windows installation themselves.

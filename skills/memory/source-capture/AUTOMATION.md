...
5. **(Mandatory) Surface actionable items to Actions** — for sources that need user action, create action items:
   ...
   - **(Mandatory) Upcoming-meeting prep — query the calendar directly; do NOT wait for meetings to appear in steps 2-3.** Steps 2-3 only scan PAST activity since [LAST_EXECUTED_SUCCESS], so upcoming meetings will never surface there. Explicitly query your calendar tool for UPCOMING events from now through the end of tomorrow: set a FORWARD time window (start = now, end = end of tomorrow) using the exact argument names from your selected calendar tool's schema — do not assume field names (e.g. GoogleWorkspace `list_workspace_calendar_events` uses `time_min`/`time_max`; Microsoft calendar uses `startDateTime`/`endDateTime`). Do not reuse the past-activity window from step 2. If the default calendar output omits attendee/response details, request the full event detail your tool offers (e.g. GoogleWorkspace `return_json: true`). For each upcoming meeting, create an action item with title "Prep for [meeting title]", `category: 'meeting-action'`, `relevantDate` set to the meeting start time, `dueBy` set to 1h before the meeting, and a `completionSpec`: `{ successCriterion: "The meeting has taken place.", signals: [{ kind: "calendar-event", startsAt: <meeting start epoch ms>, eventId: "<event id if the tool provides one>" }] }` so Rebel can tell the prep is no longer needed once the meeting is over. **Create by default — the attendance and prep-notes filters are BEST-EFFORT, not hard preconditions:** skip a meeting ONLY when the tool output CLEARLY shows the user declined / is not attending, or that prep already exists for it. When you cannot determine attendance or whether prep notes exist (e.g. the connector doesn't expose it), CREATE the item anyway — never let an unverifiable precondition block creation (that silent skip is exactly the bug this step fixes). Run this every time — it does NOT depend on any sources being captured. Source Capture owns meeting-prep creation; Morning Triage does not create these. **Deduplicate**: skip when a "Prep for …" item already exists for the same meeting occurrence — match on meeting title + start time, checking BOTH active and archived items (recurring meetings are expected; only skip the same occurrence).
   - Then, **for each source captured in steps 2-3**, evaluate: does this need the user to DO something?
   - **Emails needing a reply**: received email from a real person (not automated/newsletter), no reply sent yet, seems to expect a response → create action item with title "Reply to [person] about [subject]", `category: 'follow-up'`, `dueBy` set to 48h from now, email reference in `references`, `source: { kind: 'automation', automationId: 'system-source-capture', automationName: 'source-capture' }`. ALWAYS include `references: [{ kind: 'email', threadId: '<thread-id>', messageId: '<message-id>', provider: 'gmail' or 'outlook' }]` — this is required for reply detection and correct CTA labeling. Also set `fromEmail` (the sender's address, lowercased) and `fromDomain` (the part after the "@"), and a `completionSpec`: `{ successCriterion: "A reply has been sent in this email thread.", signals: [{ kind: "email-thread", threadId: "<same thread-id as the reference>", provider: "gmail" or "outlook" }] }`. These let Rebel later confirm the reply landed and learn sender-scoped preferences.
   ...
   - Cap at 5 new source-derived items per run to avoid overwhelming Actions. This cap does NOT apply to the upcoming-meeting prep items above — those are naturally bounded by the now→end-of-tomorrow window and auto-expire after the meeting.
6. **(Mandatory) Actions freshness check** — clean up stale action items:
   ...
   - Archive confirmed-complete items via rebel_inbox_update with archived=true, and always pass `resolution` so the archive lands in the right place:
     ...
     - **Always confirm the item's identity.** When you set `resolution`, also pass `expectedTitle` — the exact title of the item you are resolving, copied verbatim from the list — and `expectedThreadId` when the item has an email reference. The server rejects the call if they don't match the id, so this guarantees the evidence you found is applied to the right item. This matters most when you are resolving several items in one pass: pair each item's evidence with *that* item's id and title, never another's.

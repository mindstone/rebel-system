...
These conventions are aligned with Anthropic's MCP guidance on tool clarity and namespacing. See [Implement tool use](https://docs.anthropic.com/en/docs/agents-and-tools/tool-use/implement-tool-use) and [Writing tools for agents](https://www.anthropic.com/engineering/writing-tools-for-agents). Anthropic does not mandate snake_case, but their examples are heavily snake_case and that is the prevailing MCP ecosystem convention, so this doc uses snake_case as house style. For Rebel's canonical naming standard, see [mcp-development-standard.md § Tool and Parameter Naming](mcp-development-standard.md#3-tool-and-parameter-naming).
...
6. **Use consistent parameter names across tools**: When multiple tools accept the same concept (e.g., a channel identifier), use the same parameter name everywhere (e.g., always `channel`, not `channel` in one tool and `channel_id` in another). Inconsistent parameter names cause LLMs to guess wrong and waste turns on avoidable errors.

### Cross-tool parameter consistency

Use the same canonical parameter name everywhere a tool exposes the same concept. The canonical naming standard lives in [mcp-development-standard.md § Tool and Parameter Naming](mcp-development-standard.md#3-tool-and-parameter-naming); the common cases are:
...
MCP servers that persist tokens, API keys, or account configuration to disk MUST follow these rules:

**File permissions:** Always write credential files with `{ mode: 0o600 }` (owner read/write only). Without this, Node.js defaults to umask-based permissions (typically `0o644` = world-readable), allowing any local process to read OAuth refresh tokens and gain persistent account access.

**Directory permissions:** Create credential directories with `{ mode: 0o700 }` (owner access only).

**Atomic writes:** Use write-to-temp + rename to prevent token corruption on crash:

```typescript
// Good — atomic write with restrictive permissions
const tmpPath = `${tokenPath}.tmp.${process.pid}`;
await fs.writeFile(tmpPath, JSON.stringify(tokenData, null, 2), { mode: 0o600 });
await fs.rename(tmpPath, tokenPath);

// Bad — non-atomic, world-readable by default
await fs.writeFile(tokenPath, JSON.stringify(tokenData, null, 2));
```

**No tokens in logs:** Never log token values, refresh tokens, or API keys to stderr. Log token lifecycle events (refresh started, refresh succeeded, token expired) without the actual token content.

**Windows compatibility:** The `mode` parameter is silently ignored on Windows (which uses ACLs). Always include it — it's safe on all platforms.

For the full security baseline specification, see [mcp-development-standard.md § Security Baseline](mcp-development-standard.md#8-security-baseline).
